...
Функция ОписаниеФорматаДляОтправкиФайла(DocumentType) Экспорт 
	...
	Если ВРег(ТипДокумента) = ВРег("NonformalizedProforma") Тогда
		ТипДокумента = "ProformaInvoice";
	КонецЕсли;
	
	Результат = МетодКлиента("Модуль_Клиент", "НовоеОписаниеФорматаЭлектронногоДокумента");
	Результат.ТипКонтента		 = ТипыКонтента.NonformalizedContent;
	Результат.ВерсияФормата		 = "v1";
	Результат.ФункцияДокумента	 = "default";
	Результат.ИмяТипа			 = ТипДокумента;
...
Функция НовыйДокументПакета(PackageSendTask, ДокументПакета, ОбщиеПараметрыОтправки)
	...
	Иначе
		...
		СтруктураТитула	 = СформироватьТитулДокумента(ТипКонтента, ВерсияФормата, ПараметрыДокумента, ОбщиеПараметрыОтправки);
...
Функция НовыйДокументПакетаУстаревшейВерсии(PackageSendTask, ДокументПакета, ОбщиеПараметрыОтправки, Organization)
	...
	ИначеЕсли ПараметрыДокумента.DocumentType = "XmlTorg12" Тогда
		
		Если ЗначениеЗаполнено(ПараметрыДокумента.ФункцияУПД) Тогда
		...
		Иначе
			...
			СтруктураДанныеСервера= ДанныеДляDocumentToSendНаСервере(ПараметрыДокумента.DocumentType, ПараметрыДокумента, ОбщиеПараметрыОтправки, ФИОПодписанта);
			ВызватьИсключениеПриОшибкеВалидации(ДокументПакета, СтруктураДанныеСервера.СписокОшибок);
			
			DocumentToSend=	DocumentToSend_Formalized_Универсальный(PackageSendTask, ПараметрыДокумента.DocumentType, СтруктураДанныеСервера.XmlTorg12Content, "XmlTorg12Content");
			МетодКлиента("Модуль_Клиент", "ЗаполнитьПодписанта", DocumentToSend.Content, Organization);
			
		КонецЕсли;
		
	ИначеЕсли ПараметрыДокумента.DocumentType = "XmlAcceptanceCertificate" Тогда
		...
		Иначе
			...
			DocumentToSend=	DocumentToSend_Formalized_Универсальный(PackageSendTask, ПараметрыДокумента.DocumentType, СтруктураДанныеСервера.XmlAcceptanceCertificateContent, "XmlAcceptanceCertificateContent");
			МетодКлиента("Модуль_Клиент", "ЗаполнитьПодписанта", DocumentToSend.Content, Organization);
			
		КонецЕсли;
		
	ИначеЕсли ПараметрыДокумента.DocumentType = "UniversalTransferDocument" Тогда
	...
	ИначеЕсли ПараметрыДокумента.DocumentType = "UniversalCorrectionDocument"
		ИЛИ ПараметрыДокумента.DocumentType = "InvoiceCorrection" И ЗначениеЗаполнено(ПараметрыДокумента.ФункцияУПД) Тогда
	...
	ИначеЕсли ПараметрыДокумента.DocumentType = "Invoice" Тогда
		...
		Иначе
			...
			Если ТипСФ = "InvoiceCorrectionRevision" ИЛИ ТипСФ = "InvoiceCorrection" Тогда
				...
				СтруктураДанныеСервера= ДанныеДляDocumentToSendНаСервере(ТипСФ, ПараметрыДокумента, ОбщиеПараметрыОтправки, ФИОПодписанта);
				ВызватьИсключениеПриОшибкеВалидации(ДокументПакета, СтруктураДанныеСервера.СписокОшибок);
				
				DocumentToSend=	DocumentToSend_Formalized_Универсальный(PackageSendTask, ТипСФ, СтруктураДанныеСервера.InvoiceCorrectionContent, "InvoiceCorrectionContent");
			...
			Иначе
				...
				DocumentToSend=	DocumentToSend_Formalized_Универсальный(PackageSendTask, ТипСФ, СтруктураДанныеСервера.InvoiceContent, "InvoiceContent");	
...
&НаСервере
Функция ДанныеДляDocumentToSendНаСервере(DocumentType, СтруктураДокументПакета, ОбщиеПараметрыОтправки, ФИОПодписанта)
	
	ДокументСсылка	= СтруктураДокументПакета.ДокументСсылка;
	ТипДокумента	= СтруктураДокументПакета.ТипДокумента; 
	
	ДополнительныеПараметры = МетодСервера("Модуль_ИнтеграцияУниверсальный", "ДополнительныеПараметрыПолученияКонтента");
	ДополнительныеПараметры.ДополнительныеСведенияСтрокой = СтруктураДокументПакета.ДопСведения;
	ДополнительныеПараметры.ФункцияДокумента = СтруктураДокументПакета.ФункцияУПД;
	...
	Если DocumentType = "XmlTorg12" Тогда
		...
		Протоконтент = МетодСервера("Модуль_ИнтеграцияУниверсальный", "КонтентНакладной172"
						, ДокументСсылка
	...
	ИначеЕсли DocumentType = "XmlAcceptanceCertificate" Тогда 
		...
		Протоконтент = МетодСервера("Модуль_ИнтеграцияУниверсальный", "КонтентАкта172"
						, ДокументСсылка
						, ДополнительныеПараметры);
	...
	ИначеЕсли DocumentType = "UniversalTransferDocument" Тогда 
		...
		Протоконтент = МетодСервера("Модуль_ИнтеграцияУниверсальный", "КонтентУПД155"
						, ДокументСсылка
						, ДополнительныеПараметры);
	...
	ИначеЕсли 	DocumentType = "UniversalCorrectionDocument"
			ИЛИ DocumentType = "InvoiceCorrection" И ЗначениеЗаполнено(ДополнительныеПараметры.ФункцияДокумента) Тогда 
		
		Протоконтент = МетодСервера("Модуль_ИнтеграцияУниверсальный", "КонтентУКД189"
						, ДокументСсылка
						, ДополнительныеПараметры);
	...
	ИначеЕсли DocumentType = "InvoiceCorrectionRevision" ИЛИ DocumentType = "InvoiceCorrection" Тогда
		...
		Протоконтент = МетодСервера("Модуль_ИнтеграцияУниверсальный", "КонтентКорректировочногоСчетаФактуры93"
						, ДокументСсылка
						, ДополнительныеПараметры);
	...
	Иначе
		...
		Протоконтент = МетодСервера("Модуль_ИнтеграцияУниверсальный", "КонтентСчетаФактуры93"
						, Документссылка
						, ДополнительныеПараметры);
...
&НаКлиенте
Функция СформироватьТитулДокумента(ТипКонтента, ВерсияКонтента, ПараметрыДокумента, ОбщиеПараметрыОтправки)
	
	БазовыйФормат = МетодКлиента("Модуль_Клиент", "БазовыйФорматВерсииКонтента", ВерсияКонтента);
	...
	ДанныеСотрудника	 = МетодКлиента("Модуль_РаботаССерверомДиадок", "ДанныеКонтекстаДиадок", ОбщиеПараметрыОтправки.BoxId, "ДанныеСотрудника");
	...
	Результат = СформироватьТитулДокументаНаСервере(ТипКонтента
		, ВерсияКонтента
		, ПараметрыДокумента
...
&НаСервере
Функция СформироватьТитулДокументаНаСервере(ТипКонтента, ВерсияКонтента
	, ПараметрыДокумента, ОбщиеПараметрыОтправки
	, ДанныеОрганизации, ДанныеСотрудника
	, ПолномочияПодписанта)
	
	ДополнительныеПараметры = МетодСервера("Модуль_ИнтеграцияУниверсальный", "ДополнительныеПараметрыПолученияКонтента");
	ДополнительныеПараметры.ДополнительныеСведенияСтрокой = ПараметрыДокумента.ДопСведения;
	ДополнительныеПараметры.ФункцияДокумента = ПараметрыДокумента.ФункцияУПД;
	...
	ПротоКонтент = СформироватьПротоКонтент(ТипКонтента
	, ПараметрыДокумента, ДополнительныеПараметры
	, ДанныеОрганизации, ДанныеСотрудника
	, ПолномочияПодписанта);
	
	ФайлТитула = СформироватьФайлТитула(ПротоКонтент, ТипКонтента, ВерсияКонтента);
	
	СписокОшибок = ВыполнитьВалидациюПротоКонтента(ПротоКонтент, ТипКонтента);
...
&НаСервере
Функция СформироватьПротоКонтент(ТипКонтента
	, ПараметрыДокумента, ДополнительныеПараметры
	, ДанныеОрганизации, ДанныеСотрудника
	, ПолномочияПодписанта)
	...
	Если ТипКонтента = ТипыКонтента.Utd820SellerContent Тогда
		...
		Результат = МетодСервера("Модуль_ИнтеграцияУниверсальный", "КонтентУПД820"
			, ДокументСсылка
			, ДополнительныеПараметры);
		
	ИначеЕсли ТипКонтента = ТипыКонтента.UtdSellerContent Тогда
		
		Результат = МетодСервера("Модуль_ИнтеграцияУниверсальный", "КонтентУПД155"
			, ДокументСсылка
			, ДополнительныеПараметры);
		
	ИначеЕсли ТипКонтента = ТипыКонтента.UcdSellerContent Тогда
		
		Результат = МетодСервера("Модуль_ИнтеграцияУниверсальный", "КонтентУКД189"
			, ДокументСсылка
			, ДополнительныеПараметры);
		
	КонецЕсли;
	
	ДанныеПодписанта = ДанныеПодписанта(ДанныеСотрудника, ДополнительныеПараметры.ПараметрыСогласования);
	
	ЗаполнитьПодписантаВПротоКонтенте(Результат, ТипКонтента, ДанныеОрганизации, ДанныеПодписанта, ПолномочияПодписанта);
...
&НаСервере
Функция ВыполнитьВалидациюПротоКонтента(ПротоКонтент, ТипКонтента)
...
&НаСервере
Функция СформироватьФайлТитула(ПротоКонтент, ТипКонтента, ВерсияКонтента)
	...
	ПараметрыФормированияФайла.ТочностьЦены	 = Неопределено;
...
&НаСервере
Функция ДанныеПодписанта(ДанныеСотрудника, ПараметрыСогласования)
...
&НаСервере
Процедура ЗаполнитьПодписантаВПротоКонтенте(ПротоКонтент, ТипКонтента, ДанныеОрганизации, ДанныеПодписанта, ПолномочияПодписанта);
...
Процедура ЗаполнитьПараметрыДокументаПоСтруктуреВПФ(ПараметрыДокумента, ДокументПакета, СтруктураВПФ)
	...
	ПараметрыДокумента.Вставить("ФорматОтправки", ОписаниеФорматаДляОтправкиФайла(ПараметрыДокумента.DocumentType));
...
Функция DocumentToSend_Formalized_Универсальный(PackageSendTask, DocumentType, СтруктураДанныхContent, ОписаниеСтруктурыДанных)
	...
	Если DocumentToSend.Content.Type = "UniversalCorrectionDocument" Тогда
		
		DocumentToSend.Content.InvoiceCorrectionTable.ManualInitialization = Истина;
		
	ИначеЕсли DocumentToSend.Content.Type = "InvoiceCorrection" Тогда
...
Процедура ПослеОтправкиДокумента(ДокументПакета)
	...
	Если ДокументПакета.ФункцияУПД = "СЧФДОП" ИЛИ ДокументПакета.ФункцияУПД = "КСЧФДИС" Тогда 
	...
	МетодСервера(,"Установить_DocumentID_Для_Документ", ДокументПакета.ДокументСсылкаВ1С, ДокументПакета.DocumentId, ДокументПакета.OrganizationId, ДокументПакета.Type = "NonformalizedProforma");
	...
	Иначе
		Если ТипЗнч(ДокументПакета.ДокументСсылкаВ1С) = Тип(ПредставлениеСФВыданный) Тогда
			ОтразитьСобытиеВыгрузкиВСчетФактуре(ДокументПакета.ДокументСсылкаВ1С, ДокументПакета.Организация);
		ИначеЕсли НЕ (ДокументПакета.Type = "NonformalizedProforma" И ТипЗнч(ДокументПакета.ДокументСсылкаВ1С) = Тип(ПредставлениеРеализации)) Тогда
...
Процедура УстановитьСвязиДляDocument(Document, СоответствиеCustomIdСсылке1СПакета)
	...
	ЭтоФайл= 			СтруктураСоответствия.ЭтоФайл;
	...
	ИначеЕсли   ЭтоФайл И (DocumentType = "Nonformalized" ИЛИ DocumentType = "AcceptanceCertificate" ИЛИ DocumentType = "ReconciliationAct"
