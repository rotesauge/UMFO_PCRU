...
//{ ТЕЛО ФОРМЫ
	...
	Процедура ЗаполнитьПараметрыДокументаВДереве(СтрокаДерева, Document)
		...
		СтрокаДерева.ТипДокумента	= МетодКлиента("Модуль_Клиент", "ПредставлениеТипаЭлектронногоДокумента", Document);
		...
		ОграничитьРезультатСчетФактурой = Document.Type = "Invoice"
		ИЛИ	МетодКлиента("Модуль_Клиент", "ЭтоУПД_ТипаСЧФДОП" , Document)
		ИЛИ МетодКлиента("Модуль_Клиент", "ЭтоУПД_ТипаСЧФ"	  , Document)
		ИЛИ МетодКлиента("Модуль_Клиент", "ЭтоУКД_ТипаКСЧФДИС", Document)
		ИЛИ МетодКлиента("Модуль_Клиент", "ЭтоУКД_ТипаКСЧФ"	  , Document);
		
		СтрокаДерева.ПервичныйДокумент = ПолучитьDocumentID_2_Документ(Document.DocumentId, Document.OrganizationId, ОграничитьРезультатСчетФактурой, Document.Direction);
		...
		Для Каждого СтрокаДерева из СтрокиДерева Цикл
			Если СтрокаДерева.Вкл Тогда	
				...
				Если ЗначениеЗаполнено(СтрокаДерева.ПервичныйДокумент) Тогда
					...
					ОчиститьИД_Документов1СсОдинаковымDocumentID = Document.Type = "Invoice"
					ИЛИ	МетодКлиента("Модуль_Клиент", "ЭтоУПД_ТипаСЧФДОП" , Document)
					ИЛИ МетодКлиента("Модуль_Клиент", "ЭтоУПД_ТипаСЧФ"	  , Document)
					ИЛИ МетодКлиента("Модуль_Клиент", "ЭтоУКД_ТипаКСЧФДИС", Document)
					ИЛИ МетодКлиента("Модуль_Клиент", "ЭтоУКД_ТипаКСЧФ"	  , Document);
					
					Установить_DocumentID_Для_Документ(СтрокаДерева.ПервичныйДокумент,,, ОчиститьИД_Документов1СсОдинаковымDocumentID);
...
//{ СЛУЖЕБНЫЕ ПРОЦЕДУРЫ И ФУНКЦИИ
	...
	Процедура СформироватьПечатнуюФормуПоФайлуДиадок()
		...
		ЭлементМассива.НомерДокумента = ТекущиеДанные.Номер;
		ЭлементМассива.ДатаДокумента = ТекущиеДанные.Дата;
		...
		ЭлементМассива.Документ1С = ТекущиеДанные.ПервичныйДокумент;
	...
	&НаСервере
	Процедура Установить_DocumentID_Для_Документ(ДокументССылка, DocumentId, OrganizationId, ОчиститьИД_Документов1СсОдинаковымDocumentID)
		МетодСервера(,"Установить_DocumentID_Для_Документ", ДокументССылка, DocumentId, OrganizationId,, ОчиститьИД_Документов1СсОдинаковымDocumentID);
	КонецПроцедуры
