...
Процедура ЗаполнитьСписокКонтрагентов()
	...
	Для каждого СтрокаКонтекста Из Платформа.ПараметрыКлиент.КонтекстДиадока Цикл
		...
		Если ОрганизацияОтправитель = СтрокаКонтекста.Организация Тогда
			...
			МассивКонтрагентов = МетодКлиента("Модуль_Клиент", "GetCounteragentListByStatus", СтрокаКонтекста.Организация, "IsMyCounteragent");
			...
			Для Индекс = 0 По МассивКонтрагентов.Count - 1 Цикл
				...
				ЭтоТестовыйКонтрагент = МетодКлиента("Модуль_Клиент", "ЭтоТестоваяОрганизация", ЭлементМассива);
				
				Если ЭтоТестовыйКонтрагент Тогда
					СтрокаКонтрагента.Контрагент = ЭлементМассива.Name + " (тестовый)";
				Иначе
					СтрокаКонтрагента.Контрагент = ЭлементМассива.Name;
				КонецЕсли;
