...
Процедура ПриОткрытии(Отказ)
	...
	Элементы.СписокОрганизацийКонтекстноеМенюСвернутьВсе.Картинка	= БиблиотекаКартинок.СвернутьВсе;
	Элементы.СписокОрганизацийКонтекстноеМенюРазвернутьВсе.Картинка	= БиблиотекаКартинок.РазвернутьВсе;
...
Процедура Добавить(Команда)
	...
	ЕстьРезультат = Ложь;
	ЕстьСтарыйФорматОтправкиИсходящихДокументов = Ложь;
	
	ДобавитьОрганизацииНаСервере(ЕстьСтарыйФорматОтправкиИсходящихДокументов);
	
	ПараметрыЗакрытия = Новый Структура("ЕстьСтарыйФорматОтправкиИсходящихДокументов", ЕстьСтарыйФорматОтправкиИсходящихДокументов);
	Закрыть(ПараметрыЗакрытия);

КонецПроцедуры

&НаСервере
Процедура ДобавитьОрганизацииНаСервере(ЕстьСтарыйФорматОтправкиИсходящихДокументов)
	...
	Для Каждого СтрокаОрганизации Из СписокОрганизаций.ПолучитьЭлементы() Цикл
		...
		Если ЗначениеЗаполнено(СтрокаОрганизации.ОрганизацияСтарая) Тогда
			...
			МетодСервера(,"УстановитьИдентификаторЭДОДляОрганизации", СтрокаОрганизации.ОрганизацияСтарая, "");
		КонецЕсли;
		...
		Для Каждого СтрокаПодразделения Из СтрокаОрганизации.ПолучитьЭлементы() Цикл
			...
			Если ЗначениеЗаполнено(СтрокаПодразделения.ОрганизацияСтарая) Тогда
				...
				МетодСервера(,"УстановитьИдентификаторЭДОДляОрганизации", СтрокаПодразделения.ОрганизацияСтарая, "");
			КонецЕсли;
			...
			Если ЗначениеЗаполнено(СтрокаПодразделения.Организация) Тогда
				...
				МетодСервера(,"УстановитьИдентификаторЭДОДляОрганизации", СтрокаПодразделения.Организация, СтрокаПодразделения.FnsParticipantId);
				ОпределитьЕстьСтарыйФорматОтправкиИсходящихДокументов(ЕстьСтарыйФорматОтправкиИсходящихДокументов, СтрокаПодразделения.Организация);
			КонецЕсли;
		...
		Если ЗначениеЗаполнено(СтрокаОрганизации.Организация) Тогда
			...
			МетодСервера(,"УстановитьИдентификаторЭДОДляОрганизации", СтрокаОрганизации.Организация, СтрокаОрганизации.FnsParticipantId);
			
			// Установим настройку формата отправки документов по умолчанию
			ФормироватьУПД = МетодСервера(, "ПолучитьЗначениеСвойства", СтрокаОрганизации.Организация, "ДиадокФормироватьУПД");
			Если Не ЗначениеЗаполнено(ФормироватьУПД) Тогда
				МетодСервера(, "УстановитьЗначениеСвойства", СтрокаОрганизации.Организация, "ДиадокФормироватьУПД",, "СЧФДОП_820");
			КонецЕсли;
			
			ОпределитьЕстьСтарыйФорматОтправкиИсходящихДокументов(ЕстьСтарыйФорматОтправкиИсходящихДокументов, СтрокаОрганизации.Организация);
		КонецЕсли;
...
&НаСервере
Процедура ОпределитьЕстьСтарыйФорматОтправкиИсходящихДокументов(ЕстьСтарыйФорматОтправкиИсходящихДокументов, Организация)
			
	Если Не ЕстьСтарыйФорматОтправкиИсходящихДокументов Тогда
		ФормироватьУПД = МетодСервера(, "ПолучитьЗначениеСвойства", Организация, "ДиадокФормироватьУПД");
		ЕстьСтарыйФорматОтправкиИсходящихДокументов = МетодСервера(, "ФорматДокументовНаОтправкуУстаревшийФормат", ФормироватьУПД);
	КонецЕсли;
...
Процедура СвернутьВсе(Команда)
	...
	КоллекцияЭлементовДерева = СписокОрганизаций.ПолучитьЭлементы();
	
	Для каждого ЭлементКоллекции из КоллекцияЭлементовДерева Цикл
		
		ИдентификаторСтроки = ЭлементКоллекции.ПолучитьИдентификатор();
		
		Элементы.СписокОрганизаций.Свернуть(ИдентификаторСтроки);
...
Процедура РазвернутьВсе(Команда)
	...
	Для Каждого ЭлементКоллекции Из КоллекцияЭлементовДерева Цикл 
		...
		Элементы.СписокОрганизаций.Развернуть(ИдентификаторСтроки);
