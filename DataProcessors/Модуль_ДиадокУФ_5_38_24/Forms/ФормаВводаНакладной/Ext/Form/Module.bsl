...
//{ СЛУЖЕБНЫЕ ПРОЦЕДУРЫ И ФУНКЦИИ
	...
	Процедура ЗаполнитьСтрокуДанными1С(СтрокаТЧ)
		...
		Иначе
			...
			КодНоменклатурыЭД = Item.Code;
			АртикулНоменклатурыЭД = Item.NomenclatureArticle;
			НаименованиеНоменклатурыЭД = Item.Name;
			
			СтрокаТЧ.Номенклатура = МетодСервера(, "ПолучитьНоменклатуруПоставщика", МетодСервера(,"СформироватьПрофильКонфигурации"), Контрагент, КодНоменклатурыЭД, АртикулНоменклатурыЭД, НаименованиеНоменклатурыЭД);
			
			Если НЕ ЗначениеЗаполнено(СтрокаТЧ.Номенклатура) И ЗначениеЗаполнено(АртикулНоменклатурыЭД) Тогда
				СтрокаТЧ.Номенклатура = МетодСервера(, "ПолучитьНоменклатуруПоАртикулу", МетодСервера(,"СформироватьПрофильКонфигурации"), АртикулНоменклатурыЭД);
			КонецЕсли;
			
			Если НЕ ЗначениеЗаполнено(СтрокаТЧ.Номенклатура) И ЗначениеЗаполнено(КодНоменклатурыЭД) Тогда
				СтрокаТЧ.Номенклатура = МетодСервера(, "ПолучитьНоменклатуруПоАртикулу", МетодСервера(,"СформироватьПрофильКонфигурации"), КодНоменклатурыЭД);
			КонецЕсли;
		...
		СтрокаТЧ.СтавкаНДС = МетодСервера(,"ОпределитьСтавкуНДС_1С_ПоСтавкеНДС_ДД", Item.TaxRate);
	...
	Процедура УстановитьСоответствиеНоменклатуры()
		...
		СтруктураОтбора = Новый Структура("Номенклатура, НоменклатураЭД, АртикулЭД, КодЭД");
		
		Для каждого СтрокаТЧ Из ТабличнаяЧасть Цикл
			
			СтруктураОтбора.Номенклатура	 = СтрокаТЧ.Номенклатура;
			СтруктураОтбора.НоменклатураЭД	 = СтрокаТЧ.НоменклатураИзЭД;
			СтруктураОтбора.АртикулЭД		 = СтрокаТЧ.АртикулЭД;
			СтруктураОтбора.КодЭД			 = СтрокаТЧ.КодЭД;
			
			НайденныеСтроки = ТаблицаСопоставления.НайтиСтроки(СтруктураОтбора);
	...
	Функция ТаблицаНоменклатурыДляСопоставления()
		...
		Результат = МетодСервера("Модуль_ИнтеграцияУниверсальный", "НоваяТаблицаНоменклатурыДляСопоставления");
		...
		Для Каждого СтрокаТЧ Из ТабличнаяЧасть Цикл
			...
			Наименование = СокрЛП(Item.Name);
			Артикул		 = СокрЛП(Item.NomenclatureArticle);
			Код			 = СокрЛП(Item.Code);
			
			Если Не ЗначениеЗаполнено(СтрокаТЧ.НоменклатураПоставщика) Тогда
				СтрокаТЧ.НоменклатураПоставщика = НоменклатураПоставщикаПоСвойствамНоменклатуры(Наименование, Артикул, Код);
			КонецЕсли;
			
			ДанныеНоменклатуры = ДанныеНоменклатурыПоставщика(СтрокаТЧ.НоменклатураПоставщика);
			
			Если ДанныеНоменклатуры.Номенклатура = СтрокаТЧ.Номенклатура Тогда
				//И ДанныеНоменклатуры.Характеристика = СтрокаТЧ.Характеристика // Отсутствует в ТЧ
				//И ДанныеНоменклатуры.Упаковка = СтрокаТЧ.ЕдиницаИзмерения Тогда
			...
			НоваяСтрока.НоменклатураЭД	 = Наименование;
			НоваяСтрока.АртикулЭД		 = Артикул;
			НоваяСтрока.КодЭД			 = Код;
	...
	&НаСервере
	Функция НоменклатураПоставщикаПоСвойствамНоменклатуры(Наименование, Артикул, Код)
		
		СвойстваНоменклатуры = Новый Структура;
		СвойстваНоменклатуры.Вставить("Наименование", Наименование);
		СвойстваНоменклатуры.Вставить("Артикул"		, Артикул);
		СвойстваНоменклатуры.Вставить("Код"			, Код);
		
		Результат = МетодСервера("Модуль_ИнтеграцияУниверсальный", "НоменклатураПоставщикаПоСвойствамНоменклатуры", Контрагент, СвойстваНоменклатуры);
	...
	&НаСервере
	Функция ДанныеНоменклатурыПоставщика(НоменклатураПоставщика)
		
		Результат = МетодСервера("Модуль_ИнтеграцияУниверсальный", "ДанныеНоменклатурыПоставщика", НоменклатураПоставщика);
	...
	Процедура ЗаполнитьШапкуДокумента_Универсальный(ДокументОбъект, Знач ДопДанныеССервера)
		...
		Если Объект.ПараметрыКлиентСервер.МаркерКонфигурации = "УТ11" Тогда
			...
			Если ЗначениеЗаполнено(Соглашение) Тогда
				Если МетодКлиента("Модуль_Клиент", "ПолучитьЗначениеРеквизитаОбъекта", Соглашение, "ИспользуютсяДоговорыКонтрагентов") = Истина Тогда
					ДокументОбъект.Договор = ДоговорКонтрагента;
					ФормаДокумента.Элементы.Договор.Доступность = Истина;
				Иначе
					ФормаДокумента.Элементы.Договор.Доступность = Ложь;
				КонецЕсли;
				СкладИзСоглашения = МетодКлиента("Модуль_Клиент", "ПолучитьЗначениеРеквизитаОбъекта", Соглашение, "Склад");
				Если ЗначениеЗаполнено(СкладИзСоглашения) Тогда
					ДокументОбъект.Склад = СкладИзСоглашения;
				КонецЕсли;
			КонецЕсли;
...
//{ УПРАВЛЕНИЕ ФОРМОЙ И ОБРАБОТКА СОБЫТИЙ
	...
	Функция ПоDocumentЕстьГТД(Document)
		...
		Если Document.Type = "UniversalTransferDocument" Тогда
			...
			СтруктураInvoice = МетодКлиента("Модуль_Клиент", "ПолучитьСтруктуруСодержанияДокумента", Document);
...
Процедура ЗаполнитьТипНоменклатуры(СтрокаТЧ, ТипОбъектаЭДО)
	...
	Иначе
		...
		Если ТипОбъектаЭДО = "XmlAcceptanceCertificate"
			...
			Или (ТипОбъектаЭДО = "UniversalTransferDocument"
