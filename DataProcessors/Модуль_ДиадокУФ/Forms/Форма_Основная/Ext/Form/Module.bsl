...
#Иначе
	...
	Функция ИдентификаторПараметраКонфигурационныйФайл()
		Возврат "diadoc-config";
	КонецФункции
	
	&НаКлиентеНаСервереБезКонтекста
	Функция КоллекцияПараметровЗапуска(Знач ПараметрыСтрокой)
		
		Результат = Новый Соответствие;
		
		СписокПараметров = СтрРазделить(ПараметрыСтрокой, ";");
		
		Для Каждого Подстрока Из СписокПараметров Цикл
			...
			Если ЭтоФлаг Тогда
				...
				Значение = Истина;
			Иначе
				Ключ = Лев(Подстрока, ПозицияРазделителя - 1);
				Значение = Сред(Подстрока, ПозицияРазделителя + 1);
			КонецЕсли;
	...
	&НаКлиенте
	Функция ПараметрыИзКонфигурационногоФайла(ПутьКФайлу)
		
		ЧтениеФайла = Новый ЧтениеТекста;
		МонопольныйРежим = Ложь;
		ЧтениеФайла.Открыть(ПутьКФайлу, "UTF-8",,, МонопольныйРежим);
	...
	&НаКлиенте
	Процедура ЗагрузитьПараметрыИзКонфигурационногоФайла(ПутьКФайлу)
		
		Если Не ЗначениеЗаполнено(ПутьКФайлу) Тогда
			Возврат;
		КонецЕсли;
		
		КонфигурационныйФайл = Новый Файл(ПутьКФайлу);
		
		Если Не КонфигурационныйФайл.Существует() Тогда
			Ошибка = СтрШаблон("Конфигурационный файл не найден (%1)!", ПутьКФайлу);
		ИначеЕсли Не КонфигурационныйФайл.ЭтоФайл() Тогда
			Ошибка = СтрШаблон("Неправильный путь к конфигурационному файлу (%1)", ПутьКФайлу);
		Иначе
			Ошибка = "";
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Ошибка) Тогда
			ЗарегистриватьОшибкуЧтенияКонфигурационногоФайла(Ошибка);
			Возврат;
		КонецЕсли;
		
		ВходящиеПараметры = ПараметрыИзКонфигурационногоФайла(ПутьКФайлу);
		УстановитьПараметрыОкружения(ВходящиеПараметры);
		
	КонецПроцедуры
	
	&НаКлиенте
	Процедура УстановитьПараметрыОкружения(НовыеЗначения)
		
		Если Не ЗначениеЗаполнено(НовыеЗначения) Тогда
			Возврат;
		КонецЕсли;
		
		ТекущиеЗначения = Новый Структура(Объект.ПараметрыКлиентСервер.ПараметрыОкружения);
		
		Для Каждого Элемент Из НовыеЗначения Цикл
			
			Если ТекущиеЗначения.Свойство(Элемент.Ключ) Тогда
				ТекущиеЗначения.Вставить(Элемент.Ключ, Элемент.Значение);
			КонецЕсли;
			
		КонецЦикла;
		
		Объект.ПараметрыКлиентСервер.Вставить("ПараметрыОкружения", Новый ФиксированнаяСтруктура(ТекущиеЗначения));
		
	КонецПроцедуры
	
	&НаКлиенте
	Процедура ЗарегистриватьОшибкуЧтенияКонфигурационногоФайла(Ошибка)
		
		СобытиеЖР = "Диадок.Конфигурационный файл";
		УровеньЖР = "Предупреждение";
		ДатаСобытия = "";
	...
	&НаКлиенте
	Процедура ОбработатьПараметрыЗапуска(Знач ПараметрыСтрокой)
		
		ПараметрыЗапуска = КоллекцияПараметровЗапуска(ПараметрыСтрокой);
		КонфигурационныйФайл = ПараметрыЗапуска.Получить(ИдентификаторПараметраКонфигурационныйФайл());
		ЗагрузитьПараметрыИзКонфигурационногоФайла(КонфигурационныйФайл);
	...
	Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
		...
		ТочностьЦеныПрочитатьНаСервере();
		...
		ТребуетсяПроверитьУведомления = Истина;
	...
	Процедура УстановитьУсловноеОформлениеСпискаДокументов(ТаблицаФормы)
		...
		УстановитьОформлениеСтатусаМЧД(ТаблицаФормы);
	...
	&НаСервере
	Процедура УстановитьОформлениеСтатусаМЧД(ТаблицаФормы)
		
		УровниКритичности = МетодСервера("", "УровниКритичностиСтатусаДокумента");
		
		ЭлементУсловногоОформления = УсловноеОформление.Элементы.Добавить();
		ЭлементОтбора = ЭлементУсловногоОформления.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных")); 
		ЭлементОтбора.ЛевоеЗначение		 = Новый ПолеКомпоновкиДанных(ТаблицаФормы.ПутьКДанным + ".СтатусМЧДКритичность"); 
		ЭлементОтбора.ВидСравнения		 = ВидСравненияКомпоновкиДанных.Равно; 
		...
		ОформляемоеПоле.Поле = Новый ПолеКомпоновкиДанных(ТаблицаФормы.Имя + "СтатусМЧД");
	...
	Процедура УстановитьУсловноеОформлениеФорматДокументовНаОтправку(ЭлементФормы, ИмяРеквизитаТаблицы)
		...
		НастройкиОтправки = МетодСервера(,"НастройкиОтправкиДокументов");
	...
	Процедура ЗаполнитьРеквизитыЗначениямиНастроек()
		...
		ОтключитьАвтоматическоеОбновлениеМодуля = ( МетодСервера( , "ЗначениеНастройкиОтключитьАвтоматическоеОбновлениеМодуля") = Истина );
	...
	Процедура ЗаполнитьСписокВыбораТипыДокументов(Элемент)
		...
		ФильтрыДокументовПоТипу = МетодКлиента("Модуль_Клиент", "ФильтрыДокументовПоТипу");
		
		ПустаяПиктограмма = МетодКлиента("Модуль_Клиент", "ЭДО_БиблиотекаКартинок").ПустаяПиктограмма;
		
		Элемент.СписокВыбора.Очистить();
		
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.Накладные);
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.Торг12,						ПустаяПиктограмма);
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.Акт,							ПустаяПиктограмма);
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.УПД_ДОП,						ПустаяПиктограмма);
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.УКД_ДИС,						ПустаяПиктограмма);
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.ИУПД_ДОП,						ПустаяПиктограмма);
		
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.СФ);
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.ОСФ,							ПустаяПиктограмма);
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.ИСФ,							ПустаяПиктограмма);
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.КСФ,							ПустаяПиктограмма);
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.УПД_СЧФ,						ПустаяПиктограмма);
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.УКД_КСЧФ,						ПустаяПиктограмма);
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.ИУПД_СЧФ,						ПустаяПиктограмма);
		
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.УПД_СЧФДОП);
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.УПД_СвРК);
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.УПД_СвЗК);
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.УКД_КСЧФДИС);
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.УКД_СвИСРК);
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.УКД_СвИСЗК);
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.ИУПД_СЧФДОП);
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.ИУПД_СвРК);
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.ИУПД_СвЗК);
		
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.Торг2);
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.СчетаНаОплату);
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.Неформализованные);
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.АктСверки,						ПустаяПиктограмма);
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.Детализация,					ПустаяПиктограмма);
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.Договор,						ПустаяПиктограмма);
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.ПротоколСогласованияЦены,		ПустаяПиктограмма);
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.ПриглашениеКЭДО,				ПустаяПиктограмма);
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.РеестрСертификатов,			ПустаяПиктограмма);
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.ДополнительноеСоглашение,		ПустаяПиктограмма);
		ДобавитьЭлементСпискаВыбораТипа(Элемент, ФильтрыДокументовПоТипу.Прочее,						ПустаяПиктограмма);
		
	КонецПроцедуры
	
	//Добавляет в список выбора Элемента тип документа по переданной настройке типа
	&НаКлиенте
	Процедура ДобавитьЭлементСпискаВыбораТипа(Элемент, НастройкаТипа, Отступ = Неопределено)
		
		Элемент.СписокВыбора.Добавить(НастройкаТипа.ЗначениеФильтра, НастройкаТипа.Представление, , Отступ);
	...
	Процедура ПодготовитьДанныеДляОбновленияМодуля()
		...
		ДанныеДляОбновленияМодуля = МетодСервера(, "ДанныеДляОбновленияМодуля");
	...
	Процедура ДополнитьДанныеДляОбновленияМодуля()
		...
		Если Не БылаВыполненаПопыткаАвтообновления Тогда
			МетодКлиента("Модуль_Клиент", "ДополнитьДанныеДляОбновленияМодуля", ДанныеДляОбновленияМодуля);
		КонецЕсли;
	...
	Процедура ОбновитьМодульАвтоматически(Отказ)
		...
		Если БылаВыполненаПопыткаАвтообновления Тогда
			Возврат;
		КонецЕсли;
		
		БылаВыполненаПопыткаАвтообновления = Истина;
		
		Если Не ВозможноОбновитьМодульАвтоматически() Тогда
			Если НужноЗарегистрироватьОшибкуАвтообновленияМодуля() Тогда 
				МетодКлиента("Модуль_Клиент", "Метрика_ЗарегистрироватьОшибкуАвтообновленияМодуля", ДанныеДляОбновленияМодуля);	
			КонецЕсли;
			Возврат;
		КонецЕсли;
		
		Попытка
			МетодКлиента("Модуль_Клиент", "ВыполнитьОбновлениеМодуля", ДанныеДляОбновленияМодуля);
		Исключение  
			
			ОписаниеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			МетодКлиента("Модуль_Клиент", "ЗарегистрироватьОшибкуАвтообновленияМодуляВЖурналеРегистрации", ОписаниеОшибки);
	...
	&НаКлиенте
	Функция ВозможноОбновитьМодульАвтоматически()
	...
	&НаКлиенте
	Функция НужноЗарегистрироватьОшибкуАвтообновленияМодуля()
	...
	&НаКлиенте
	Процедура ПерезапуститьМодуль()
		
		МестоположенияМодуля = МетодКлиента("Модуль_Клиент", "Перечисление_МестоположенияМодуля");
		
		ЭтаФорма.Закрыть();
		
		Попытка
			ПодключитьНовуюВерсиюМодуля(МестоположенияМодуля);
		Исключение
	...
	#Область ПодключениеНовойВерсииМодуля
	
	// Подключает новую версию модуля в метаданные сеанса
	//
	&НаКлиенте
	Процедура ПодключитьНовуюВерсиюМодуля(МестоположенияМодуля)
	...
	Процедура ПодключитьОбработкуМодуляИзСправочника(ВнешняяОбработка)
		...
		ДвоичныеДанные	 = ОбработкаОбъект().ФайлОбработкиИзСправочникаВнешнихОбработок(ВнешняяОбработка);
		АдресВХ			 = ПоместитьВоВременноеХранилище(ДвоичныеДанные);
	...
	Процедура ВнешниеОбработки_Подключить(АдресВХ)
		...
		ОбработкаОбъект().ВнешниеОбработки_Подключить(АдресВХ);	
		
	КонецПроцедуры
	
	#КонецОбласти	
	
	#Область УведомлениеПользователяОбАвтообновлении
	
	&НаКлиенте
	Процедура УведомитьПользователяОбАвтообновленииМодуля()
		
		МодульБылОбновлен = ДанныеДляОбновленияМодуля.МодульБылОбновлен;
		
		Если МодульБылОбновлен И Не УведомилиПользователяОбАвтообновлении Тогда
			МетодКлиента(, "ОткрытьФормуОбработкиМодально", "ФормаУведомленияОбУспешномАвтообновлении", , ЭтаФорма);
			УведомилиПользователяОбАвтообновлении = Истина;
		КонецЕсли;
	...
	&НаКлиенте
	Процедура ПродолжитьИнициализациюМодуляОтложенно()
		
		ИнтервалОжидания = 0.1;
		Однократно = Истина;
		
		ПодключитьОбработчикОжидания(
		"ПродолжитьИнициализациюМодуля",
		ИнтервалОжидания,
		Однократно
		);
	...
	&НаКлиенте
	Процедура ПродолжитьИнициализациюМодуля()
		...
		УведомитьПользователяОбАвтообновленииМодуля();
	...
	// Проверяет настройки отправки документов по всем организациям и контрагентам контекста
	// и, если хотя бы у одной организации или контрагента есть настройка, позволяющая отправлять
	...
	Процедура ПроверитьАктуальностиНастройкиФорматовОтправки() Экспорт
		...
		УстревшиеНастройки = МетодСервераБезКонтекста(,"ОрганизацииИКонтрагентыСУтаревшимиНастройкамиОтправки");
		
		ЕстьНастройкаОтправкиВУстаревшемФормате = УстревшиеНастройки.ЕстьУстаревшее;
		
		СохраненнаяВерсияМодуля = Объект.ПараметрыКлиентСервер.СохраненнаяВерсияМодуля;
		ТекущаяВерсияМодуля = Объект.ПараметрыКлиентСервер.ВерсияОбработки;
		
		Если ЕстьНастройкаОтправкиВУстаревшемФормате Тогда
			
			Если МетодКлиента("Модуль_Клиент", "СравнитьВерсии", ТекущаяВерсияМодуля, СохраненнаяВерсияМодуля) > 0 Тогда
				
				МетодСервераБезКонтекста(
				, "ВыполнитьУстановкуРекомендуемыхНастроекОтправки"
				, УстревшиеНастройки.ОрганизацииИБ
				, УстревшиеНастройки.КонтрагентыИБ);
				
				ПоказатьСообщениеОбАвтоматическомОбновленииНастроекОтправки(УстревшиеНастройки);
				
				Оповестить("ИзмененыНастройкиОтправки");
		...
		Если СохраненнаяВерсияМодуля <> ТекущаяВерсияМодуля Тогда
			МетодСервераБезКонтекста(, "СохранитьНастройкуСохраненнаяВерсияМодуля", ТекущаяВерсияМодуля);
			Объект.ПараметрыКлиентСервер.СохраненнаяВерсияМодуля = ТекущаяВерсияМодуля;
		КонецЕсли; 
	...
	Процедура ПроверитьНаличиеУведомлений()
		...
		Если Не ТребуетсяПроверитьУведомления Тогда
			
			ПродолжитьИнициализациюМодуляОтложенно();
			Возврат;
			
		КонецЕсли;
		
		ВерсияОбработки = МетодСервераБезКонтекста(, "ВерсияОбработки");
		СписокУведомлений = МетодКлиента("Модуль_Клиент", "УведомленияПользователей_СписокУведомлений", ВерсияОбработки);
		
		ТребуетсяПроверитьУведомления = Ложь;
		
		ОткрытьФормуУведомленияПользователя(СписокУведомлений);
		
	КонецПроцедуры
	
	&НаКлиенте
	Процедура ОткрытьФормуУведомленияПользователя(СписокУведомлений)
		
		Если Не ЗначениеЗаполнено(СписокУведомлений) Тогда
		...
		МетодКлиента(, "ОткрытьФормуОбработкиМодально", "ФормаУведомленияПользователя", ПараметрыФормы, ЭтаФорма,
		"ПослеЗакрытияФормыУведомленияПользователя");
	...
	#Область СлужебныеМетодыУведомлениеПользователя
	...
	Функция Метрика_НазваниеКатегории() Экспорт 
		...
		Результат = МетодКлиента("Модуль_Клиент", "Метрика_Категории");
	...
	Процедура Метрика_ДобавитьПоведение_НажатиеКнопки(НазваниеФормы, НазваниеКатегории, НазваниеДействия) Экспорт 
		...
		МетодКлиента("Модуль_Клиент", "Метрика_ДобавитьПоведение_НажатиеКнопки", НазваниеФормы, НазваниеКатегории, НазваниеДействия);
		
	КонецПроцедуры
	
	&НаКлиенте
	Процедура ОткрытьХостВIE(АдресРесурса) Экспорт 
	...
	&НаКлиенте
	Процедура ОткрытьОнлайнЧатПродолжение() Экспорт 
		
		ФормаТП = МетодКлиента(,"ПолучитьФормуОбработки", "ФормаТехподдержка",, ВладелецФормы);
		ФормаТП.Платформа = Платформа;
		
		ФормаТП.ОткрытьОнлайнЧатПродолжение();
		
	КонецПроцедуры
	
	&НаКлиенте
	Процедура ОтправитьEmailПродолжение() Экспорт 
		
		ФормаТП = МетодКлиента(,"ПолучитьФормуОбработки", "ФормаТехподдержка",, ВладелецФормы);
		ФормаТП.Платформа = Платформа;
		
		ФормаТП.ОтправитьEmailПродолжение();
	...
	Процедура СкачатьОбновление(Команда)
		...
		ДобавитьПоведение_НажалиКнопкуСкачатьОбновление();
		
		ТекущаяВерсияМодуля = Объект.ПараметрыКлиентСервер.ВерсияОбработкиБезНомераСборки;
		
		ДобавитьСтатистику_НажалиКнопкуСкачатьОбновление(ТекущаяВерсияМодуля, АктуальнаяВерсияМодуля);
	...
	&НаСервере
	Процедура ТочностьЦеныПрочитатьНаСервере()
		
		ТочностьЦеныСФ = Объект.ПараметрыКлиентСервер.ТочностьЦены;
	...
	Процедура ОбработчикОбновлениеСпискаДокументов()
		...
		Для Каждого СтруктураДокумента Из МассивДокументов Цикл
			...
			ЗаполнитьЗначенияСвойств(ТЧ.Добавить(), СтруктураДокумента);
	...
	Процедура ОбработчикОткрытиеФормыПараметрыОтправки(РезультатЗакрытия, ТекущиеДанные) Экспорт
		...
		Если РезультатЗакрытия <> Неопределено Тогда
			...
			Если МетодСервераБезКонтекста(,"ЭтоУстаревшаяНастройкаОтправки", РезультатЗакрытия.ФормироватьУПД) Тогда
				ПоказатьПредупреждениеОбУстаревшихНастройкахОтправки();
			КонецЕсли;
	...
	&НаКлиенте
	Процедура ОбработчикОткрытьФормуНастроекМЧД(BoxIDДляФормыНастроек, ДополнительныеПараметры = Неопределено) Экспорт
		
		МетодКлиента("Модуль_Клиент", "ОткрытьФормуСпискаДоверенностей", BoxIDДляФормыНастроек, ЭтотОбъект);
	...
	Процедура СохранитьЗначенияОпций(Команда)
		...
		Объект.ПараметрыКлиентСервер.ТочностьЦены = ТочностьЦеныСФ;
	...
	&НаСервере
	Процедура СохранитьЗначенияОпцийНаСервере(Точность)
		МетодСервера(, "ТочностьЦеныЗаписать", Точность);
	КонецПроцедуры
	...
	&НаКлиенте
	Процедура НастройкаМашиночитаемыхДоверенностей(Команда)
		...
		Иначе
			...
			МетодКлиента("Модуль_Клиент", "ОткрытьФормуСпискаДоверенностей", ТекущиеДанные.IdBox, ЭтотОбъект);
	...
	Процедура ОбработкаОповещения(ИмяСобытия, ПараметрыОповещения, Источник) Экспорт
		...
		ИначеЕсли ИмяСобытия = "ИзмененыНастройкиОтправки" Тогда
			...
			ЗаполнитьСписок(Неопределено);
			//pcru	
		ИначеЕсли ИмяСобытия = "ЗапросПрикрепленногоДокумента" Тогда
			ПараметрыОтбора = Новый Структура;
			ПараметрыОтбора.Вставить("ПервичныйДокумент", ПараметрыОповещения.Ссылка);
			НайденныеСтроки = ЭтаФорма.ТаблицаДокументовВходящих.НайтиСтроки(ПараметрыОтбора);
			Для каждого НайденнаяСтрока Из НайденныеСтроки Цикл
				...
				Document= Платформа.ПараметрыКлиент.КонтекстРаботаССерверомДиадок.DiadocConnection.GetOrganizationById(НайденнаяСтрока.ЭлектронныйДокумент.Идентификатор.ИдентификаторОрганизации).GetDocumentById( НайденнаяСтрока.DocumentId);
				PCRU_ДвоичныеДанныеДокумента = "";
				...
				КонецЕсли; 
				//+/asevryugin@PCRU.LOCAL, 2020-09-15 08:31:48
				PCRU_AddInnerFileНаСервере(PCRU_ДвоичныеДанныеДокумента,PCRU_ИмяФайла,PCRU_РасширениеФайла,ПараметрыОповещения.Ссылка);
	...
	&НаСервере
	Процедура PCRU_AddInnerFileНаСервере(PCRU_ДвоичныеДанныеДокумента,PCRU_ИмяФайла,PCRU_РасширениеФайла,Ссылка1c)
		//+asevryugin@PCRU.LOCAL, 2020-09-15 07:41:16
		
		ДвоичныеДанные = Base64Значение(PCRU_ДвоичныеДанныеДокумента);
		адресВХранилище = ПоместитьВоВременноеХранилище(ДвоичныеДанные);
		СтрПарам = РаботаСФайлами.ПараметрыДобавленияФайла();
		СтрПарам.ВладелецФайлов = Ссылка1c;
		СтрПарам.ИмяБезРасширения  = PCRU_ИмяФайла;
		СтрПарам.РасширениеБезТочки  = PCRU_РасширениеФайла;
	...
	&НаКлиенте
	Процедура ОтключитьАвтоматическоеОбновлениеМодуляПриИзменении(Элемент)
		
		МетодСервераБезКонтекста( , "СохранитьНастройкуОтключитьАвтоматическоеОбновлениеМодуля", ОтключитьАвтоматическоеОбновлениеМодуля);			
		
	КонецПроцедуры
	
	&НаКлиенте
	Процедура ОтключитьАвтоматическоеОбновлениеМодуляРасширеннаяПодсказкаОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
		
		АдресаРесурсов = МетодКлиента("Модуль_Клиент", "АдресаИнтернетРесурсов");
		
		НавигационнаяСсылкаФорматированнойСтроки = АдресаРесурсов.Инструкция_ОбновлениеМодуляВручную;	
	...
	Процедура ВыполнитьЛогикуПослеАвторизации()
		...
		Иначе
			...
			НачатьПроверкуМЧД();
			УстановитьВидимостьКнопкиДоверенности();
	...
	//
	&НаКлиенте
	Процедура ПоказатьСообщениеОбАвтоматическомОбновленииНастроекОтправки(УстревшиеНастройки)
		
		МетодКлиента("Модуль_Клиент", "ПоказатьСообщениеОбАвтоматическомОбновленииНастроекОтправки"
		, ЭтаФорма, , УстревшиеНастройки);
	...
	Процедура НадписьОкончанияОплаченногоПериодаНажатие(Элемент)  
		...
		МетодКлиента(, "ОткрытьФормуОбработкиМодально", "ФормаСчетовНаОплату");
		
	КонецПроцедуры
	
	&НаКлиенте
	Процедура ПоказатьПанельОкончанияОплаченногоПериода()
		ЕстьОнлайнСчета = ЕстьОнлайнСчетаНаОплату();
	...
	&НаКлиенте
	Функция ЕстьОнлайнСчетаНаОплату()
		...
		Для Каждого ЭлементКонтекста Из КонтекстДиадока Цикл
			...
			СтруктураОрганизации 				= Новый Структура("Наименование, BoxId");
			...
			AccountID 							= МетодКлиента("Модуль_Клиент", "ИдентификаторЛицевогоСчетаДиадок", СтруктураОрганизации.BoxId, Token);
			Результат 							= МетодКлиента("Модуль_Клиент", "ЕстьОнлайнСчетаНаОплату", AccountID, Token);
	...
	&НаКлиенте
	Процедура ДобавитьПоведение_НажалиКнопкуСкачатьОбновление()
		
		Категории = МетодКлиента("Модуль_Клиент", "Метрика_Категории");		
		
		НазваниеФормы 		= СинонимТекущейФормы();
		НазваниеКатегории 	= Категории.ОбновлениеМодуля;
		НазваниеДействия 	= "Ручное обновление";
	...
	&НаКлиенте
	Процедура ДобавитьСтатистику_НажалиКнопкуСкачатьОбновление(ТекущаяВерсия, НоваяВерсия)
		
		МетодКлиента(	"Модуль_Клиент", 
		"Метрика_ДобавитьСтатистику_НажалиКнопкуСкачатьОбновление",
	...
	Процедура ДобавитьСтатистику_Авторизация()
		...
		Переменные.Вставить("МестоХраненияПМ"				, ТехническаяИнформация.ИнформацияПМ.Размещение);
		Переменные.Вставить("DiadocUserID"					, ТехническаяИнформация.UserId);
		
		ТекущийРежимПроксиКомпоненты = МетодКлиента("Модуль_РаботаССерверомДиадок", "ПолучитьТекущийРежимПрокси");
		РежимПроксиМодуля = МетодКлиента("Модуль_Клиент", "РежиПроксиКомпонентыВРежимПроксиМодуля", ТекущийРежимПроксиКомпоненты);
		
		Переменные.Вставить("РежимПрокси", РежимПроксиМодуля.Представление);
		
		Если Платформа.Объект.ПараметрыКлиентСервер.ФайловыйРежимРаботы Тогда
			ВариантИБ = "Файловая";
		Иначе
			ВариантИБ = "Серверная";
		КонецЕсли;
		Переменные.Вставить("ВариантИБ", ВариантИБ);
		
		ОтключеноАвтоматическоеОбновлениеМодуля = МетодКлиента("Модуль_Клиент", "ОтключеноАвтоматическоеОбновлениеМодуля");
		
		Переменные.Вставить("ОтключеноАвтоматическоеОбновлениеМодуля", ОтключеноАвтоматическоеОбновлениеМодуля);
		Переменные.Вставить("BaseId", Объект.ПараметрыКлиентСервер.ИдентификаторБазы);
		
		ДополнитьПеременныеАвторизацииНаСервере(Переменные);
	...
	&НаСервере
	Процедура ДополнитьПеременныеАвторизацииНаСервере(Переменные)
		
		РежимСовместимостиПлатформы = Строка(Метаданные.РежимСовместимости);
		РежимСовместимостиПлатформы = СтрЗаменить(РежимСовместимостиПлатформы, "Версия", "");
		
		Переменные.Вставить("РежимСовместимостиПлатформ", РежимСовместимостиПлатформы);
		
		// реализован альтернативный вариант определения местонахождения модуля, т.к. при добавлении в версию
		// данных о номере сборки, не корректно работает механизм поиска в доп. обработках. 
		// Длина полной версии модуля превышает допустимую в типовой конфигурации.
		
		ОбработкаОбъект = ОбработкаОбъект();
		
		ЗначениеСвойства = "";
		ЕстьСвойство_ИспользуемоеИмяФайла = МетодСервера("", "ЕстьРеквизитИлиСвойствоОбъекта", ОбработкаОбъект, "ИспользуемоеИмяФайла", ЗначениеСвойства);
		
		ПолноеИмяОбработки 	= ОбработкаОбъект().Метаданные().ПолноеИмя();
		ЭтоВнешняяОбработка = (Найти(ПолноеИмяОбработки, "ВнешняяОбработка.") = 1);
		
		Если ЭтоВнешняяОбработка
			И ЕстьСвойство_ИспользуемоеИмяФайла
			И Найти(ЗначениеСвойства, "e1cib\") > 0 Тогда 
			
			РасположениеМодуля = "в справочнике";
			
		ИначеЕсли ЭтоВнешняяОбработка Тогда
			РасположениеМодуля = "на диске";
		ИначеЕсли Найти(ПолноеИмяОбработки, "Обработка.") = 1 Тогда
			РасположениеМодуля = "в конфигурации";
		Иначе
			РасположениеМодуля = "не удалось определить";
		КонецЕсли;
		
		Переменные.Вставить("РасположениеМодуля", РасположениеМодуля);
	...
	&НаКлиенте
	Процедура НачатьПроверкуМЧД()
		
		ПодключитьОбработчикОжидания("МЧД_ПроверитьНастройкиМЧДПоУмолчаниюДляСтрокКонтекста", 0.1, Истина);
		
	КонецПроцедуры
	
	&НаКлиенте
	Процедура МЧД_ПроверитьНастройкиМЧДПоУмолчаниюДляСтрокКонтекста() Экспорт
		
		ШаблонНетМЧД			 = "Отсутствует доверенность для сотрудника %1 по организации %2";
		ШаблонНетМЧДПоУмолчанию	 = "Не настроена доверенность по умолчанию для сотрудника %1 по организации %2";
		
		СписокОшибокМЧД			 = Новый Массив;
		BoxIDДляФормыНастроек	 = Неопределено;
		
		Для Каждого СтрокаОрганизации Из Платформа.ПараметрыКлиент.КонтекстДиадока Цикл
			
			Если СтрокаОрганизации.ТребуетсяМЧД Тогда
				
				КонтентСпискаМЧД = МетодКлиента("Модуль_Клиент", "МЧД_ДанныеПоИспользованиюДоверенностей", СтрокаОрганизации.BoxId);
				
				Если НЕ ЗначениеЗаполнено(КонтентСпискаМЧД.СписокМЧД) Тогда
					ОписаниеОшибки = ШаблонНетМЧД;
				ИначеЕсли НЕ ЗначениеЗаполнено(КонтентСпискаМЧД.МЧД) Тогда
					ОписаниеОшибки = ШаблонНетМЧДПоУмолчанию;
				Иначе
					Продолжить;
				КонецЕсли;
				
				ОписаниеОшибки = СтрШаблон(ОписаниеОшибки, СтрокаОрганизации.ДанныеСотрудника.ФИО, СтрокаОрганизации.Организация);
				
				СписокОшибокМЧД.Добавить(ОписаниеОшибки);
				
				Если BoxIDДляФормыНастроек = Неопределено Тогда
					BoxIDДляФормыНастроек = СтрокаОрганизации.BoxId;
				КонецЕсли;
		...
		Если ЗначениеЗаполнено(СписокОшибокМЧД) Тогда
			...
			МетодКлиента("Модуль_Клиент", "ОткрытьФормуУведомлениеНетМЧД"
			, ЭтаФорма
			, СписокОшибокМЧД
			, BoxIDДляФормыНастроек);
	...
	&НаКлиенте
	Процедура УстановитьВидимостьКнопкиДоверенности()
		
		Элементы.СписокПочтовыхЯщиковНастройкаМашиночитаемыхДоверенностей.Видимость = ЕстьЯщикДляДействийСДокументамиВКоторомНужнаМЧД();
