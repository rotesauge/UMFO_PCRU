...
Функция ПолучитьЗаполненныйPackageSendTask(Знач ПараметрыPackageSendTask, СоответствиеCustomIdСсылке1С) Экспорт
	...
	ПараметрыPackageSendTask.Свойство("КонтрактМЧД", КонтрактМЧД);
	МетодКлиента("Модуль_РаботаССерверомДиадок", "ЗаполнитьМЧДдляОтправки", PackageSendTask, КонтрактМЧД, Organization.Id);
...
Функция СформироватьТитулДокументаНаСервере(ФорматОтправки
	...
	ДополнительныеПараметры.ПараметрыФормированияФайла = ПараметрыФормированияФайла;
...
Процедура ЗаполнитьПодписантаВПротоКонтенте(ПротоКонтент, ФорматОтправки, ДанныеОрганизации, ДанныеПодписанта, ПолномочияПодписанта) Экспорт
	...
	Если ИспользуетсяРасширенныйПодписант(ФорматОтправки) Тогда
		...
		Если ЗначениеЗаполнено(ПолномочияПодписанта) Тогда
			...
			Signer.JobTitle			 = ПолномочияПодписанта.JobTitle;
		Иначе
			Возврат;
		КонецЕсли;
		
		Signer.OrganizationName	= ДанныеОрганизации.Name;
		
		Подписант_ЗаполнитьТипПодписантаИННДляКонтента(Signer, ДанныеОрганизации);
...
Процедура Подписант_ЗаполнитьТипПодписантаИННДляКонтента(Signer, ДанныеОрганизации) Экспорт
	
	ТипыСертификатовКЭП		 = Перечисление_SignerType();
	ДанныеСертификата		 = ДанныеОрганизации.Certificate;
	Если ЗначениеЗаполнено(ДанныеСертификата) Тогда
		ТипСертификата		 = ДанныеСертификата.OwnerType;
		Signer.Inn			 = ДанныеСертификата.Inn;
		Signer.OrganizationName	 = ДанныеСертификата.OrganizationName;
	Иначе
		Signer.Inn			 = ДанныеОрганизации.Inn;
	КонецЕсли;
	
	Если ТипСертификата = ТипыСертификатовКЭП.PhysicalPerson Тогда
		
		ИННИзОрганизации = ДанныеОрганизации.INN;
		ЭтоОрганизацияЮЛ = СтрДлина(ИННИзОрганизации) <> 12;
		
		Если ЭтоОрганизацияЮЛ Тогда
			SignerType	 = ТипыСертификатовКЭП.LegalEntity;
		Иначе
			SignerType	 = ТипыСертификатовКЭП.IndividualEntity;
		КонецЕсли;	
		
		Signer.Inn = ИННИзОрганизации;
		Signer.OrganizationName = ДанныеОрганизации.Name;
		
	ИначеЕсли ТипСертификата = ТипыСертификатовКЭП.IndividualEntity Тогда
		
		SignerType		 = ТипыСертификатовКЭП.IndividualEntity;
		
	ИначеЕсли ТипСертификата = ТипыСертификатовКЭП.LegalEntity Тогда
...
&НаСервере
Функция ТипПодписантаДляКонтента(ДанныеСертификата)
...
Функция Перечисление_SignerType()
