...
//{ ПЕРЕМЕННЫЕ МОДУЛЯ
	...
	Перем Итерация;
	&НаКлиенте
	Перем ДанныеКонтрагентовДиадока;
...
//{ УПРАВЛЕНИЕ ФОРМОЙ
	...
	Процедура ПриОткрытии(Отказ)
		...
		ДанныеКонтрагентовДиадока = Новый Соответствие;
	...
	Процедура ИнициироватьОбновлениеСписка()
		...
		ДанныеКонтрагентовДиадока.Очистить();
...
//{ СЛУЖЕБНЫЕ ПРОЦЕДУРЫ И ФУНКЦИИ
	...
	Функция УсловиеОграниченияВыборкиКонтрагентовСнизу(ПоследняяСсылка)
		
		Результат = "ИСТИНА";
		
		Если ЗначениеЗаполнено(ПоследняяСсылка) Тогда
			Результат = "Контрагенты.Ссылка > &ПоследняяСсылка";
		КонецЕсли;
	...
	&НаКлиентеНаСервереБезКонтекста
	Функция УсловиеОграниченияВыборкиКонтрагентовПоРодителю(СписокРодителей, МаркерКонфигурации)
		
		Если Не ЗначениеЗаполнено(СписокРодителей) Тогда
			
			Результат = "ИСТИНА";
			
		ИначеЕсли МаркерКонфигурации = "УТ11" Тогда
			
			Результат = "Контрагенты.Партнер В(&СписокРодителей)";
			
		Иначе
			
			Результат = "Контрагенты.Родитель В ИЕРАРХИИ (&СписокРодителей)";
	...
	Функция ПолучитьСписокКонтрагентовДляПроверки(ПоследняяСсылка, Знач Родитель, Знач МаркерКонфигурации)
		...
		Если МаркерКонфигурации = "УТ11" Тогда
			...
			|ГДЕ
			|	&УсловиеОграниченияСнизу
			|	И &УсловиеОграниченияПоРодителю
			|
			|УПОРЯДОЧИТЬ ПО
			|	Ссылка";
			
			СписокРодителей = ПолучитьСписокРодителей(Родитель);
		...
		УсловиеОграниченияСнизу = УсловиеОграниченияВыборкиКонтрагентовСнизу(ПоследняяСсылка);
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса
			, "&УсловиеОграниченияСнизу"
			, УсловиеОграниченияСнизу);
		
		УсловиеОграниченияПоРодителю = УсловиеОграниченияВыборкиКонтрагентовПоРодителю(СписокРодителей, МаркерКонфигурации);
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса
			, "&УсловиеОграниченияПоРодителю"
			, УсловиеОграниченияПоРодителю);
	...
	Процедура ОбработатьМассивИНН(СтрокаИНН, МассивКонтрагентовДляПроверки)
		...
		Для Каждого ЯчейкаМассива Из МассивКонтрагентовДляПроверки Цикл
			...
			Иначе
				НоваяСтрока.СтатусВзаимоотношений	= "InnNotValid";
				НоваяСтрока.СтатусРасшифровка		= РасшифровкаТекущегоСостоянияВзаимоотношений(НоваяСтрока.СтатусВзаимоотношений);
	...
	&НаКлиенте
	Процедура ОбработатьИтемы(СоответствиеИтемов, OrganizationInn)
		
		Для Каждого ТекущийИтем Из СоответствиеИтемов Цикл
			
			ОбработкаПрерыванияПользователя();
			
			СтруктураИтема = ТекущийИтем.Значение;
			...
			Для Каждого НайденнаяСтрока Из НайденныеСтрокиКонтрагентов Цикл
				...
				Иначе
					...
					Если СтруктураИтема.IsRoaming
						И Не СтрЗаканчиваетсяНа(НайденнаяСтрока.Наименование, Постфикс) Тогда
						
						НайденнаяСтрока.Наименование = НайденнаяСтрока.Наименование + " " + Постфикс;
	...
	Процедура ОбработатьCounteragentList()
		...
		Если НЕ РезультатЗапроса = Неопределено Тогда
			...
			Для Индекс = 0 По РезультатЗапроса.Count - 1 Цикл
				...
				ДанныеКонтрагентовДиадока.Вставить(CounteragentID, СтруктураИтема);
	...
	Процедура ВыполнитьОтключениеОбработчика()
		...
		ОтключитьОбработчикОжидания("ПодключаемыйОбработчикОбновитьСтатусРаботыСКА");
		ОбработатьИтемы(ДанныеКонтрагентовДиадока, Organization.Inn);
