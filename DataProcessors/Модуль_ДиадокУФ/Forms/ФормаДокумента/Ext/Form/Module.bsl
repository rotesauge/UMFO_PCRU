...
//{ ОБРАБОТЧИКИ ФОРМЫ
	...
	Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
		...
		Иначе
			...
			ЕстьОшибкаПроверкиМЧД = ТекущиеДанные.СтатусМЧДКритичность = МетодСервера("", "УровниКритичностиСтатусаДокумента").Error;
...
Функция ТочностьЦеныТекущееЗначение()
	...
	Результат = Объект.ПараметрыКлиентСервер.ТочностьЦены;
	Возврат Результат;
...
//{ ОБРАБОТКА СОБЫТИЙ
	...
	Процедура ОбработчикОткрытиеФормыВыбораПолучателя(ПараметрыОтправкиНаСогласование, НаборДанных) Экспорт
		...
		Если ЭтоНовыйДокумент Тогда
			...
			ВыполнитьОтправку(ПараметрыОтправкиНаСогласование);
...
//{ УПРАВЛЕНИЕ ФОРМОЙ
	...
	Функция СформироватьПредставлениеДокумента(СтрокаМассива)
		...
		Если ЗначениеЗаполнено(СтрокаМассива.ВнешняяПечатнаяФорма) Тогда
			Результат = СтрокаМассива.ТипДокумента;
		Иначе	
	...
	Функция ПоказатьГруппуКнопокПодписания(UserPermissions)
		...
		И (ЭДОбъект.RevocationStatus = "RevocationStatusNone" ИЛИ ЭДОбъект.RevocationStatus = "RevocationRejected")
	...
	Функция ПоказатьКнопкуПередачиНаПодпись(UserPermissions)
		...
		ВсеТочкиВызова 		= ТочкиВызоваУниверсальныхФорм();
		СтатусыСогласования = ВозможныеСтатусыЗапрошенногоСогласованияПодписания();
		
		Если ТочкаВызова = ВсеТочкиВызова.ТаблицаДокументовНаОтправку Тогда
			...
			ИначеЕсли Операция = "СоздатьНовыйПакет" ИЛИ Операция = "ОткрытьПакет" Тогда
				Если UserPermissions = Неопределено Тогда
					Результат = Ложь;
				Иначе
					Результат = UserPermissions.CanRequestResolutions;	// Пользователь имеет право запросить согласование или подписание документа
				КонецЕсли;
			Иначе
				Результат = Ложь; 			
			КонецЕсли;
			
		ИначеЕсли ТочкаВызова = ВсеТочкиВызова.ТаблицаДокументовИсходящих Тогда 
			
			Результат = Ложь;
			
		ИначеЕсли ТочкаВызова = ВсеТочкиВызова.ПереотправкаДокумента Тогда 
			
			Результат = UserPermissions.CanRequestResolutions;
								
		ИначеЕсли ТочкаВызова = ВсеТочкиВызова.ТаблицаДокументовВнутренних Тогда
			...
			Иначе
				...
				СтатусСогласованияПодходит = СтатусСогласования = Неопределено
											ИЛИ СтатусСогласования = СтатусыСогласования.None
											ИЛИ СтатусСогласования = СтатусыСогласования.Approved
											ИЛИ СтатусСогласования = СтатусыСогласования.Disapproved
											ИЛИ СтатусСогласования = СтатусыСогласования.SignatureDenied;
		...
		ИначеЕсли ТочкаВызова = ВсеТочкиВызова.ТаблицаДокументовВходящих Тогда 	
...
КонецФункции
	...
	Процедура НастроитьЭлементыФормы()
		...
		Иначе
			...
			Элементы.ГруппаСодержаниеДокумента.ОтображениеСтраниц	 = ОтображениеСтраницФормы.Нет;
			Элементы.ГруппаСодержаниеДокумента.ТекущаяСтраница		 = Элементы.ГруппаПечатнаяФорма;
...
КонецПроцедуры
	...
	Процедура ПередатьНаСогласование(Команда)
		...
		МассивПомеченныеСтроки = ПомеченныеСтрокиТаблицы(ТаблицаДокументов);
		
		Переменные = Новый Соответствие;
		Переменные.Вставить("КоличествоДокументовВсего"		, ТаблицаДокументов.Количество());
		Переменные.Вставить("КоличествоДокументовКОбработке", МассивПомеченныеСтроки.Количество());
		ДобавитьСтатистику_НажалиПередатьНаСогласование(Переменные);
		...
		Если МассивПомеченныеСтроки.Количество() = 0 Тогда
			...
			Возврат;
		ИначеЕсли ТочкаВызова = "ТаблицаДокументовНаОтправку" 
			И НЕ МассивПомеченныеСтроки.Количество() = ТаблицаДокументов.Количество() Тогда
	...
	Процедура ПередатьНаПодписание(Команда)
		...
		Переменные.Вставить("КоличествоДокументовВсего"		, ТаблицаДокументов.Количество());
		Переменные.Вставить("КоличествоДокументовКОбработке", МассивПомеченныеСтроки.Количество());
		ДобавитьСтатистику_НажалиПередатьНаПодписание(Переменные);
	...
	Процедура ЗапроситьУточнение(Команда)
		...
		ИмяОбработчика = "ОбработчикОткрытиеФормыОтветаЗапроситьУточнение";
		ПараметрыОбработчика = Новый Структура("Organization", Organization); 
		
		МетодКлиента(,"ОткрытьФормуОбработкиМодально", "ФормаОтветаПоДокументу", ПараметрыФормы, ЭтаФорма, ИмяОбработчика, ПараметрыОбработчика);
	...
	&НаКлиенте
	Функция ВозможныеСтатусыЗапрошенногоСогласованияПодписания()
		
		// https://1c-docs.diadoc.ru/ru/latest/ComObjects/Enums/ResolutionStatus.html
		
		Результат = Новый Структура;
		
		Результат.Вставить("None"				, "None"); 					// документ не согласовывался
		Результат.Вставить("Approved"			, "Approved"); 				// согласован
		Результат.Вставить("ApprovementRequested", "ApprovementRequested"); // запрошено согласование
		Результат.Вставить("Disapproved"		, "Disapproved"); 			// в согласовании отказано
		Результат.Вставить("SignatureRequested"	, "SignatureRequested"); 	// запрошена подпись
		Результат.Вставить("SignatureDenied"	, "SignatureDenied"); 		// в подписи отказано
		Результат.Вставить("ActionRequested"	, "ActionRequested"); 		// запрошено действие
	...
	Процедура ПодписатьИОтправить(Команда)
		...
		Переменные.Вставить("КоличествоДокументовВсего"		, ТаблицаДокументов.Количество());
		Переменные.Вставить("КоличествоДокументовКОбработке", МассивПомеченныеСтроки.Количество());
		ДобавитьСтатистику_НажалиПодписатьИОтправить(Переменные);
	...
	Процедура ВыполнитьОтправку(ПараметрыОтправкиНаСогласование = Неопределено)
		...
		ResolutionRequestType = МетодКлиента("Модуль_Клиент", "СвойствоСтруктуры", ПараметрыОтправкиНаСогласование, "ResolutionRequestType");
		
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("ПараметрыОтправкиНаСогласование"	, ПараметрыОтправкиНаСогласование);
		ДополнительныеПараметры.Вставить("ТребуетсяМЧД"						, ТребуетсяМЧД);
		
		ОписаниеОповещения = Новый ОписаниеОповещения("ВыполнитьОтправкуЗавершение", ЭтотОбъект, ДополнительныеПараметры);
		
		Если ТребуетсяМЧД 
			И ResolutionRequestType <> "ApprovementRequest" 
			И ResolutionRequestType <> "SignatureRequest" Тогда
		...
		Иначе
			...
			ВыполнитьОтправкуЗавершение(РезультатВыбораМЧД, ДополнительныеПараметры);		
	...
	&НаКлиенте
	Процедура ВыполнитьОтправкуЗавершение(Результат, ДополнительныеПараметры = Неопределено) Экспорт
		...
		ПараметрыОтправкиНаСогласование = ДополнительныеПараметры.ПараметрыОтправкиНаСогласование;
		
		ОтправкаПрошлаУспешно = Истина;
		
		Попытка 
			
			КонтрактМЧД	= МетодКлиента("Модуль_Клиент", "СвойствоСтруктуры", Результат, "КонтрактМЧД");
			
			ПараметрыPackageSendTask = ПараметрыPackageSendTask(ПараметрыОтправкиНаСогласование, КонтрактМЧД);
			...
			ТипЗапроса = МетодКлиента("Модуль_Клиент", "СвойствоСтруктуры", ПараметрыОтправкиНаСогласование, "ResolutionRequestType");
			Для Каждого ДокументПакета Из ПараметрыPackageSendTask.МассивДокументовПакета Цикл
				ДобавитьСтатистику_ВыполнитьОтправку(ДокументПакета, ТипЗапроса);
			КонецЦикла;
			
		Исключение
			
			ОтправкаПрошлаУспешно = Ложь;
		...
		ЭтоОтправкаНаСогласование = (ПараметрыОтправкиНаСогласование <> Неопределено);
		Если ЭтоОтправкаНаСогласование Тогда
			ОбработатьРезультаОтправкиДокументаНаСогласование(ОтправкаПрошлаУспешно);
		Иначе
			ОбработатьРезультаОтправкиДокумента(ОтправкаПрошлаУспешно);
		КонецЕсли;
		
	КонецПроцедуры
	
	&НаКлиенте
	Процедура ОбработатьРезультаОтправкиДокумента(ОтправкаПрошлаУспешно)
	...
	&НаКлиенте
	Процедура ОбработатьРезультаОтправкиДокументаНаСогласование(ОтправкаПрошлаУспешно)
	...
	&НаКлиенте
	Функция ПараметрыPackageSendTask(ПараметрыОтправкиНаСогласование, КонтрактМЧД = Неопределено)
	...
	Процедура ПодписатьИОтправить_ПодписаниеЗапрошенного(СтруктураSigner, Document) 
		...
		Иначе
			...
			РезультатВыбораМЧД = Новый Структура("РазрешитьДействие", Истина);
			ПодписатьИОтправить_ПодписаниеЗапрошенногоЗавершение(РезультатВыбораМЧД, ДополнительныеПараметры);
	...
	&НаКлиенте
	Процедура ПодписатьИОтправить_ПодписаниеЗапрошенногоЗавершение(РезультатЗакрытия, ДополнительныеПараметры = Неопределено) Экспорт 
		...
		СтруктураSigner	= МетодКлиента("Модуль_Клиент", "СвойствоСтруктуры", ДополнительныеПараметры, "СтруктураSigner");
		Document		= МетодКлиента("Модуль_Клиент", "СвойствоСтруктуры", ДополнительныеПараметры, "Document");
		КонтрактМЧД		= МетодКлиента("Модуль_Клиент", "СвойствоСтруктуры", РезультатЗакрытия, "КонтрактМЧД");
		
		СтруктураРезультатОперации= МетодКлиента("Модуль_РаботаССерверомДиадок", "ПодписатьИОтправить_ПодписаниеЗапрошенного", СтруктураSigner, Document, КонтрактМЧД);
...
////////////////////////////////////////////////////////////////////////////////
	...
	Процедура СформироватьФайлДокумента(ДополнительныеПараметры)
		...
		ПараметрыФормированияФайла.ТочностьЦены = ТочностьЦеныТекущееЗначение();
...
&НаКлиенте
Функция ТочкиВызоваУниверсальныхФорм()
	
	Результат = МетодКлиента("Модуль_Клиент", "ТочкиВызоваУниверсальныхФорм");
...
&НаКлиенте
Функция Метрики_ПредставлениеBoxId(ИдентификаторЯщика)
	
	Результат = МетодКлиента("Модуль_Клиент", "Метрики_ПредставлениеBoxId", ИдентификаторЯщика);
...
Функция Метрики_ДействияНаФорме()
	...
	Результат.Вставить("ОтменитьСопоставление"		, "Отменить сопоставление с документом 1С");
...
Процедура ДобавитьСтатистику_НажалиОтменитьСопоставление()
	...
	НазваниеДействия 	= Метрики_ДействияНаФорме().ОтменитьСопоставление;
...
&НаКлиенте
Процедура ДобавитьСтатистику_ВыполнитьОтправку(ДокументПакета, ТипЗапроса)
	...
	ПараметрBoxId 		= ПолучитьBoxIdПоОрганизации1С(ДокументПакета.Организация);
	ПредставлениеBoxId 	= Метрики_ПредставлениеBoxId(ПараметрBoxId);
		
	DocumentId = ДокументПакета.DocumentId;
	Если Не ЗначениеЗаполнено(DocumentId) Тогда
		DocumentId = МетодСервера(, "Документ_2_DocumentID", ДокументПакета.Документ);
	КонецЕсли;
	...
	Переменные.Вставить("ТипКонтента"		, ДокументПакета.ФорматОтправки.ТипКонтента);
	Переменные.Вставить("ФункцияДокумента"	, ДокументПакета.ФорматОтправки.ФункцияДокумента);
	Переменные.Вставить("БазовыйФормат"		, ДокументПакета.ФорматОтправки.БазовыйФормат);
	Переменные.Вставить("ИнфоПоля"			, ЗначениеЗаполнено(ДокументПакета.ДопПоле));
...
&НаСервере
Процедура PCRU_AddInnerFileНаСервере(PCRU_ДвоичныеДанныеДокумента,PCRU_ИмяФайла,PCRU_РасширениеФайла)
	//+asevryugin@PCRU.LOCAL, 2020-09-15 07:41:16
		...
		СтрПарам.ВладелецФайлов = ЭтаФорма.Документ1С;
...
&НаКлиенте
Процедура PCRU_AddInnerFile(Команда)
	//+asevryugin@PCRU.LOCAL, 2020-09-15 08:31:48
	Document= Платформа.ПараметрыКлиент.КонтекстРаботаССерверомДиадок.DiadocConnection.GetOrganizationById(Organization.Id).GetDocumentById(DocumentID);
	...
	Если Document.type <> "Nonformalized" и Document.type <> "NonformalizedProforma"  Тогда
		...
		КонецЕсли; 
	//+/asevryugin@PCRU.LOCAL, 2020-09-15 08:31:48
	PCRU_AddInnerFileНаСервере(PCRU_ДвоичныеДанныеДокумента,PCRU_ИмяФайла,PCRU_РасширениеФайла);
КонецПроцедуры
