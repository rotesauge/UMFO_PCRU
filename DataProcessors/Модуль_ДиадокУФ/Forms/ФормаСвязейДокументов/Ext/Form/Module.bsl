...
//{ ТЕЛО ФОРМЫ
	...
	Процедура ЗаполнитьПараметрыДокументаВДереве(СтрокаДерева, Document)
		...
		СтрокаДерева.Номер = МетодКлиента("Модуль_Клиент","ПредставлениеНомераЭД", Document);
	...
	Процедура УдалитьДокументыСтрок(МассивДокументов, СтрокиДерева)
		...
		Для Каждого СтрокаДерева из СтрокиДерева Цикл
			Если СтрокаДерева.Вкл Тогда	
				...
				Document = ЭлектронныйДокументПоИдентификатору(СтрокаДерева.BoxID, СтрокаДерева.DocumentID);
				ФорматЭД = ФорматЭлектронногоДокумента(Document);
				ИмяТипа	 = ФорматЭД.ИмяТипа;
