...
Функция СоздатьДокументУчета(ДанныеЭД, ИдентификаторПравилаОбработки)
	...
	//++ Севрюгин А.А  01.11.2022 8:42:43   
	Если НовыйОбъект.Контрагент.НаименованиеПолное = "ООО ""Интернет Решения""" Тогда
	НовыйОбъект.ВидОперации  = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.Товары;
	Запрос = Новый Запрос;
		...
		|ИЗ
		|	Документ.ПоступлениеТоваровУслуг.Товары КАК ПоступлениеТоваровУслугУслуги
		|ГДЕ
		...
		|	ПоступлениеТоваровУслугУслуги.СчетУчета КАК СчетУчета,
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПоступлениеТоваровУслуг.Товары КАК ПоступлениеТоваровУслугУслуги
		...
		Запрос.УстановитьПараметр("ВидОперации", Перечисления.ВидыОперацийПоступлениеТоваровУслуг.Товары);
		Запрос.УстановитьПараметр("Контрагент",НовыйОбъект .Контрагент  );
		...
		ЕстьВыборка = ВыборкаПараметровСтроки.Следующий();
			Для каждого СтрокаУслуги Из НовыйОбъект.Услуги Цикл
			НоваяСтрокаТовары = НовыйОбъект.Товары.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрокаТовары,СтрокаУслуги);
			Если ЕстьВыборка Тогда
						ЗаполнитьЗначенияСвойств(НоваяСтрокаТовары,ВыборкаПараметровСтроки);
					КонецЕсли;   
				Если ЗначениеЗаполнено(СтрокаУслуги.Содержание)  Тогда
							...
							Запрос.УстановитьПараметр("НаименованиеПолное",СтрокаУслуги.Содержание );
							...
							Если не ЗначениеЗаполнено(Номенклатура)  Тогда
								...
								НоменклатураОб.ВидНоменклатуры            = Справочники.ВидыНоменклатуры.НайтиПоНаименованию("Товар");
								НоменклатураОб.Наименование               = СтрокаУслуги.Содержание;
								НоменклатураОб.НаименованиеПолное         = СтрокаУслуги.Содержание;
								НоменклатураОб.БНФОГруппаФинансовогоУчета = Справочники.БНФОГруппыФинансовогоУчетаАктивов.НайтиПоНаименованию("Товары");
		...
		НовыйОбъект.ПодразделениеОрганизации =  pcru_УМФО.ФинОтдел(); 
	     НовыйОбъект.Услуги.Очистить();
	
	КонецЕсли;
		//-- Севрюгин А.А  01.11.2022 8:42:45 Администратор
