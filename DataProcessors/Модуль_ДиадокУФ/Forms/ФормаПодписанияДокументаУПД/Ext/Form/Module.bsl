...
Процедура ВыполнитьДействиеПослеПроверкиМЧД(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Не РазрешитьДействие
		Или ЕстьОшибкиЗаполнения() Тогда
...
Функция ОтветныйТитул()
	...
	МетодКлиента("Модуль_Выгрузка", "Подписант_ЗаполнитьТипПодписантаИННДляКонтента", ДанныеПодписанта, ДанныеОрганизации);
	
	Если НЕ ЗначениеЗаполнено(ДанныеПодписанта.JobTitle) Тогда
		
		ДанныеСотрудника = МетодКлиента("Модуль_РаботаССерверомДиадок", "ДанныеКонтекстаДиадок", BoxId, "ДанныеСотрудника");
		ДанныеПодписанта.JobTitle = ДанныеСотрудника.Должность;
...
Функция TovTorgBuyerContent()
	...
	Результат = МетодКлиента("Модуль_Клиент", "Новый_TovTorgBuyerContent");
	...
	ЗаполнитьAdditionalInfos(Результат.AdditionalInfo.StructedAdditionalInfos, Истина);
...
Функция DocumentCreator() 
	...
	Иначе
		Результат = ДанныеОрганизации.Name;
	КонецЕсли;
...
Процедура ЗаполнитьAdditionalInfos(КоллекцияДопСведений, StructedAdditionalInfos = Ложь)
	...
	Для НомерСведения = 1 По КоличествоДопСведений Цикл
		...
		Если StructedAdditionalInfos = Истина Тогда 
			ДопСведение.Вставить("Key", КлючСведения);
		Иначе 	
