...
Процедура ДокументОснованиеНажатие(Элемент)
	...
	Иначе
		СоздатьСопоставитьДокументОснование(Элементы.ДокументОснование.Заголовок = "Создать...");
	КонецЕсли;
...
Процедура СчетФактураНажатие(Элемент)
	...
	ИначеЕсли ЗначениеЗаполнено(СчетФактураДляСопоставления) Тогда
...
Процедура ОбработкаКомандыЗавершение(РезультатЗапроса, ДополнительныеПараметры) Экспорт
	...
	ЕстьОшибки = Ложь;
	СсылкаНаОбъект = ДополнительныеПараметры.ДокументСсылка;
	
	Если РезультатЗапроса = КодВозвратаДиалога.Да Тогда
	
		РезультатОбновления = МетодСервераБезКонтекста( ,	"ОбновитьДанныеДокумента",
															СсылкаНаОбъект,
															ДополнительныеПараметры.DocumentDate,
															ДополнительныеПараметры.DocumentNumber	);
															
		Если РезультатОбновления.ДанныеОбновлены = Истина Тогда
			ОтобразитьИзменениеДанных(СсылкаНаОбъект, ВидИзмененияДанных.Изменение);
		КонецЕсли;
...
&НаКлиенте
Процедура СоздатьСопоставитьДокументОснование(СоздатьДокументОснование)
...
Процедура ОбработчикОткрытиеФормыВводаНакладной(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если РезультатЗакрытия <> Неопределено Тогда
		...
		ИначеЕсли РезультатЗакрытия.Свойство("Режим") Тогда
			...
			Если РезультатЗакрытия.Режим = "Сопоставление" Тогда
...
Процедура ОбработчикОткрытиеФормыВыбораДокумента(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
	...
	Если НЕ РезультатЗакрытия = Неопределено Тогда
		...
		Если ЗначениеЗаполнено(РезультатЗакрытия.Документ1С) Тогда
			...
			РезультатОбновления = МетодСервераБезКонтекста(,"ОбновитьДанныеДокумента", ДокументОснование, ЭДОбъект.DocumentDate, ЭДОбъект.DocumentNumber);
			Если РезультатОбновления.ДанныеОбновлены = Истина Тогда
		...
		ИначеЕсли РезультатЗакрытия.Режим = "ВводУПД" Тогда 
			
			СоздатьСопоставитьДокументОснование(Истина);
...
Процедура ОбработчикЗакрытиеФормыСФ(РезультатЗакрытия, ДокументОбъект) Экспорт
	...
	Если ЗначениеЗаполнено(ДокументОбъект.Ссылка) Тогда
		...
		ПодключитьОбработчикОжидания("ОбновитьСопоставлениеСФ", 0.1, Истина);
	КонецЕсли;
...
&НаКлиенте
Процедура ОбновитьСопоставлениеСФ() Экспорт
