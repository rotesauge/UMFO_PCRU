...
//{ ТЕЛО МОДУЛЯ
	...
	&НаКлиенте
	Функция SendNonformalizedDocument(	Organization, 
										ПутьКФайлу, 
										ИмяФайла, 
										Комментарий, 
										CounteragentId, 
										...
										ТребуетсяПодпись, 
										InitialDocumentId, 
										SubordinateDocumentId = "",
										ToDepartmentId = "", 
										DelaySend, 
										IsTest	)
										
		БинарныеДанные = Новый ДвоичныеДанные(ПутьКФайлу);
		ФорматОтправки = МетодКлиента("Модуль_Выгрузка", "ОписаниеФорматаДляОтправкиФайла", "Nonformalized");
		
		PackageSendTask = Organization.CreatePackageSendTask2();
		
		PackageSendTask.CounteragentId = CounteragentId;
		PackageSendTask.ToDepartmentId = ToDepartmentId;
		PackageSendTask.DelaySend = DelaySend;
						
		СтрокаBase64 = Base64Строка(БинарныеДанные);
		...
		CustomDocumentToSend = PackageSendTask.AddDocumentFromBase64(ИмяТипа
							, ФункцияДокумента
							, ВерсияФормата
							, СтрокаBase64);
							
		CustomDocumentToSend.NeedRecipientSignature = ТребуетсяПодпись;
				
		Если ЗначениеЗаполнено(Комментарий) Тогда
			CustomDocumentToSend.Comment = Комментарий;
		КонецЕсли;
		
		Мета = Новый Структура;
		
		Мета.Вставить("FileName"		, ИмяФайла);
		Мета.Вставить("DocumentDate"	, ДатаДокумента);
		Мета.Вставить("DocumentNumber"	, НомерДокумента);
		...
		Если ЗначениеЗаполнено(InitialDocumentId) тогда 
			CustomDocumentToSend.AddInitialDocument(InitialDocumentId);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(SubordinateDocumentId) тогда 
			CustomDocumentToSend.AddSubordinateDocument(SubordinateDocumentId);
		КонецЕсли;
					
		DocumentsPackage = PackageSendTask.Send();
		
		Document = DocumentsPackage.Documents.GetItem(0);
		
		Возврат Document.DocumentId;
	...
	Функция ОтправитьНеформализованныеДокументы(CounteragentId, ПодразделениеКонтрагента = Неопределено, МассивФайлов, ОтправкаДокументаНаСогласование = Ложь) Экспорт
		...
		Если OrganizationId = "" Тогда
			...
			ШаблонСообщения = НСтр("ru = 'Организация %1 не сопоставлена с организацией в %2'");
			ТекстСообщения =  СтрШаблон(ШаблонСообщения,
										ОрганизацияОтправитель,
										Платформа.ПараметрыКлиент.СловарьWL.КраткоеНаименованиеСистемыПредложныйПадеж);
		...
		Попытка
			Файл_Путь			= МассивФайлов[0].ПутьКФайлу;
			Файл_Имя			= МассивФайлов[0].ИмяФайла;
			Файл_Комментарий 	= МассивФайлов[0].Комментарий;
			Документ_Номер		= " ";
			Документ_Дата 		= Строка(ТекущаяДата());
			ТребуетсяПодпись 	= МассивФайлов[0].ЗапрашиватьОтветнуюПодпись;
			InitialDocumentID = SendNonformalizedDocument(Organization, Файл_Путь, Файл_Имя, Файл_Комментарий, CounteragentId, Документ_Дата, Документ_Номер, ТребуетсяПодпись, InitialDocumentID, , , , isTest);
		...
		Если МассивФайлов.Количество() > 1 Тогда
			...
			Для К = 1 по МассивФайлов.Количество()-1 Цикл
				...
				Попытка
					...
					Файл_Путь 			= МассивФайлов[К].ПутьКФайлу;
					Файл_Имя 			= МассивФайлов[К].ИмяФайла;
					Файл_Комментарий 	= МассивФайлов[К].Комментарий;
					Документ_Дата 		= Строка(ТекущаяДата());
					Документ_Номер 		= " ";
					ТребуетсяПодпись 	= МассивФайлов[К].ЗапрашиватьОтветнуюПодпись;
					
					Если ПодразделениеКонтрагента = Неопределено Тогда
						ID = SendNonformalizedDocument(Organization, Файл_Путь, Файл_Имя, Файл_Комментарий, CounteragentId, Документ_Дата, Документ_Номер, ТребуетсяПодпись, InitialDocumentID, , , ОтправкаДокументаНаСогласование, isTest);
					Иначе
						ID = SendNonformalizedDocument(Organization, Файл_Путь, Файл_Имя, Файл_Комментарий, CounteragentId, Документ_Дата, Документ_Номер, ТребуетсяПодпись, InitialDocumentID, , ПодразделениеКонтрагента, ОтправкаДокументаНаСогласование, isTest);
					КонецЕсли;
	...
	&НаКлиенте
	Процедура НачатьОбновлениеСпискаИсходящихДокументов()
	...
	&НаКлиенте
	Процедура ПоказатьПредупреждениеОбУстаревшихНастройкахОтправки() Экспорт
		
		Если Не ЕстьНастройкаОтправкиВУстаревшемФормате Тогда
			Возврат;
		КонецЕсли;
	...
	Функция ПолучитьСписокАктивныхОрганизаций()
		...
		Организации = МетодКлиента("Модуль_Клиент", "СопоставленныеОрганизации");
		
		Результат = Новый СписокЗначений;
		Результат.ЗагрузитьЗначения(Организации);
	...
	&НаСервере
	Процедура СформироватьТаблицуЗначенийДокументовДляОтправкиДиадок(ДатаНачала, ДатаОкончания, ТаблицаОтношенийОрганизацийКонтрагентов)
		
		НачалоПериода = ДатаНачала;
		КонецПериода = ?(ЗначениеЗаполнено(ДатаОкончания), КонецДня(ДатаОкончания), КонецДня(ТекущаяДата()));
		...
		МетодСервера("Модуль_РаботаСВнешнимиПечатнымиФормами"
			, "ЗаполнитьТаблицуДокументовНаОтправкуПараметрамиОтправкиВнешнихПечатныхФорм"
			, ТаблицыДляВыгрузкиВДиадок.ТаблицаВыгрузки);
		
		ИтоговаяТаблица = МетодСервера("Модуль_РаботаСВнешнимиПечатнымиФормами", "ДополнитьТаблицуВнешнимиПФ"
			, ТаблицаОтношенийОрганизацийКонтрагентов
			, ТаблицыДляВыгрузкиВДиадок);
		
		СписокДокументов.Загрузить(ИтоговаяТаблица);
	...
	//{ ВЫГРУЗКА ДОКУМЕНТОВ ОДНИМ СООБЩЕНИЕМ
		...
		Процедура ЗапуститьВыгрузку()
			...
			Если ПроверитьВозможностьЗапускаВыгрузки() Тогда
				...
				МетодКлиента("Модуль_Клиент", "МЧД_ИнициализироватьКэшКонтекстов");
				
				СнятьПометкиСоСкрытыхСтрок();
							
				ПараметрыПоиска = Новый Структура;
				
				ПараметрыПоиска.Вставить("Вкл"			, Истина);
				ПараметрыПоиска.Вставить("ДокВыгружен"	, Ложь);
				
				ОбщийМассив					= СписокДокументов.НайтиСтроки(ПараметрыПоиска);
				...
				СоответствиеFnsParticipantIdПолучателя = Неопределено;	// на случай, если пользователь пересопоставил контрагента
																		// без закрытия формы выгрузки
				ДополнительныеПараметры = Новый Структура;
				ДополнительныеПараметры.Вставить("ДанныеДляОбработки", ОбщийМассив);
					
				ОписаниеОповещения = Новый ОписаниеОповещения("ЗапуститьВыгрузкуЗавершение", ЭтотОбъект, ДополнительныеПараметры);
													
				Если МетодКлиента("Модуль_Клиент", "МЧД_ДляВыполненияДействияТребуетсяМЧДпоУмолчанию", ResolutionRequestType) Тогда
					МетодКлиента("Модуль_Клиент", "МЧД_ПроверитьДоверенностьПоУмолчаниюИВыполнитьОповещение", ОписаниеОповещения);
				Иначе
		...
		&НаКлиенте
		Процедура ЗапуститьВыгрузкуЗавершение(Результат, ДополнительныеПараметры = Неопределено) Экспорт
		...
		Процедура ВыполнитьВыгрузкуДокумента() Экспорт
			...
			ПараметрыПоиска_ЕщеНеОтправдены = Новый Структура;
			ПараметрыПоиска_ЕщеНеОтправдены.Вставить("Вкл", Истина);
			ПараметрыПоиска_ЕщеНеОтправдены.Вставить("ДокВыгружен", Ложь);
			ПараметрыПоиска_ЕщеНеОтправдены.Вставить("ПропускатьПриВыгрузке", Ложь);
			
			ОставшиесяКВыгрузке = СписокДокументов.НайтиСтроки(ПараметрыПоиска_ЕщеНеОтправдены);
			Если ОставшиесяКВыгрузке.Количество() > 0 Тогда
				...
				ПараметрыПоиска_ДокументыПакета = Новый Структура;
				ПараметрыПоиска_ДокументыПакета.Вставить("Вкл", Истина);
				ПараметрыПоиска_ДокументыПакета.Вставить("ДокВыгружен", Ложь);
				ПараметрыПоиска_ДокументыПакета.Вставить("ВременныйИдентификаторПакета", ПерваяСтрокаКВыгрузке.ВременныйИдентификаторПакета);
				
				МассивСтрокПакета		= СписокДокументов.НайтиСтроки(ПараметрыПоиска_ДокументыПакета);
				НомерДокумента			= НомерДокумента + 1;
		...
		Процедура ВыполнитьОтправку(НаборДанных, ПараметрыОтправкиНаСогласование = Неопределено)
			...
			Если ПустаяСтрока(IdОтправленого) Тогда 
				...
				Попытка
					...
					КонтрактМЧД = ПервыйДокументВНаборе.КонтрактМЧД;
					ПараметрыPackageSendTask.Вставить("КонтрактМЧД", КонтрактМЧД);
					...
					ПараметрыЗаданияНаОтправку = Новый Структура;
					ПараметрыЗаданияНаОтправку.Вставить("AsyncTask"							, AsyncTask);
					ПараметрыЗаданияНаОтправку.Вставить("OrganizationId"					, ЗаполненныйPackageSendTask.Organization.id);
					ПараметрыЗаданияНаОтправку.Вставить("МассивСтрок"						, НаборДанных);
					ПараметрыЗаданияНаОтправку.Вставить("ПараметрыОтправкиНаСогласование"	, ПараметрыОтправкиНаСогласование);
					
					МассивЗаданийОтправкиПакета.Добавить(ПараметрыЗаданияНаОтправку);
	...
	//{ ОБРАБОТКА РЕЗУЛЬТАТОВ ВЫГРУЗКИ ДОКУМЕНТОВ
		...
		Процедура ОбработатьРезультатОтправкиПакетаДокументов(ЗаданиеНаОтправку)
			...
			Попытка
				...
				Для Каждого СтрокаСпискаВыгрузки Из МассивСтрокСпискаВыгрузки Цикл
					...
					ТипЗапроса = МетодКлиента("Модуль_Клиент", "СвойствоСтруктуры", ЗаданиеНаОтправку.ПараметрыОтправкиНаСогласование, "ResolutionRequestType");
					ДобавитьСтатистику_ВыполнитьОтправку(СтрокаСпискаВыгрузки, ТипЗапроса);
						...
						Для каждого СтрокаДокПакета из СтрокиДокиПакета Цикл
							...
							ИначеЕсли ЗначениеЗаполнено(СтрокаДокПакета.ВнешняяПечатнаяФорма) Тогда
								...
								Для каждого СтрокаВозможныйДокОснование из МассивСтрокВозможныеДокументыОснования Цикл
									...
									Если ТипОснованияПодходит(СтрокаВозможныйДокОснование.ФорматОтправки) И СтрокаВозможныйДокОснование.ДокВыгружен Тогда
		...
		&НаКлиенте
		Функция ТипОснованияПодходит(ФорматОтправки)
			
			Результат = ФорматОтправки.ЕстьФункцияПД И Не ФорматОтправки.ЭтоКорректировка;
...
//{ СЛУЖЕБНЫЕ ПРОЦЕДУРЫ И ФУНКЦИИ
	...
	Процедура ВыбратьФайлыНаДиске(Операция)
		...
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("Организация"	, ЗначениеПоУмолчанию_Организация());
		ДополнительныеПараметры.Вставить("Контрагент"	, Контрагент);
		
		Если Операция = "СоздатьНовыйПакет" Тогда
			
			ОбработкаВыбораФайла = Новый ОписаниеОповещения("ПослеВыбораФайлаВСамостоятельныйПакет"
				, ЭтаФорма
				, ДополнительныеПараметры);
			
		ИначеЕсли Операция = "ДобавитьВВыделенныеПакеты" Тогда
			
			ОбработкаВыбораФайла = Новый ОписаниеОповещения("ПослеВыбораФайлаВВыделенныеПакеты"
				, ЭтаФорма
				, ДополнительныеПараметры);
			
		Иначе
			
			ВызватьИсключение СтрШаблон("Неизвестная операция при выборе файла (%1)!", Операция);
	...
	Функция ПредставлениеТипаПолуформализованного(DocumentType)
		...
		ТипыНеформализованных.Вставить("ProformaInvoice"		, "Счет на оплату");
...
//{ ОБРАБОТКА СОБЫТИЙ
	...
	Процедура ОбработчикОткрытиеФормыФормаВводаТаблицыЗначений(РезультатЗакрытия, ТекущиеДанные) Экспорт
		...
		Если НЕ РезультатЗакрытия = Неопределено Тогда
			...
			Метрики_ДобавитьПоведение_НажатиеКнопки(Метрики_ДействияНаФорме().СохранитьДопИнформацию);
			ДобавитьСтатистику_СохранитьДопИнформацию(РезультатЗакрытия, ТекущиеДанные);
	...
	Процедура ПослеВыбораФайлаВВыделенныеПакеты(МассивАдресовФайлов, ДополнительныеПараметры) Экспорт
		...
		МетодКлиента(, "ОткрытьФормуОбработкиМодально", "ФормаДокумента"
			, ПараметрыФормы
			, ЭтаФорма
			, "ОбработчикЗакрытияФормыДокумента_МодификацияПакета"
			, 
			, ЭтаФорма);
...
//{ УПРАВЛЕНИЕ ФОРМОЙ
	...
	Процедура УстановитьУсловноеОформлениеСпискаДокументов(ТаблицаФормы)
		...
		// Красный цвет текста, если документ в устаревшем формате
		...
		// Оранжевый цвет фона, если это тестовый документ
		ЭлементУО = УсловноеОформление.Элементы.Добавить();
		...
		ЭлементУО.Оформление.УстановитьЗначениеПараметра("ЦветФона", WebЦвета.ТемноОранжевый);
		
		// Серый фон флажка, если документ уже выгружен
		ЭлементУО = УсловноеОформление.Элементы.Добавить();
		
		ОформляемоеПоле = ЭлементУО.Поля.Элементы.Добавить();
		ОформляемоеПоле.Поле = Новый ПолеКомпоновкиДанных(ТаблицаФормы.ПутьКДанным + "Вкл");
		
		ЭлементОтбора = ЭлементУО.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(ТаблицаФормы.Имя + ".ДокВыгружен");
		ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
		ЭлементОтбора.ПравоеЗначение = Истина;
		
		ЭлементУО.Оформление.УстановитьЗначениеПараметра("ЦветФона", WebЦвета.НейтральноСерый);
		
		// Флажок только на просмотр, если поле ID заполнено
		ЭлементУО = УсловноеОформление.Элементы.Добавить();
		...
		ЭлементОтбора = ЭлементУО.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(ТаблицаФормы.Имя + ".ID");
	...
	Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
		...
		УстановитьУсловноеОформлениеСпискаДокументов(Элементы.СписокДокументов);
		...
		ЕстьНастройкаОтправкиВУстаревшемФормате = УстревшиеНастройки.ЕстьУстаревшее;
	...
	Процедура ПриОткрытии(Отказ)
		...
		Иначе
			НачатьОбновлениеСпискаИсходящихДокументов();
		КонецЕсли;
		...
		ПодключитьОбработчикОжидания("ПоказатьПредупреждениеОбУстаревшихНастройкахОтправки", 0.1, Истина);
	...
	&НаКлиенте
	Процедура УстановитьПараметрыПодразделения(ТекущиеДанные, DepartmentName, DepartmentID, УстановитьДляВсех = Ложь)
	...
	Процедура СписокДокументовПередНачаломИзменения(Элемент, Отказ)
		...
		Если ИмяТекущейКолонки = "СписокДокументовДопПоле" Тогда
			...
			Метрики_ДобавитьПоведение_НажатиеКнопки(Метрики_ДействияНаФорме().ЗаполнитьДопИнформацию);
			ДобавитьСтатистику_ЗаполнитьДопИнформацию(ТекущиеДанные.Организация);			
	...
	Процедура ОрганизацияОтправительНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
		...
		СписокОрг = ПолучитьСписокАктивныхОрганизаций();
...
//{ КОМАНДЫ
	...
	Процедура ПросмотрФормы(Команда)
		...
		ПараметрыФормы.Вставить("Контрагент"			, ТекДанные.Контрагент);
	...
	Процедура Выгрузить(Команда)
		...
		МассивОтмеченных = ПомеченныеСтрокиТаблицы(СписокДокументов);
		
		ДопПеременные = Новый Соответствие;
		ДопПеременные.Вставить("КоличествоДокументовВсего"		, СписокДокументов.Количество());
		ДопПеременные.Вставить("КоличествоДокументовКОбработке"	, МассивОтмеченных.Количество());
		ДобавитьСтатистику_НажалиПодписатьИОтправить(ДопПеременные);
	...
	Процедура ВыгрузитьСДиска(Команда)
		...
		Если НЕ ЗначениеЗаполнено(ОрганизацияОтправитель) Тогда
			СообщитьПользователю(НСтр("ru = 'Выберите отправителя'"));
			Возврат;
		КонецЕсли;
		
		Если ПустаяСтрока(КонтрагентПолучательID) Тогда
			СообщитьПользователю(НСтр("ru = 'Выберите получателя'"));
			Возврат;
		КонецЕсли;
		
		Если СписокФайловНаОтправку.Количество() = 0 Тогда
			СообщитьПользователю(НСтр("ru = 'Нет файлов, предназначенных для отправки'"));
			Возврат;
		КонецЕсли;
		...
		Для Каждого СтрокаТаблицы Из СписокФайловНаОтправку Цикл
			...
			ДанныеФайла.Вставить("ИмяФайла"						, СтрокаТаблицы.Файл);
			ДанныеФайла.Вставить("ПутьКФайлу"					, СтрокаТаблицы.ПутьКФайлу);
			ДанныеФайла.Вставить("Комментарий"					, СтрокаТаблицы.Комментарий);
			ДанныеФайла.Вставить("ЗапрашиватьОтветнуюПодпись"	, СтрокаТаблицы.ЗапрашиватьответнуюПодпись);
			
			ДанныеФайловДляОтправки.Добавить(ДанныеФайла);
		КонецЦикла;
		...
		Если КоличествоОтправлено > 0 Тогда
			...
			ШаблонСообщения = НСтр("ru = 'Успешно отправлено %1 из %2'");
			
			ПараметрОтправлено = Строка(КоличествоОтправлено);
			Если КоличествоОтправлено >= 2 И КоличествоОтправлено <= 4 Тогда
				ПараметрОтправлено = ПараметрОтправлено + " файла";
			Иначе
				ПараметрОтправлено = ПараметрОтправлено + " файлов";
			КонецЕсли;
			
			ТекстСообщения = СтрШаблон(	ШаблонСообщения,
										ПараметрОтправлено,
										СписокФайловНаОтправку.Количество()	);
	...
	Процедура ПередатьНаСогласование(Команда)
		...
		ДопПеременные.Вставить("КоличествоДокументовВсего"		, СписокДокументов.Количество());
		ДопПеременные.Вставить("КоличествоДокументовКОбработке"	, МассивОтмеченных.Количество());
		ДобавитьСтатистику_НажалиПередатьНаСогласование(ДопПеременные);
	...
	Процедура ПередатьНаПодписание(Команда)
		...
		ДопПеременные.Вставить("КоличествоДокументовВсего"		, СписокДокументов.Количество());
		ДопПеременные.Вставить("КоличествоДокументовКОбработке"	, МассивОтмеченных.Количество());
		ДобавитьСтатистику_НажалиПередатьНаПодписание(ДопПеременные);
	...
	Процедура ПоказатьИнструкцию_ЕслиНеВижуДокумент(Команда)
		...
		Метрики_ДобавитьПоведение_НажатиеКнопки(Метрики_ДействияНаФорме().ИнструкцияНеВижуДокумент);
		ДобавитьСтатистику_ПоказатьИнструкцию_ЕслиНеВижуДокумент();
...
//{ РАСШИРЕНИЕ КОНФИГУРАЦИИ
	...
	Процедура СформироватьТаблицуЗначенийДокументовДляОтправкиДиадокРасширение(СписокОрганизаций, СписокКонтрагентов, МассивОтношенийОрганизацийКонтрагентов, СтруктураПараметрыДокументаРасширение, ПользовательАвторизованПоСертификату = Ложь)
		...
		МассивОрганизаций.Добавить(СтруктураПараметрыДокументаРасширение.Организация);
		...
		СтруктТаблицыДляВыгрузкиВДиадок = МетодСервера("Модуль_ИнтеграцияУниверсальный"
			, "СформироватьТаблицыДляВыгрузкиВДиадок"
			,
			,
			,
			, МассивОрганизаций
			...
			, Истина
			, ПользовательАвторизованПоСертификату);
			
		ИтоговаяТаблица = МетодСервера("Модуль_РаботаСВнешнимиПечатнымиФормами", "ДополнитьТаблицуВнешнимиПФ"
			, МассивОтношенийОрганизацийКонтрагентов
			, СтруктТаблицыДляВыгрузкиВДиадок);
...
&НаКлиенте
Функция КонтекстДиадока()
	
	Результат = Платформа.ПараметрыКлиент.КонтекстДиадока;
...
&НаКлиенте
Функция Метрики_ПредставлениеBoxId(ПараметрBoxId)
	
	Результат = МетодКлиента("Модуль_Клиент", "Метрики_ПредставлениеBoxId", ПараметрBoxId);
...
&НаКлиенте
Функция ПолучитьBoxIdПоОрганизации1С(ОрганизацияСсылка) 
	
	Результат = МетодКлиента("Модуль_РаботаССерверомДиадок", "ПолучитьBoxIdПоОрганизации1С", ОрганизацияСсылка);
...
Функция ИспользоватьМетрики()
	...
	Результат = МетодКлиента("Модуль_Клиент", "ИспользоватьМетрики");
...
&НаКлиенте
Функция Метрики_ДействияНаФорме()
	
	Результат = Новый Структура;
	
	Результат.Вставить("ИнструкцияНеВижуДокумент"	, "Инструкция не вижу документ");
	Результат.Вставить("ЗаполнитьДопИнформацию"		, "Заполнить доп информацию");
	Результат.Вставить("СохранитьДопИнформацию"		, "Сохранить доп информацию");
	Результат.Вставить("ПодписатьИОтправить"		, "Подписать и отправить");
	Результат.Вставить("ПередатьНаСогласование"		, "Передать на согласование");
	Результат.Вставить("ПередатьНаПодпись"			, "Передать на подпись");
...
&НаКлиенте
Процедура ДобавитьСтатистику_ПоказатьИнструкцию_ЕслиНеВижуДокумент()
	
	ВозможныеДействия = Метрики_ДействияНаФорме();
	
	НазваниеДействия = ВозможныеДействия.ИнструкцияНеВижуДокумент;
	...
	ДопПеременные.Вставить("BoxId", МассивBoxId);
	
	Метрики_ДобавитьСтатистику(НазваниеДействия, , ДопПеременные);
	
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьСтатистику_ЗаполнитьДопИнформацию(ОрганизацияДокумента)
	
	ВозможныеДействия = Метрики_ДействияНаФорме();
	
	НазваниеДействия = ВозможныеДействия.ЗаполнитьДопИнформацию;
									
	ДопПеременные = Новый Соответствие;
	
	ПараметрBoxId 		= ПолучитьBoxIdПоОрганизации1С(ОрганизацияДокумента);
	ПредставлениеBoxId 	= Метрики_ПредставлениеBoxId(ПараметрBoxId);
	
	ДопПеременные.Вставить("BoxId", ПредставлениеBoxId);
...
&НаКлиенте
Процедура ДобавитьСтатистику_СохранитьДопИнформацию(РезультатЗакрытия, СтрокаСпискаНаОтправку)
	
	ВозможныеДействия = Метрики_ДействияНаФорме();
	
	НазваниеДействия = ВозможныеДействия.СохранитьДопИнформацию;
	
	ДопПеременные = Новый Соответствие;
	
	ПараметрBoxId 		= ПолучитьBoxIdПоОрганизации1С(СтрокаСпискаНаОтправку.Организация);
	ПредставлениеBoxId 	= Метрики_ПредставлениеBoxId(ПараметрBoxId);
	ПредставлениеCounteragentID = Метрики_ПредставлениеBoxId(СтрокаСпискаНаОтправку.CounteragentID);
	
	ДопПеременные.Вставить("BoxId"				, ПредставлениеBoxId);
	ДопПеременные.Вставить("BoxIdКонтрагента"	, ПредставлениеCounteragentID);
	
	ДопСведения 	= СтрокаСпискаНаОтправку.ДопПоле;
	КоллекцияКлючей = Новый Массив;
	Пока ДопСведения <> "" Цикл
	
		КлючИЗначение =	МетодКлиента("Модуль_Клиент", "ВыделитьСлово", ДопСведения, ";");
		Ключ = СтрЗаменить(МетодКлиента("Модуль_Клиент", "ВыделитьСлово", КлючИЗначение, "="), "=", "");
		
		КоллекцияКлючей.Добавить(Ключ);
		
	КонецЦикла;
	
	ДопПеременные.Вставить("Ключи"				, КоллекцияКлючей);
	
	ДанныеФормата = СтрокаСпискаНаОтправку.ФорматОтправки;
	
	ДопПеременные.Вставить("ТипКонтента"		, ДанныеФормата.ТипКонтента);
	ДопПеременные.Вставить("ФункцияДокумента"	, ДанныеФормата.ФункцияДокумента);
	ДопПеременные.Вставить("БазовыйФормат"		, ДанныеФормата.БазовыйФормат);
...
&НаКлиенте
Процедура ДобавитьСтатистику_ВыполнитьОтправку(СтрокаСпискаВыгрузки, ТипЗапроса)
	
	Если Не ИспользоватьМетрики() Тогда
		Возврат;
	КонецЕсли;
	
	ВсеТипыЗапросов = МетодКлиента("Модуль_Клиент", "ДоступныеТипыЗапросовВКомпоненте");
	
	ВозможныеДействияНаФорме = Метрики_ДействияНаФорме();
	
	Если ТипЗапроса = Неопределено Тогда
		НазваниеДействия = ВозможныеДействияНаФорме.ПодписатьИОтправить;
	ИначеЕсли ТипЗапроса = ВсеТипыЗапросов.ЗапросСогласования Тогда
		НазваниеДействия = ВозможныеДействияНаФорме.ПередатьНаСогласование;
	ИначеЕсли ТипЗапроса = ВсеТипыЗапросов.ЗапросКонечнойПодписи Тогда
		НазваниеДействия = ВозможныеДействияНаФорме.ПередатьНаПодпись;
	Иначе
		НазваниеДействия = "НеопределенноеЗначение";
	КонецЕсли;
	
	ПараметрBoxId 		= ПолучитьBoxIdПоОрганизации1С(СтрокаСпискаВыгрузки.Организация);
	ПредставлениеBoxId 	= Метрики_ПредставлениеBoxId(ПараметрBoxId);
				
	DocumentId 	= СтрокаСпискаВыгрузки.Id;
			
	Переменные = Новый Соответствие;
	Переменные.Вставить("BoxID"				, ПредставлениеBoxId);
	Переменные.Вставить("LetterId"			, Лев(DocumentId, 36));
	Переменные.Вставить("DocumentId"		, DocumentId);
	Переменные.Вставить("ТипКонтента"		, СтрокаСпискаВыгрузки.ФорматОтправки.ТипКонтента);
	Переменные.Вставить("ФункцияДокумента"	, СтрокаСпискаВыгрузки.ФорматОтправки.ФункцияДокумента);
	Переменные.Вставить("БазовыйФормат"		, СтрокаСпискаВыгрузки.ФорматОтправки.БазовыйФормат);
	Переменные.Вставить("ИнфоПоля"			, ЗначениеЗаполнено(СтрокаСпискаВыгрузки.ДопПоле));
					
	МетодКлиента("Модуль_Клиент", "Метрика_ДобавитьСтатистику_ВыполнитьОтправку", НазваниеДействия, Переменные);
	
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьСтатистику_НажалиПодписатьИОтправить(Переменные)
	
	Категории = МетодКлиента("Модуль_Клиент", "Метрика_Категории");
		
	НазваниеФормы 		= СинонимТекущейФормы();
	НазваниеКатегории 	= Категории.ОтправкаДокументов;
	НазваниеДействия 	= Метрики_ДействияНаФорме().ПодписатьИОтправить;
...
&НаКлиенте
Процедура ДобавитьСтатистику_НажалиПередатьНаСогласование(Переменные)
	
	Категории = МетодКлиента("Модуль_Клиент", "Метрика_Категории");
		
	НазваниеФормы 		= СинонимТекущейФормы();
	НазваниеКатегории 	= Категории.ОтправкаДокументов;
	НазваниеДействия 	= Метрики_ДействияНаФорме().ПередатьНаСогласование;
...
&НаКлиенте
Процедура ДобавитьСтатистику_НажалиПередатьНаПодписание(Переменные)
	
	Категории = МетодКлиента("Модуль_Клиент", "Метрика_Категории");
		
	НазваниеФормы 		= СинонимТекущейФормы();
	НазваниеКатегории 	= Категории.ОтправкаДокументов;
	НазваниеДействия 	= Метрики_ДействияНаФорме().ПередатьНаПодпись;
...
&НаКлиенте
Процедура Метрики_ДобавитьПоведение_НажатиеКнопки(Действие, Категория = Неопределено, Переменные = Неопределено)
	...
	Если Не ЗначениеЗаполнено(Категория) Тогда
		...
		Категории = МетодКлиента("Модуль_Клиент", "Метрика_Категории");
		Категория = Категории.ОтправкаДокументов;
		
	КонецЕсли;
	
	МетодКлиента("Модуль_Клиент", "Метрика_ДобавитьПоведение_НажатиеКнопки", НазваниеФормы, Категория, Действие, Переменные);
	
КонецПроцедуры

&НаКлиенте
Процедура Метрики_ДобавитьСтатистику(Действие, Категория = Неопределено, Переменные = Неопределено)
