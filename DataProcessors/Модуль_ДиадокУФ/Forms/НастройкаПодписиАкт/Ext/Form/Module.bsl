...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Заголовок") Тогда
		Элементы.Группа2.Заголовок = Параметры.Заголовок;
	КонецЕсли;
	
	ПараметрыПодписания = МетодСервера(,"ПолучитьПараметрыПодписанияТ12вДиадок", Организация);
	...
	ФорматыУПДУКД = Новый Массив;
	ФорматыУПДУКД.Добавить(БазовыеФорматы.utd820);
	ФорматыУПДУКД.Добавить(БазовыеФорматы.utd);
	ФорматыУПДУКД.Добавить(БазовыеФорматы.ucd736);
	ФорматыУПДУКД.Добавить(БазовыеФорматы.ucd);
	
	БазовыйФормат = ФорматЭД.БазовыйФормат;
	
	Элементы.НастроитьПараметрыПодписанияУПД.Видимость = (ФорматыУПДУКД.Найти(БазовыйФормат) <> Неопределено);
...
&НаКлиенте
Процедура НастроитьПараметрыПодписанияУПД(Команда)
