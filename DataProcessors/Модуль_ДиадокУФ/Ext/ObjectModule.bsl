...
#Область МАНИФЕСТ
	...
	Функция ФункцииМодуля()
		...
		ДобавитьФункциюВМанифест(Результат, "УведомленияПользователей_Новый_КритерииОтбора");
		ДобавитьФункциюВМанифест(Результат, "УведомленияПользователей_СписокУведомлений");
		
		// Автообновление модуля
		ДобавитьФункциюВМанифест(Результат, "ДанныеДляОбновленияМодуля");
		ДобавитьФункциюВМанифест(Результат, "Перечисление_МестоположенияМодуля");
		ДобавитьФункциюВМанифест(Результат, "ФайлОбработкиИзСправочникаВнешнихОбработок");
		ДобавитьФункциюВМанифест(Результат, "ЗначениеНастройкиОтключитьАвтоматическоеОбновлениеМодуля");
...
// Набор параметров, заполненный частично и необходимый для:
//	- оценки возможности автообновления,
// 	- выполнения автообновления,
//	- уведомления пользователя об автообновлении.
//	Дальнейшее заполнение выполняется в методе ДополнитьДанныеДляОбновленияМодуля()
// 
// Возвращаемое значение:
//  Структура - см. метод НовыеДанныеДляОбновленияМодуля()
//
Функция ДанныеДляОбновленияМодуля() Экспорт
	
	Результат = НовыеДанныеДляОбновленияМодуля();
	
	Если МодульЗапущенВпервые() Тогда
	...
	РасположениеМодуля = РасположениеТекущегоМодуля();
	Результат.РасположениеМодуля = РасположениеМодуля;
		
	МестоположенияМодуля = Перечисление_МестоположенияМодуля();
	
	Если РасположениеМодуля.Местоположение = МестоположенияМодуля.ОбработкаКонфигурации Тогда
		Результат.КомментарийМодуля = "МодульВСоставеКонфигурации";
		Результат.МожноОбновитьМодуль = Ложь;
		Возврат Результат;
	КонецЕсли;
	
	Если РасположениеМодуля.Местоположение = МестоположенияМодуля.ЭлементСправочника Тогда
		ТекстОшибки = ПроверитьВозможностьОбновленияЭлементаСправочникаМодуля(РасположениеМодуля.Ссылка);
...
// Получает файл обработки из справочника внешних обработок
//
// Параметры:
//  ВнешняяОбработка - СправочникСсылка - элемент в справочнике внешних обработок,
//                                        из которого нужно получить файл
// 
// Возвращаемое значение:
//  ДвоичныеДанные - двоичные данные файла обработки
//
Функция ФайлОбработкиИзСправочникаВнешнихОбработок(ВнешняяОбработка) Экспорт
	
	ХранилищеОбработки = ЗначениеРеквизитаОбъекта(ВнешняяОбработка, "ХранилищеОбработки");
	
	Результат = ХранилищеОбработки.Получить();
...
// Записывает новый файл в справочник внешних обработок
//
// Параметры:
//  ВнешняяОбработка - СправочникСсылка - элемент в справочнике внешних обработок,
//                                        в который нужно записать файл модуля;
//  ДвоичныеДанные   - ДвоичныеДанные   - файл нового модуля;
//
Процедура ОбновитьМодульВСправочникеДополнительныхОбработок(ВнешняяОбработка, ДвоичныеДанные) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	СведенияОбОбработке = СведенияОВнешнейОбработкеИзФайла(ДвоичныеДанные);
	
	ОбработкаОбъект = ВнешняяОбработка.ПолучитьОбъект();
	ОбработкаОбъект.Версия = СведенияОбОбработке.Версия;
	ОбработкаОбъект.Наименование = СведенияОбОбработке.Наименование;	
	ОбработкаОбъект.ХранилищеОбработки = Новый ХранилищеЗначения(ДвоичныеДанные);
	ОбновитьКомандыОбработкиВСправочнике(ОбработкаОбъект, СведенияОбОбработке);
...
Процедура СохранитьНастройкуРежимСовместимостиФорматов155и820()
	
	ИдентификаторНастройки = ИдентификаторНастройкиРежимСовместимостиФорматов155и820();
	ЗначениеНастройки = ИспользоватьРежимСовместимостиФорматов155и820();
...
// Сохраняет версию текущей обработки.
Процедура СохранитьНастройкуСохраненнаяВерсияМодуля(ВерсияОбработки = "") Экспорт
	
	ИдентификаторНастройки = ИдентификаторСвойстваСохраненнаяВерсияМодуля();
	
	Если НЕ ЗначениеЗаполнено(ВерсияОбработки) Тогда
		ВерсияОбработки = ВерсияОбработки();
	КонецЕсли; 
	
	УстановитьЗначениеКонстанты(ИдентификаторНастройки, ВерсияОбработки);
...
Функция ИдентификаторНастройкиОтключитьАвтоматическоеОбновлениеМодуля()
	
	Возврат "ДиадокОтключитьАвтоматическоеОбновлениеМодуля";
	
КонецФункции

// Устанавливает значение настройки "ОтключитьАвтоматическоеОбновлениеМодуля"
//
// Параметры:
//	ЗначениеНастройки - Булево - значение настройки, которое требуется устаноить   
//
Процедура СохранитьНастройкуОтключитьАвтоматическоеОбновлениеМодуля(ЗначениеНастройки) Экспорт
	
	ИдентификаторНастройки = ИдентификаторНастройкиОтключитьАвтоматическоеОбновлениеМодуля();		
	
	УстановитьЗначениеКонстанты(ИдентификаторНастройки, ЗначениеНастройки);
	
КонецПроцедуры

// Возвращает значение настройки "ОтключитьАвтоматическоеОбновлениеМодуля"
// 
// Возвращаемое значение:
// - Булево - если настройка изменялась;
// - Неопределено - если настройка никогда не устанавливалась;
//
Функция ЗначениеНастройкиОтключитьАвтоматическоеОбновлениеМодуля() Экспорт
	
	ИдентификаторНастройки = ИдентификаторНастройкиОтключитьАвтоматическоеОбновлениеМодуля();
	
	Возврат ПолучитьЗначениеКонстанты(ИдентификаторНастройки);
...
Функция НовыеДанныеДляОбновленияМодуля()
	...
	Результат.Вставить("ОписаниеОкруженияМодуля", ОписаниеОкруженияМодуля());
	Результат.Вставить("РасположениеМодуля", НовоеОписаниеРасположенияМодуля());
...
Функция ОписаниеОкруженияМодуля()
	
	Результат = Новый Структура;
	
	СистемнаяИнформация	= Новый СистемнаяИнформация;
	СведенияОКонфигурации = СведенияОКонфигурации();
	
	Результат.Вставить("ВерсияПлатформы", СистемнаяИнформация.ВерсияПриложения);
	
	Результат.Вставить("Маркер" , СведенияОКонфигурации.Маркер);
	Результат.Вставить("ИмяКонфигурации", СведенияОКонфигурации.Имя);
	Результат.Вставить("ВерсияКонфигурации" , СведенияОКонфигурации.ВерсияКонфигурации);
	Результат.Вставить("ИмяБазовойПодсистемы" , СведенияОКонфигурации.БазоваяПодсистема);
	Результат.Вставить("ВерсияБазовойПодсистемы" , СведенияОКонфигурации.ВерсияБазовойПодсистемы);
...
Функция МодульЗапущенВпервые()
	
	Результат = Ложь;
	
	СохраненнаяВерсия = ЗначениеНастройкиСохраненнаяВерсияМодуля();
	НайденаСохраненнаяВерсия = ЗначениеЗаполнено(СохраненнаяВерсия);
	
	Результат = Не НайденаСохраненнаяВерсия;
...
Функция МодульБылОбновлен()
	...
	Если ЗначениеЗаполнено(СохраненнаяВерсия) Тогда
...
// 
// Возвращаемое значение:
//  Структура - см. функцию НовоеОписаниеРасположенияМодуля()
//
Функция РасположениеТекущегоМодуля()
	
	Результат = НовоеОписаниеРасположенияМодуля();
	
	Местоположение = МестоположениеМодуля();
	ИмяОбработки = ПараметрыКлиентСервер.ИмяОбработки;
	ЗначениеИспользуемоеИмяФайла = ЗначениеСвойстваОбъектаИспользуемоеИмяФайла();
	
	Результат.Имя = ИмяОбработки;
	Результат.Местоположение = Местоположение;
	Результат.ПолноеИмя = ПараметрыКлиентСервер.ПолноеИмяОбработки;
	Результат.ИспользуемоеИмяФайла = ЗначениеИспользуемоеИмяФайла;
	
	МестоположенияМодуля = Перечисление_МестоположенияМодуля();
	
	Если Местоположение = МестоположенияМодуля.ФайлНаДиске Тогда
		
		Результат.ПутьКФайлу = ЗначениеИспользуемоеИмяФайла;
	
	ИначеЕсли Местоположение = МестоположенияМодуля.ЭлементСправочника Тогда
		
		СсылкаНаМодуль = СсылкаНаМодульИзСправочникаВнешнихОбработок();
		Если ЗначениеЗаполнено(СсылкаНаМодуль) Тогда
			Результат.Ссылка = СсылкаНаМодуль;
		КонецЕсли;
...
Функция МестоположениеМодуля()
	
	Результат = "";
	
	СтрокаИспользуемоеИмяФайла = ЗначениеСвойстваОбъектаИспользуемоеИмяФайла();
	
	ИмяФайлаОбъектаОтсутствует = ПустаяСтрока(СтрокаИспользуемоеИмяФайла);
	ИмяФайлаОбъектаСодержитКаталогВременныхФайлов = ( Найти(СтрокаИспользуемоеИмяФайла, КаталогВременныхФайлов()) > 0 );
	ИмяФайлаОбъектаСодержитАдресВременногоХранилища = СтрокаСодержитАдресВременногоХранилища(СтрокаИспользуемоеИмяФайла);
													  
	МестоположенияМодуля = Перечисление_МестоположенияМодуля();                                    
	
	Если ИмяФайлаОбъектаОтсутствует Тогда
		Результат = МестоположенияМодуля.ОбработкаКонфигурации;
	ИначеЕсли ИмяФайлаОбъектаСодержитКаталогВременныхФайлов или ИмяФайлаОбъектаСодержитАдресВременногоХранилища Тогда
...
Функция ЗначениеСвойстваОбъектаИспользуемоеИмяФайла()
	
	Результат = "";
	
	ЕстьРеквизитИлиСвойствоОбъекта(ЭтотОбъект, "ИспользуемоеИмяФайла", Результат);
...
Функция СтрокаСодержитАдресВременногоХранилища(Строка)
	...
	Для Каждого Префикс Из ПрефиксыАдресовВХ Цикл
		...
		Если Найти(СтрокаДляПроверки, Префикс) = 1 Тогда
			Результат = Истина;
			Прервать;
		КонецЕсли;
...
// Ищет подходящую ссылку, в которой содержится файл обработки модуля
// 	подходящей версии
// 
// Возвращаемое значение:
//  СправочникСсылка
//
Функция СсылкаНаМодульИзСправочникаВнешнихОбработок()
	...
	Если Не ЕстьМетаданныеКонфигурации_ДД("Справочники." + ИмяСправочника) Тогда
		Возврат Результат;
	КонецЕсли;
	
	ИмяОбработки = ПараметрыКлиентСервер.ИмяОбработки;
	
	РеквизитВерсия		 = Метаданные.Справочники[ИмяСправочника].Реквизиты.Версия;
	ДлинаРеквизитаВерсия = РеквизитВерсия.Тип.КвалификаторыСтроки.Длина;
	ТребуемаяВерсия 	 = ВерсияОбработки();
	Версия				 = Лев(ТребуемаяВерсия, ДлинаРеквизитаВерсия);
	...
	Пока Выборка.Следующий() Цикл
		...
		Попытка
			ДвоичныеДанные		 = ФайлОбработкиИзСправочникаВнешнихОбработок(Выборка.Ссылка);
			СведенияОбОбработке	 = СведенияОВнешнейОбработкеИзФайла(ДвоичныеДанные);
		...
		Если ВерсияОбработки = ТребуемаяВерсия Тогда
			Результат = Выборка.Ссылка;
			Прервать;
		КонецЕсли;
...
Функция СведенияОВнешнейОбработкеИзФайла(ДвоичныеДанные)
	
	ИмяТекущейОбработки	 = ПараметрыКлиентСервер.ИмяОбработки;
	ИмяНовойОбработки	 = ИмяТекущейОбработки + "_ДляЧтенияСведений";
	АдресОбработки		 = ПоместитьВоВременноеХранилище(ДвоичныеДанные);
	
	ВнешниеОбработки_Подключить(АдресОбработки, ИмяНовойОбработки);
	
	ОбъектОбработки = ВнешниеОбработки_СоздатьПоИмени(ИмяНовойОбработки);
...
Процедура ОбновитьКомандыОбработкиВСправочнике(ОбработкаОбъект, СведенияОбОбработке)
	
	НовыеКоманды = СведенияОбОбработке.Команды;
	ТекущиеКоманды = ОбработкаОбъект.Команды;
	
	Для Каждого ОписаниеНовойКоманды Из НовыеКоманды Цикл
		
		Идентификатор = ОписаниеНовойКоманды.Идентификатор;
		
		СтрокаТекущейКоманды = ТекущиеКоманды.Найти(Идентификатор, "Идентификатор"); 
		
		Если СтрокаТекущейКоманды = Неопределено Тогда
		    ДобавитьНовуюКомандуОбработкиВСправочнике(ОбработкаОбъект, ОписаниеНовойКоманды);
		Иначе
			ОбновитьТекущуюКомандуОбработкиВСправочнике(ОбработкаОбъект, СтрокаТекущейКоманды, ОписаниеНовойКоманды);	
		КонецЕсли;
...
Процедура ДобавитьНовуюКомандуОбработкиВСправочнике(ОбработкаОбъект, ОписаниеНовойКоманды)
	
	Использование = ОписаниеНовойКоманды.Использование;
	ВариантЗапуска = Перечисления.СпособыВызоваДополнительныхОбработок[Использование];
			
	НоваяКоманда = ОбработкаОбъект.Команды.Добавить();
	ЗаполнитьЗначенияСвойств(НоваяКоманда, ОписаниеНовойКоманды);
	НоваяКоманда.ВариантЗапуска = ВариантЗапуска;
	
КонецПроцедуры

Процедура ОбновитьТекущуюКомандуОбработкиВСправочнике(ОбработкаОбъект, ТекущаяКоманда, ОписаниеНовойКоманды)
	
	ТекущаяКоманда.Представление = ОписаниеНовойКоманды.Представление;
	
	ИмяКолонки = "Скрыть";
	Если ОписаниеНовойКоманды.Владелец().Колонки.Найти(ИмяКолонки) <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(ТекущаяКоманда, ОписаниеНовойКоманды, ИмяКолонки);	
	КонецЕсли;
...
Функция КлючРазработчикаПоУмолчанию()
	Возврат "1S83-5-55-05-24427b35-6ab4-4d96-8615-e43ab5e9be5c";
КонецФункции
...
//{ СЛУЖЕБНЫЕ ПРОЦЕДУРЫ И ФУНКЦИИ
	...
	Функция КаталогиВложенныхОбработокВРежимеОтладки()
		...
		Результат.Вставить("УведомленияПользователей"	, КаталогБиблиотек);
	...
	Процедура СохранитьНастройкиПодключаемогоМодуля() Экспорт
		...
		СохранитьНастройкуРежимСовместимостиФорматов155и820();
...
Функция УведомленияПользователей_Новый_КритерииОтбора() Экспорт 
	
	УведомленияПользователей = МодульУведомленияПользователей();
	Результат = УведомленияПользователей.Новый_КритерииОтбора();
	
	Возврат Результат;
	
КонецФункции

Функция УведомленияПользователей_СписокУведомлений(ДокументXML, КритерииОтбораУведомлений) Экспорт 
	
	УведомленияПользователей = МодульУведомленияПользователей();
	Результат = УведомленияПользователей.СписокУведомлений(ДокументXML, КритерииОтбораУведомлений);
	
	Возврат Результат;
	
КонецФункции

Функция МодульУведомленияПользователей()
	
	ИмяМодуля = ИмяБиблиотекиУведомленияПользователей();
	Результат = ПолучитьМодуль(ИмяМодуля);
...
Функция ИмяБиблиотекиУведомленияПользователей()
	
	Возврат "УведомленияПользователей";
