...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИнформацияОбОбработке = ОбъектОбработка.ПолучитьИнформациюОбОбработке();	
	
	ТекущаяВерсияПоследняя = ИнформацияОбОбработке.ТекущаяВерсия;
	
	ЭтаФорма.Заголовок = "Загрузка данных из табличного документа (v" + ИнформацияОбОбработке.ТекущаяВерсия + ") © Алексеенко П.В.";
	
	ИдентификаторОбработки = ОбъектОбработка.Метаданные().ПолноеИмя();	
	Если Лев(Строка(ТипЗнч(ОбъектОбработка)), 24) = "Внешняя обработка объект" Тогда	
	...
	Для каждого ОбъектМетаданных Из Метаданные.Документы Цикл
		СписокДокументов.Добавить("ДокументСсылка." + ОбъектМетаданных.Имя, ОбъектМетаданных.Представление());
	КонецЦикла;
		
	Для каждого ОбъектМетаданных Из Метаданные.Справочники Цикл
		СписокДокументов.Добавить("СправочникСсылка." + ОбъектМетаданных.Имя, ОбъектМетаданных.Представление());
	КонецЦикла;
	СписокДокументов.СортироватьПоПредставлению();
	
	Если СсылкаИсточника <> Неопределено Тогда
		МетаданныеСсылкиИсточника = СсылкаИсточника.Метаданные();
		Если Метаданные.Справочники.Содержит(МетаданныеСсылкиИсточника) Тогда
			СсылкаИсточникаСписокВыбора = "СправочникСсылка." + МетаданныеСсылкиИсточника.Имя;	
		ИначеЕсли Метаданные.Документы.Содержит(МетаданныеСсылкиИсточника) Тогда
			СсылкаИсточникаСписокВыбора = "ДокументСсылка." + МетаданныеСсылкиИсточника.Имя;
		КонецЕсли;
	КонецЕсли;
	...
	Если Параметры.ОбъектыНазначения <> Неопределено Тогда
		...
		Элементы.Переместить(Элементы.КоманднаяПанельИфостарт, Элементы.ГруппаВариантаЗагрузки);
	...
	ДобавитьПредопределенныеМакетыНастроек(ОбъектОбработка);
	
	УстановитьПростыеНастройки(ОбъектОбработка);
...
Процедура ПриОткрытии(Отказ)
	...
	Если НЕ ВыполняетсяЗаполнениеТЧОбъекта И ВыполнитьСравнениеВерсий(ТекущаяВерсияПоследняя, ТекущаяВерсия) Тогда		
		ПодключитьОбработчикОжидания("ОткрытьОписаниеИзмененийСистемы", 0.2, Истина);
	КонецЕсли;
	ТекущаяВерсия = ТекущаяВерсияПоследняя;
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") Тогда
		...
		ИначеЕсли ВыбранноеЗначение.Источник = "ФормаРедактированияСвязи" И ВыбранноеЗначение.Результат = Истина Тогда
			...
			+?(ПустаяСтрока(ТекДанные.СвязьПоТипу), "", " по типу "+ТекДанные.СвязьПоТипу);
		...
		ИначеЕсли ВыбранноеЗначение.Источник = "ФормаДобавленияДополнительнойНастройки" Тогда
			...
			ПолучитьНастройкиДляСтраницыЗагрузки(НаименованиеСтраницы, ДобавляемаяНастройка.РежимЗагрузки);
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗакрытиеОбработки Тогда
		Отказ =	Истина;
		СтандартнаяОбработка = Ложь;
		
		нПараметры = Новый Структура;
		
		ТекстВопроса = "При закрыти, все сделанные Вами настройки будут утеряны. Продолжить?";
		Если ВерсияПлатформы83 Тогда
			Выполнить("
			|	ОписаниеОповещения = Новый ОписаниеОповещения(""ВопросЗакрытиеОбработки"", ЭтотОбъект, нПараметры);
			|	ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, 60);
			|");
		Иначе
			ОтветНаВопрос = Вопрос(ТекстВопроса, РежимДиалогаВопрос.ДаНет);
			ВопросЗакрытиеОбработки(ОтветНаВопрос, нПараметры)
		КонецЕсли;
...
&НаКлиенте
Процедура ВопросЗакрытиеОбработки(РезультатВопроса, ДополнительныеПараметры) Экспорт
	
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		ЗакрытиеОбработки = Истина;
		ЭтаФорма.Закрыть();	
	КонецЕсли; 
...
&НаКлиенте
Процедура ОткрытьОписаниеИзмененийСистемы()
	
	ОткрытьФорму(ИдентификаторОбработки+".Форма.ФормаОписаниеИзмененийПрограммы", , ЭтаФорма);	
...
Процедура ВосстановитьНастройкиИзСостоянияВыполнения(СостояниеВыполнения)
	...
	Если НЕ ВыполняетсяЗаполнениеТЧОбъекта Тогда
		...
		СсылкаИсточникаСписокВыбора = Неопределено;
	...
	Если РежимЗагрузки = 3 Тогда
		ЗаполнитьВТаблицуЗначенийДвиженияРегистраДокумента("ТаблицаЗагружаемыхРеквизитов")
	КонецЕсли;
	...
	Для каждого СтрокаНастроек Из ВосстановленнаяТаблицаНастроек Цикл
		Если ВыполнитьСравнениеВерсий(ВосстановленнаяТекущаяВерсия, "3.0.1.3") Тогда
			...
			Если СтрокаНастроек.ДопНастройки ИЛИ СтрокаНастроек.ЗагружаетсяДоОбъекта Тогда
				ПолучитьНастройкиДляСтраницыЗагрузки(СтрокаНастроек.КлючСтраницыНастроек, СтрокаНастроек.РежимЗагрузки, Ложь);				
				Если (СтрокаНастроек.РежимЗагрузки = 3  И СтрокаНастроек.ЭтоРегистр) ИЛИ СтрокаНастроек.РежимЗагрузки = 6 Тогда
					СостояниеВыполнения.Свойство(СтрокаНастроек.Значение + "ОбъектЗагрузки", ЭтаФорма[СтрокаНастроек.КлючСтраницыНастроек.КлючСтраницы + "ОбъектЗагрузки"]);
				КонецЕсли;
			КонецЕсли
		ИначеЕсли ВыполнитьСравнениеВерсий(ВосстановленнаяТекущаяВерсия, "3.0.0.3") Тогда		
			СтрокаНастроек.КлючСтраницыНастроек = ДобавитьСтраницуНастроек(СтрокаНастроек.ИмяОбъектаЗагрузки, СтрокаНастроек.Представление, СтрокаНастроек.ДопНастройки, СтрокаНастроек.ЭтоРегистр, СтрокаНастроек.ЗагружаетсяДоОбъекта, ?(СтрокаНастроек.РежимЗагрузки = 5 И СтрокаНастроек.ЭтоРегистр, 3, ?(СтрокаНастроек.РежимЗагрузки = 5, 1, СтрокаНастроек.РежимЗагрузки)));
			Если СтрокаНастроек.ДопНастройки ИЛИ СтрокаНастроек.ЗагружаетсяДоОбъекта Тогда
				ЗаполнитьНастройкамиСозданнуюСтраницу(СтрокаНастроек.КлючСтраницыНастроек, СтрокаНастроек.ЭтоРегистр, СтрокаНастроек.ЗагружаетсяДоОбъекта, Ложь)
			КонецЕсли	
		...
		ИначеЕсли ВыполнитьСравнениеВерсий(ВосстановленнаяТекущаяВерсия, "2.4") Тогда
			...
			Если СтрокаНастроек.ДопНастройки Тогда
				ЗаполнитьНастройкамиСозданнуюСтраницу(СтрокаНастроек.КлючСтраницыНастроек, СтрокаНастроек.ЭтоРегистр, , Ложь)
			КонецЕсли
		Иначе	
			...
			Если СтрокаНастроек.ДопНастройки Тогда
				ЗаполнитьНастройкамиСозданнуюСтраницу(СтрокаНастроек.КлючСтраницыНастроек,  , ,Ложь)
			КонецЕсли
		КонецЕсли
		
	КонецЦикла;
	
	ЗаполнитьНастройкамиСозданнуюСтраницу();
	...
	Для каждого СтрокаНастроек Из ТаблицыНастроек Цикл
		Если (СтрокаНастроек.РежимЗагрузки = 3  И СтрокаНастроек.ЭтоРегистр) ИЛИ СтрокаНастроек.РежимЗагрузки = 6 Тогда		
			ТаблицаЗаполненныхНастроек = ЭтаФорма[СтрокаНастроек.Значение];
			Строки = ТаблицаЗаполненныхНастроек.НайтиСтроки(Новый Структура("ИмяРеквизита", ЭтаФорма[СтрокаНастроек.Значение + "ОбъектЗагрузки"]));		
			Если Строки.Количество() Тогда
				СтрокаНастроек.ТипСтрокой = ПолучитьТипСтрокой(Строки[0].ОписаниеТипов.Типы()[0]);			
			Иначе
				ЭтаФорма[СтрокаНастроек.Значение + "ОбъектЗагрузки"] = "";		
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура СохранитьНастройкуВбазе(Пометка, Представление)
	...
	Для каждого КлючИЗначение Из ТаблицыНастроек Цикл 
		...
		Если (КлючИЗначение.РежимЗагрузки = 3 И КлючИЗначение.ЭтоРегистр) ИЛИ КлючИЗначение.РежимЗагрузки = 6 Тогда
			Таблицы.Добавить(КлючИЗначение.Значение + "ОбъектЗагрузки");	
		КонецЕсли;
	КонецЦикла;
...
&НаСервере
Процедура ДобавитьПредопределенныеМакетыНастроек(ОбработкаОбъект)
...
Процедура УстановитьСписокРегистров()
	...
	Если Документы.ТипВсеСсылки().СодержитТип(ТипЗнч(ЭтаФорма[ИмяОбъекта])) Тогда
		Для каждого Движение Из ЭтаФорма[ИмяОбъекта].Метаданные().Движения Цикл
			//Метаданные.РегистрыНакопления.Содержит(Движение)
...
Процедура СформироватьСтруктуруКолонокТаблицы(Колонки, ПолучаемаяТаблица, НомерКолонки, ИндексТаблицыНастройки, ЗаголовокНастройки, ТипОбъектаСтрокой)
	...
	ТаблицаЗаполненныхНастроек = ЭтаФорма[ПолучаемаяТаблица];
	
	Если (ТаблицыНастроек[ИндексТаблицыНастройки].РежимЗагрузки = 3 ИЛИ ТаблицыНастроек[ИндексТаблицыНастройки].РежимЗагрузки = 6) И ТаблицыНастроек[ИндексТаблицыНастройки].ЗагружаетсяДоОбъекта Тогда
		Строки = ТаблицаЗаполненныхНастроек.НайтиСтроки(Новый Структура("ИмяРеквизита", ЭтаФорма[ПолучаемаяТаблица + "ОбъектЗагрузки"]));	
		Если Строки.Количество() Тогда
			...
			Колонка["ИмяРеквизита"] = "ОбъектЗагрузки";
			Колонки.Вставить(?(ПолучаемаяТаблица = "ТаблицаЗагружаемыхРеквизитов", "", ПолучаемаяТаблица) + Колонка.ИмяРеквизита, Колонка);
	...
	Для каждого ЗагружаемыйРеквизит Из ТаблицаЗаполненныхНастроек Цикл
...
Процедура СформироватьКолонкиТаблицыПодготовленныхДанных()
	...
	Для каждого Таблица Из ТаблицыНастроек Цикл
		...
		Префикс = ?(Таблица.Значение = "ТаблицаЗагружаемыхРеквизитов", "", Таблица.Значение);		
		МассивДобавляемыхРеквизитов = Новый Массив;
		
		Если ЗначениеЗаполнено(Таблица.ТипСтрокой) ИЛИ (Таблица.РежимЗагрузки = 3 И Таблица.ЗагружаетсяДоОбъекта) Тогда
			
			Если ЗначениеЗаполнено(Таблица.ТипСтрокой) Тогда
				
				МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы(Префикс + "ОбъектЗагрузки", Новый ОписаниеТипов(Таблица.ТипСтрокой), "ТаблицаПодготовленныхДанных", "Объект"));
									
			Иначе
				Строки = ЭтаФорма[Таблица.Значение].НайтиСтроки(Новый Структура("ИмяРеквизита", ЭтаФорма[Таблица.Значение + "ОбъектЗагрузки"]));
				Если Строки.Количество() Тогда
					МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы(Префикс + "ОбъектЗагрузки", Строки[0].ОписаниеТипов, "ТаблицаПодготовленныхДанных", "Объект"));	
				КонецЕсли;
			КонецЕсли;
			
			МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы(Префикс + "ОбъектЗагрузкиНайден", Новый ОписаниеТипов("Булево"), "ТаблицаПодготовленныхДанных", "ОбъектЗагрузкиНайден"));
			МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы(Префикс + "ОбъектЗагрузкиНеЗаписывать", Новый ОписаниеТипов("Булево"), "ТаблицаПодготовленныхДанных", "ОбъектЗагрузкиНеЗаписывать"));
			МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы(Префикс + "ОбъектЗагрузкиНеСоздан", Новый ОписаниеТипов("Булево"), "ТаблицаПодготовленныхДанных", "ОбъектЗагрузкиНеСоздан"));
			...
			Отбор.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ТаблицаПодготовленныхДанных." + Префикс + "ОбъектЗагрузкиНайден");
			Отбор.ПравоеЗначение = Ложь;		
			Отбор = ОформлениеНовый.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
			Отбор.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
			Отбор.Использование = Истина;
			Отбор.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ТаблицаПодготовленныхДанных." + Префикс + "ОбъектЗагрузкиНеСоздан");
			Отбор.ПравоеЗначение = Ложь;
			...
			ПолеУО.Поле = Новый ПолеКомпоновкиДанных("ТаблицаПодготовленныхДанных" + Префикс + "ОбъектЗагрузки");
		...
		Для каждого ЗагружаемыйРеквизит Из ЭтаФорма[Таблица.Значение].НайтиСтроки(Новый Структура("Пометка", Истина)) Цикл 
			Если ЗначениеЗаполнено(Таблица.ТипСтрокой) Тогда
				ПолеУО = ОформлениеНайден.Поля.Элементы.Добавить();
				ПолеУО.Поле = Новый ПолеКомпоновкиДанных("ТаблицаПодготовленныхДанных" + Префикс + ЗагружаемыйРеквизит.ИмяРеквизита);
				ПолеУО.Использование = Истина;
				ПолеУО = ОформлениеНеНайден.Поля.Элементы.Добавить();
				ПолеУО.Поле = Новый ПолеКомпоновкиДанных("ТаблицаПодготовленныхДанных" + Префикс + ЗагружаемыйРеквизит.ИмяРеквизита);
				ПолеУО.Использование = Истина;
			КонецЕсли;
			МассивДобавляемыхРеквизитов.Добавить(Новый РеквизитФормы(Префикс + ЗагружаемыйРеквизит.ИмяРеквизита, ЗагружаемыйРеквизит.ОписаниеТипов, "ТаблицаПодготовленныхДанных",ЗагружаемыйРеквизит.ПредставлениеРеквизита)) 
		КонецЦикла;
		...
		Если ЗначениеЗаполнено(Таблица.ТипСтрокой) ИЛИ (Таблица.РежимЗагрузки = 3 И Таблица.ЗагружаетсяДоОбъекта) Тогда
			Элементы["ТаблицаПодготовленныхДанных" + Префикс + "ОбъектЗагрузкиНайден"].Видимость = Ложь;	                              
			Элементы["ТаблицаПодготовленныхДанных" + Префикс + "ОбъектЗагрузкиНеЗаписывать"].Видимость = Ложь;
			Элементы["ТаблицаПодготовленныхДанных" + Префикс + "ОбъектЗагрузкиНеСоздан"].Видимость = Ложь;
			Элементы["ТаблицаПодготовленныхДанных" + Префикс + "ОбъектЗагрузки"].УстановитьДействие("ОбработкаВыбора","ТаблицаПодготовленныхДанныхОбъектЗагрузкиОбработкаВыбора");
			Элементы["ТаблицаПодготовленныхДанных" + Префикс + "ОбъектЗагрузки"].УстановитьДействие("Очистка","ТаблицаПодготовленныхДанныхОбъектЗагрузкиОчистка");	
			Элементы["ТаблицаПодготовленныхДанных" + Префикс + "ОбъектЗагрузки"].УстановитьДействие("ПриИзменении","ТаблицаПодготовленныхДанныхОбъектЗагрузкиПриИзменении");              
		КонецЕсли
	КонецЦикла	
...
//  Объект метаданных
//
Функция ПолучитьМетаданныеОбъектаЗагрузки(ОбъектЗагрузки = Неопределено, ЭтоРегистр = Ложь, ТекРежимЗагрузки = Неопределено)
	
	Если РежимЗагрузки = ТекРежимЗагрузки И ОбъектЗагрузки = Неопределено Тогда
		Возврат ПолучитьМетаданныеИсточника();	
	ИначеЕсли ЗначениеЗаполнено(ОбъектЗагрузки) Тогда
		МетаданныеОсновногоОбъекта = "МетаданныеОсновногоОбъекта = ПолучитьМетаданныеОбъектаПоКлючуНастройки(""ТаблицаЗагружаемыхРеквизитов"")";
		
		Если ТекРежимЗагрузки = 0 ИЛИ ТекРежимЗагрузки = 6 Тогда	
			
			//Возврат Метаданные.Справочники[ОбъектЗагрузки]
			Возврат "МетаданныеОбъекта = Метаданные.Справочники[""" + ОбъектЗагрузки + """]";
			
		ИначеЕсли ТекРежимЗагрузки = 1 Тогда
			
			//Возврат МетаданныеОсновногоОбъекта.ТабличныеЧасти[ОбъектЗагрузки]
			Возврат МетаданныеОсновногоОбъекта +  ";МетаданныеОбъекта = МетаданныеОсновногоОбъекта.ТабличныеЧасти[""" + ОбъектЗагрузки + """]";
			
		ИначеЕсли ТекРежимЗагрузки = 2 Тогда
			
			Если РежимЗагрузки = 3 И ЗначениеЗаполнено(СсылкаДокумента) Тогда
				МетаданныеОсновногоОбъекта = "МетаданныеОсновногоОбъекта = СсылкаДокумента.Метаданные()";
			КонецЕсли;
			
			Возврат МетаданныеОсновногоОбъекта +  ";Для каждого Движение Из МетаданныеОсновногоОбъекта.Движения Цикл
			|	Если Движение.Имя = """ + ОбъектЗагрузки + """ Тогда МетаданныеОбъекта = Движение КонецЕсли	
			|КонецЦикла;";
			
		ИначеЕсли ТекРежимЗагрузки = 3 Тогда
			
			//Возврат Метаданные.РегистрыСведений[ОбъектЗагрузки]
			Возврат "МетаданныеОбъекта = Метаданные.РегистрыСведений[""" + ОбъектЗагрузки + """]";
...
Функция ПолучитьМетаданныеИсточника()
	...
	Если РежимЗагрузки = 0 И ЗначениеЗаполнено(ТипОбъектаСправочника) Тогда
		
		//Возврат Метаданные.Справочники[ТипОбъектаСправочника]
		Возврат "МетаданныеОбъекта = Метаданные.Справочники[""" + ТипОбъектаСправочника + """]";
		
	ИначеЕсли РежимЗагрузки = 1 И ЗначениеЗаполнено(СсылкаИсточника) И ЗначениеЗаполнено(ТабличнаяЧастьИсточника) Тогда
		
		//Возврат СсылкаИсточника.Метаданные().ТабличныеЧасти.Найти(ТабличнаяЧастьИсточника)
		Возврат "МетаданныеОбъекта = СсылкаИсточника.Метаданные().ТабличныеЧасти[""" + ТабличнаяЧастьИсточника + """]";
		
	ИначеЕсли РежимЗагрузки = 2 И ЗначениеЗаполнено(ИмяВидаРегистра) Тогда
		
		//Возврат Метаданные.РегистрыСведений[ИмяВидаРегистра]
		Возврат "МетаданныеОбъекта = Метаданные.РегистрыСведений[""" + ИмяВидаРегистра + """]";
		
	ИначеЕсли (РежимЗагрузки = 3 ИЛИ РежимЗагрузки = 4) И ЗначениеЗаполнено(СсылкаДокумента) Тогда
		
		//Для каждого Движение Из СсылкаДокумента.Метаданные().Движения Цикл Если Движение.Имя = РегистрДокумента Тогда Возврат Движение КонецЕсли КонецЦикла
		Если НЕ ЗначениеЗаполнено(РегистрДокумента) Тогда
			Возврат "";
		КонецЕсли;
		Возврат "Для каждого Движение Из СсылкаДокумента.Метаданные().Движения Цикл Если Движение.Имя = """ + РегистрДокумента + """ Тогда МетаданныеОбъекта = Движение КонецЕсли КонецЦикла;";
		
	ИначеЕсли РежимЗагрузки = 5 И СсылкаИсточника <> Неопределено Тогда	
		
		//Возврат СсылкаИсточника.Метаданные()
		Возврат "МетаданныеОбъекта = СсылкаИсточника.Метаданные()"
...
//
Функция НетВыбранныхМетаданных()

	Возврат ПолучитьМетаданныеОбъектаПоКлючуНастройки("ТаблицаЗагружаемыхРеквизитов") = Неопределено
...
Функция ПолучитьТекстВопросаИсточника()
	...
	МетаданныеИсточника = ПолучитьМетаданныеОбъектаПоКлючуНастройки("ТаблицаЗагружаемыхРеквизитов");
...
Процедура ВыполнитьЗагрузкуДанных()
	...
	ТекстСообщения = ПолучитьТекстВопросаИсточника().ТекстВопроса;
	...
	Если РежимЗагрузки = 5 Тогда
		...
		Для каждого Строка Из ТаблицаДляСветрки Цикл
			...
			Если НЕ ЗначениеЗаполнено(Строка["ОбъектЗагрузки"]) Тогда Продолжить КонецЕсли;
			МассивСтрока = ТаблицаДанных.НайтиСтроки(Новый Структура("ОбъектЗагрузки", Строка.ОбъектЗагрузки));
			ВыполнитьПоследовательнуюЗагрузкуЭлементов(МетаданныеИсточника, МассивСтрока, ТаблицаРеквизитов, ТребуетсяЗагрузкаДоОбъекта, ТекстСообщения, КоличествоЭлементов, Загружено, ТаблицаДанных)
			
		КонецЦикла;
		
	Иначе
		ВыполнитьПоследовательнуюЗагрузкуЭлементов(МетаданныеИсточника, ТаблицаДанных, ТаблицаРеквизитов, ТребуетсяЗагрузкаДоОбъекта, ТекстСообщения, КоличествоЭлементов, Загружено, ТаблицаДанных)
	КонецЕсли;
	
	Для каждого СтрокаНастроек Из ТаблицыНастроек Цикл
		Если ЗначениеЗаполнено(СтрокаНастроек.ТипСтрокой) Тогда
			ТаблицаДанных.ЗаполнитьЗначения(Истина, ?(СтрокаНастроек.Значение="ТаблицаЗагружаемыхРеквизитов", "", СтрокаНастроек.Значение) + "ОбъектЗагрузкиНайден")
		КонецЕсли;
	КонецЦикла;	
	ЗначениеВРеквизитФормы(ТаблицаДанных, "ТаблицаПодготовленныхДанных");
...
&НаСервере
Процедура ВыполнитьПоследовательнуюЗагрузкуЭлементов(МетаданныеИсточника, КоллекцияЗагружаемыхСтрок, ТаблицаРеквизитов, ТребуетсяЗагрузкаДоОбъекта, ТекстСообщения, КоличествоЭлементов, Загружено, ТаблицаДанных)
	...
	ИначеЕсли ТекРежимЗагрузки = 5 И КоллекцияЗагружаемыхСтрок.Количество() Тогда
		
		ОбъектНайден = КоллекцияЗагружаемыхСтрок[0]["ОбъектЗагрузкиНайден"];
		Если ОбъектНайден Тогда
			ЗагружаемыйОбъект = КоллекцияЗагружаемыхСтрок[0]["ОбъектЗагрузки"].ПолучитьОбъект();
		Иначе				
			Если Метаданные.Документы.Содержит(МетаданныеИсточника) Тогда
				...
				ЗагружаемыйОбъект.УстановитьНовыйНомер();
			Иначе
				ЗагружаемыйОбъект = Справочники[МетаданныеИсточника.Имя].СоздатьЭлемент();
				ЗагружаемыйОбъект.УстановитьНовыйКод();
			КонецЕсли;
			ЗагружаемыйОбъект.УстановитьСсылкуНового(КоллекцияЗагружаемыхСтрок[0]["ОбъектЗагрузки"]);
	...
	Для каждого ЗагружаемаяСтрока Из КоллекцияЗагружаемыхСтрок Цикл
		...
		Для Каждого сСтрока из ТребуетсяЗагрузкаДоОбъекта Цикл
			...
			Если сСтрока.РежимЗагрузки = 6 ИЛИ (сСтрока.РежимЗагрузки = 3 И сСтрока.ЭтоРегистр) ИЛИ ЗагружаемаяСтрока[сСтрока.Значение+"ОбъектЗагрузкиНеЗаписывать"] Тогда
				Продолжить;
			КонецЕсли;
			сСтрока.ОбъектСсылка = ЗагрузитьВСправочник(ЗагружаемаяСтрока, сСтрока, , ТребуетсяЗагрузкаДоОбъекта, ТаблицаДанных)			
		...
		Если ТекРежимЗагрузки = 0 Тогда
			...
			Если НЕ ЗначениеЗаполнено(ЗагружаемаяСтрока["ОбъектЗагрузки"]) Тогда Продолжить КонецЕсли;
		...
		Если Не ОбъектНайден ИЛИ ЗамещатьСуществующиеЗаписи Тогда
			...
			Попытка
				Если ТекущаяСтрока.Свойство("Ссылка") Тогда
					ЗаполнитьЗначенияСвойств(ЗагружаемыйОбъект, ТекущаяСтрока, , "Ссылка");
				Иначе
		...
		Для каждого Строка Из ТЗ_ДопНастроек Цикл
			...
			Если ТекРежимЗагрузки = 0 Тогда
				...
				РезультатЗагрузкиСправочника = ЗагрузитьВСправочник(ЗагружаемаяСтрока, Строка, ЗагружаемыйОбъект.Ссылка, ТребуетсяЗагрузкаДоОбъекта, ТаблицаДанных);	
			...
			ИначеЕсли (ТекРежимЗагрузки = 1 ИЛИ ТекРежимЗагрузки = 3 ИЛИ ТекРежимЗагрузки = 5) И НЕ Строка.ЗагружаетсяДоОбъекта Тогда
				...
				ЗагрузитьДанныеВДполнительнуюТЧИлиРегистр(ЗагружаемыйОбъект, ЗагружаемаяСтрока, Строка, Строка.ЭтоРегистр, ТребуетсяЗагрузкаДоОбъекта)					
...
//
Функция ЗагрузитьВСправочник(ЗагружаемаяСтрока, СтрокаНастроек, Владелец = Неопределено, ТребуетсяЗагрузкаДоОбъекта, ТаблицаДанных)
	
	ТаблицаНастроек = СтрокаНастроек.Значение;
	
	Если НЕ ЗначениеЗаполнено(ЗагружаемаяСтрока[ТаблицаНастроек + "ОбъектЗагрузки"]) Тогда
		Возврат Неопределено
	КонецЕсли;
	
	МетаданныеИсточника = ПолучитьМетаданныеОбъектаПоКлючуНастройки(СтрокаНастроек.Значение);
	
	ТекущаяСтрока = Новый Структура();
	ЗаполнитьСтруктуруТекущейСтроки(ТекущаяСтрока, СтрокаНастроек.Значение, ЗагружаемаяСтрока, ТребуетсяЗагрузкаДоОбъекта);
...
//
Функция ЗагрузитьДанныеВДполнительнуюТЧИлиРегистр(ОбъектИсточника, ЗагружаемаяСтрока, СтрокаНастроек, ЭтоРегистр, ТребуетсяЗагрузкаДоОбъекта)
	
	ОбъектЗагрузки = СтрокаНастроек.ИмяОбъектаЗагрузки;
	ТаблицаНастроек = СтрокаНастроек.Значение;
		...
		Если Справочники.ТипВсеСсылки().СодержитТип(ТипКолонки) Тогда
			Для каждого Владелец Из Метаданные.НайтиПоТипу(ТипКолонки).Владельцы Цикл
				...
				Если ТипыВладельцев[ТипВладельца] = Неопределено Тогда
					...
					Если РежимЗагрузки = 0 Тогда
						Для каждого СтрокаНастроек Из ТаблицыНастроек Цикл	
							Если СтрокаНастроек.РежимЗагрузки = 3 И СтрокаНастроек.ЗагружаетсяДоОбъекта Тогда
...
Процедура ПолучитьТаблицуДанныхНаСервере()
	...
	Для Каждого Колонка Из ДополнительныеСвойства.Колонки Цикл Если НЕ Колонка.Значение.КолонкаОбработана Тогда ПолучитьКолонкуТаблицыДанных(ДополнительныеСвойства.Колонки, Колонка, ТаблицаПолучаемыхДанных, ТаблицаПостроитель) КонецЕсли КонецЦикла;	
	...
	Для каждого СтрокаНастроек Из ТаблицыНастроек Цикл	
		...
		Если МетаданныеИсточника <> Неопределено Тогда 
			Если Метаданные.Документы.Содержит(МетаданныеИсточника) ИЛИ Метаданные.Справочники.Содержит(МетаданныеИсточника) Тогда
				ПолучитьСсылкиОбъектовЗагрузкиТипаСправочник(ТаблицаПостроитель, ТаблицаПолучаемыхДанных, СтрокаНастроек);
			КонецЕсли;	
		КонецЕсли;
		Если СтрокаНастроек.РежимЗагрузки = 3 И СтрокаНастроек.ЗагружаетсяДоОбъекта Тогда
			ПолучитьСсылкиОбъектовЗагрузкиТипаСправочник(ТаблицаПостроитель, ТаблицаПолучаемыхДанных, СтрокаНастроек);
...
&НаСервере
Процедура ПолучитьКолонкуТаблицыДанных(Колонки, Колонка, Таблица, ТаблицаПостроитель)
			...
			Если ЗначениеЗаполнено(СвязьПоВладельцу) Тогда
				Если Найти(СвязьПоВладельцу, "ОбъектЗагрузки") Тогда
					...
					Если Не СтрокиПоиска.Количество() = 0 Тогда
						...
						КонецЦикла;
					КонецЕсли;
					ПолучитьСсылкиОбъектовЗагрузкиТипаСправочник(ТаблицаПостроитель, Таблица, ТаблицыНастроек[Колонка.Значение.ИндексТаблицыНастройки]);	
				Иначе
...
&НаСервере
Процедура ПолучитьСсылкиОбъектовЗагрузкиТипаСправочник(ТаблицаПостроитель, Таблица, СтрокаНастроек, НастройкаВызвавшаяЗаполнение = Неопределено)
	
	Если СтрокаНастроек.Обработана Тогда Возврат КонецЕсли;
	
	ИмяТЗ = СтрокаНастроек.Значение;
	ИмяОбъектаЗагрузки = СтрокаНастроек.ИмяОбъектаЗагрузки;
	Префикс = ?(СтрокаНастроек.Значение = "ТаблицаЗагружаемыхРеквизитов", "", СтрокаНастроек.Значение);
	ТекРежимЗагрузки = ?(СтрокаНастроек.РежимЗагрузки = 3, 2, СтрокаНастроек.РежимЗагрузки);
	...
	СтрокиКлючевыхЗначений = ЭтаФорма[ИмяТЗ].НайтиСтроки(Новый Структура("КлючевойРеквизитПоиска,Пометка",Истина,Истина));
	
	МенеджерСсылки = Неопределено;
	Если Метаданные.Документы.Содержит(МетаданныеИсточника) Тогда
		МенеджерОбъекта = Документы[МетаданныеИсточника.Имя];
		РеквизитСсылка = "Ссылка";
		ИмяДляЗапроса = "Документ";
		МенеджерСсылки = Документы[МетаданныеИсточника.Имя];
	ИначеЕсли Метаданные.Справочники.Содержит(МетаданныеИсточника) И ТекРежимЗагрузки = 6 Тогда
		МенеджерОбъекта = Справочники[МетаданныеИсточника.Имя];
	    РеквизитСсылка = ЭтаФорма[ИмяТЗ + "ОбъектЗагрузки"];
		ИмяДляЗапроса = "Справочник";
		Строки = ЭтаФорма[ИмяТЗ].НайтиСтроки(Новый Структура("ИмяРеквизита", РеквизитСсылка));
		Если Строки.Количество() Тогда
			Ссылка = Строки[0].ОписаниеТипов.ПривестиЗначение();
			Если Справочники.ТипВсеСсылки().СодержитТип(Строки[0].ОписаниеТипов.Типы()[0]) Тогда
				ИмяОбъекта = Ссылка.Метаданные().Имя;
				МенеджерСсылки = Справочники[ИмяОбъекта];
			ИначеЕсли Документы.ТипВсеСсылки().СодержитТип(Строки[0].ОписаниеТипов.Типы()[0]) Тогда
				ИмяОбъекта = Ссылка.Метаданные().Имя;
				МенеджерСсылки = Документы[ИмяОбъекта];	
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли Метаданные.Справочники.Содержит(МетаданныеИсточника) Тогда
		...
		ИмяДляЗапроса = "Справочник";
		РеквизитСсылка = "Ссылка";
		МенеджерСсылки = Справочники[МетаданныеИсточника.Имя];
	ИначеЕсли Метаданные.РегистрыСведений.Содержит(МетаданныеИсточника) Тогда
		МенеджерОбъекта = РегистрыСведений[МетаданныеИсточника.Имя];
	...
	МассивСтроковыхПолейПоиска = Новый Массив;
	...
	Если Не СтрокиПоиска.Количество() = 0 Тогда
		...
		ТекстЗапроса =
		"Выбрать Различные
		|Справочник." + РеквизитСсылка + " КАК ОбъектЗагрузки";
		
		КолонкиПоиска = "";
		КолонкиПоискаБезПрефикса = Новый СписокЗначений;
		ТекстЗапросаРеквизиты = "";
		ТекстЗапросаПараметры = "";
		
		Для каждого СтрокаПоиска Из СтрокиПоиска Цикл
					
			Если СтрокаПоиска.ОписаниеТипов.Типы()[0] = Тип("Строка") Тогда
				
				МассивСтроковыхПолейПоиска.Добавить(СтрокаПоиска.ИмяРеквизита);
				ТекстЗапросаРеквизиты = ТекстЗапросаРеквизиты +",
					|Представление(Справочник." + СтрокаПоиска.ИмяРеквизита + ") КАК " + Префикс + СтрокаПоиска.ИмяРеквизита;
			...
			НаименованиеКолонки = Префикс + СтрокаПоиска.ИмяРеквизита;
			КолонкиПоиска = КолонкиПоиска + "," + НаименованиеКолонки;
			
			КолонкиПоискаБезПрефикса.Добавить(СтрокаПоиска.ИмяРеквизита);
			ТекущаяКолонкаПоиск = ДополнительныеСвойства.Колонки[НаименованиеКолонки];
			ИмяДополнительнойЗагрузки = ТекущаяКолонкаПоиск.НастройкаЗагрузки;			
			Если НЕ ТекущаяКолонкаПоиск.КолонкаОбработана И НЕ ЗначениеЗаполнено(ТекущаяКолонкаПоиск.НастройкаЗагрузки) Тогда
				ПолучитьКолонкуТаблицыДанных(ДополнительныеСвойства.Колонки, ТекущаяКолонкаПоиск, Таблица, ТаблицаПостроитель);	
			ИначеЕсли ЗначениеЗаполнено(ИмяДополнительнойЗагрузки) Тогда
				МассивСтрокНастроек = ТаблицыНастроек.НайтиСтроки(Новый Структура("Значение", ИмяДополнительнойЗагрузки));
				СтрокаНастройкиПоиска = МассивСтрокНастроек[0];
				ПолучитьСсылкиОбъектовЗагрузкиТипаСправочник(ТаблицаПостроитель, Таблица, СтрокаНастройкиПоиска);
				Таблица.ЗагрузитьКолонку(Таблица.ВыгрузитьКолонку(?(ИмяДополнительнойЗагрузки = "ТаблицаЗагружаемыхРеквизитов", "", ИмяДополнительнойЗагрузки)+"ОбъектЗагрузки"), НаименованиеКолонки);	
			КонецЕсли;
		...
		МассивИдентичныхНастроек = ПолучитьМассивИдентичныхНастроекПоискаОбъектов(СтрокаНастроек, КолонкиПоискаБезПрефикса);
		...
		ТекстЗапроса = ТекстЗапроса +"
		|Из " + ИмяДляЗапроса + "." + МетаданныеИсточника.Имя+" КАК Справочник
		|Где";
		...
		Для каждого Элемент Из ТаблицаРезультатов Цикл 		
			Для каждого ЭлементПоиска Из МассивСтроковыхПолейПоиска Цикл
				Элемент[Префикс + ЭлементПоиска] = СокрЛП(ВРег(Элемент[Префикс + ЭлементПоиска]));		
			КонецЦикла;	
		КонецЦикла;
		
		КолонкиПоиска = Прав(КолонкиПоиска, СтрДлина(КолонкиПоиска)-1);
		СтруктураПоиска = Новый Структура(КолонкиПоиска);
		СтруктураПоискаПоТаблицеДанных = Новый Структура(КолонкиПоиска);
		
		Для каждого Строка Из Таблица Цикл
			
			Если ЗначениеЗаполнено(Строка[Префикс + "ОбъектЗагрузки"]) Тогда Продолжить КонецЕсли;			
			ЗаполнитьЗначенияСвойств(СтруктураПоиска, Строка);
			ЗаполнитьЗначенияСвойств(СтруктураПоискаПоТаблицеДанных, Строка);
			Для каждого ЭлементПоиска Из МассивСтроковыхПолейПоиска Цикл
				СтруктураПоиска[Префикс + ЭлементПоиска] = СокрЛП(ВРег(СтруктураПоиска[Префикс + ЭлементПоиска]));
				СтруктураПоискаПоТаблицеДанных[Префикс + ЭлементПоиска] = СтруктураПоискаПоТаблицеДанных[Префикс + ЭлементПоиска];
			КонецЦикла;
			МассивСтрок = ТаблицаРезультатов.НайтиСтроки(СтруктураПоиска);
			Если МассивСтрок.Количество() Тогда
				Строка[Префикс + "ОбъектЗагрузки"] = МассивСтрок[0].ОбъектЗагрузки;
				Строка[Префикс + "ОбъектЗагрузкиНайден"] = Истина;
			ИначеЕсли ТекРежимЗагрузки = 3 ИЛИ ТекРежимЗагрузки = 6 Тогда
				Строка[Префикс + "ОбъектЗагрузкиНеСоздан"] = Истина;	
			ИначеЕсли МенеджерСсылки <> Неопределено Тогда
				КоличествоПустыхКлючевыхПолей = 0;
				Для каждого КлючеваяСтрока Из СтрокиКлючевыхЗначений Цикл
					Если НЕ ЗначениеЗаполнено(СтруктураПоиска[Префикс + КлючеваяСтрока.ИмяРеквизита]) Тогда КоличествоПустыхКлючевыхПолей = КоличествоПустыхКлючевыхПолей + 1 КонецЕсли	
				КонецЦикла;
				Если КоличествоПустыхКлючевыхПолей И КоличествоПустыхКлючевыхПолей = СтрокиКлючевыхЗначений.Количество() Тогда Строка[Префикс + "ОбъектЗагрузкиНеСоздан"] = Истина; Продолжить КонецЕсли;
				Строка[Префикс + "ОбъектЗагрузки"] = МенеджерСсылки.ПолучитьСсылку(Новый УникальныйИдентификатор);
			КонецЕсли;
			
			Если ЗначениеЗаполнено(Строка[Префикс + "ОбъектЗагрузки"]) Тогда 
				МассивСтрок = Таблица.НайтиСтроки(СтруктураПоискаПоТаблицеДанных);
				Для каждого НайденнаяСтрока Из МассивСтрок Цикл
					Если НайденнаяСтрока = Строка Тогда Продолжить КонецЕсли;
					НайденнаяСтрока[Префикс + "ОбъектЗагрузки"] = Строка[Префикс + "ОбъектЗагрузки"];
					НайденнаяСтрока[Префикс + "ОбъектЗагрузкиНайден"] = Строка[Префикс + "ОбъектЗагрузкиНайден"];
					НайденнаяСтрока[Префикс + "ОбъектЗагрузкиНеЗаписывать"] = Истина;
				КонецЦикла;
							
				Для каждого СтруктураИдентичнойНастройки Из МассивИдентичныхНастроек Цикл
					Если СтруктураИдентичнойНастройки.СтрокаИдентичнойНастройки = НастройкаВызвавшаяЗаполнение Тогда Продолжить КонецЕсли;
					Если НЕ СтруктураИдентичнойНастройки.СтрокаИдентичнойНастройки.Обработана Тогда
						ПолучитьСсылкиОбъектовЗагрузкиТипаСправочник(ТаблицаПостроитель, Таблица, СтруктураИдентичнойНастройки.СтрокаИдентичнойНастройки, СтрокаНастроек);	
					КонецЕсли;
					СтруктураПоискаИН = Новый Структура(СтруктураИдентичнойНастройки.КолонкиПоиска);
					Для каждого КлючИЗначение Из СтруктураПоиска Цикл
						Ключ = ?(ЗначениеЗаполнено(Префикс), СтрЗаменить(КлючИЗначение.Ключ, Префикс, СтруктураИдентичнойНастройки.Префикс), СтруктураИдентичнойНастройки.Префикс + КлючИЗначение.Ключ);
						СтруктураПоискаИН[Ключ] = КлючИЗначение.Значение;		
					КонецЦикла;	
					МассивСтрок = Таблица.НайтиСтроки(СтруктураПоискаИН);
					Для каждого НайденнаяСтрока Из МассивСтрок Цикл
						НайденнаяСтрока[СтруктураИдентичнойНастройки.Префикс + "ОбъектЗагрузки"] = Строка[Префикс + "ОбъектЗагрузки"];
						НайденнаяСтрока[СтруктураИдентичнойНастройки.Префикс + "ОбъектЗагрузкиНайден"] = Строка[Префикс + "ОбъектЗагрузкиНайден"];
						НайденнаяСтрока[СтруктураИдентичнойНастройки.Префикс + "ОбъектЗагрузкиНеЗаписывать"] = Истина;
					КонецЦикла;
				КонецЦикла;
	...
	Иначе
		...
		Если ТекРежимЗагрузки = 3 И ТекРежимЗагрузки = 6 Тогда
			Для каждого Строка Из Таблица Цикл		
				Строка[Префикс + "ОбъектЗагрузкиНеСоздан"] = Истина;		
			КонецЦикла;
		ИначеЕсли МенеджерСсылки <> Неопределено Тогда
	...
	СтрокаНастроек.Обработана = Истина;
...
&НаСервере
Функция ПолучитьМассивИдентичныхНастроекПоискаОбъектов(СтрокаНастрокиЗагрузки, КолонкиПоискаНастройкиЗагрузки)
	
	МассивИдентичныхНастроек = Новый Массив;
	
	МассивСтрокНастроек = ТаблицыНастроек.НайтиСтроки(Новый Структура("ТипСтрокой", СтрокаНастрокиЗагрузки.ТипСтрокой));
	
	Для каждого СтрокаНастройки Из МассивСтрокНастроек Цикл
		Если СтрокаНастрокиЗагрузки = СтрокаНастройки Тогда Продолжить КонецЕсли;
		
		Префикс = ?(СтрокаНастройки.Значение = "ТаблицаЗагружаемыхРеквизитов", "", СтрокаНастройки.Значение);
		СтрокиПоиска = ЭтаФорма[СтрокаНастройки.Значение].НайтиСтроки(Новый Структура("ПолеПоиска,Пометка",Истина,Истина));
		КолонкиПоиска = "";
		КолонкиПоискаБезПрефикса = Новый СписокЗначений;
		Для каждого СтрокаПоиска Из СтрокиПоиска Цикл
			    НаименованиеКолонки = Префикс + СтрокаПоиска.ИмяРеквизита;
		...
		КолонкиПоискаБезПрефикса.СортироватьПоЗначению();
		КолонкиПоискаНастройкиЗагрузки.СортироватьПоЗначению();
		
		Если ЗначениеВСтрокуВнутр(КолонкиПоискаНастройкиЗагрузки.ВыгрузитьЗначения()) = ЗначениеВСтрокуВнутр(КолонкиПоискаБезПрефикса.ВыгрузитьЗначения()) Тогда
			МассивИдентичныхНастроек.Добавить(Новый Структура("Префикс, КолонкиПоиска, СтрокаИдентичнойНастройки", Префикс, Прав(КолонкиПоиска, СтрДлина(КолонкиПоиска)-1), СтрокаНастройки));
		КонецЕсли;
	КонецЦикла;
	
	Возврат МассивИдентичныхНастроек
	...
	Для каждого Колонка Из ВосстановленнаяТаблицаРеквизитов Цикл
		...
		Если Не ЗагружаемыйРеквизит = Неопределено Тогда
			...
			Если Индекс >= НомерКолонкиОформления Тогда
				ЗаполнитьЗначенияСвойств(ЗагружаемыйРеквизит, Колонка, , "ДоступныеТипы");
...
//
Процедура ЗаполнитьНастройкиКолонокТабличнойЧасти(ТЗ, КлючСтраницы)
	
	МетаданныеИсточника = ПолучитьМетаданныеОбъектаПоКлючуНастройки(КлючСтраницы);
	...
	Для каждого Реквизит Из МетаданныеИсточника.СтандартныеРеквизиты Цикл
		...
		Если Реквизит.Имя = "Ссылка" ИЛИ Реквизит.Имя = "ПометкаУдаления" Тогда
			Продолжить;	
		КонецЕсли;
...
//
Процедура ЗаполнитьНастройкиКолонокСправочника(ТЗ, КлючСтраницы)
...
//
Процедура ЗаполнитьНастройкиКолонокРегистраСведений(ТЗ, КлючСтраницы)
	...
	ОбработатьТаблицуЗагружаемыхРеквизитов(ТЗ, ТекРежимЗагрузки);
...
//
Процедура ЗаполнитьНастройкиКолонокРегистраДокумента(ТаблицаЗагружаемыхРеквизитов, КлючСтраницы)
	...
	ДвиженияРегистра = ПолучитьИзВременногоХранилища(АдресХранилищаДвиженияРегистра);
	
	ИсключаемыеРеквизиты = Новый Структура("МоментВремени, НомерСтроки, Регистратор");
	
	Для каждого Колонка Из ДвиженияРегистра.Колонки Цикл
		
		Если ИсключаемыеРеквизиты.Свойство(Колонка.Имя) Тогда Продолжить КонецЕсли;
		
		ЗагружаемыйРеквизит                        = ТаблицаЗагружаемыхРеквизитов.Добавить();
		ЗагружаемыйРеквизит.ИмяРеквизита           = Колонка.Имя;
		ЗагружаемыйРеквизит.ПредставлениеРеквизита = Колонка.Заголовок;
		ЗагружаемыйРеквизит.ОписаниеТипов 		   = Новый ОписаниеТипов(Колонка.ТипЗначения, , "Null");
...
//
Процедура ОбработатьТаблицуЗагружаемыхРеквизитов(ТаблицаЗагружаемыхРеквизитов, ТекРежимЗагрузки = Неопределено)
	...
	Для каждого ЗагружаемыйРеквизит Из ТаблицаЗагружаемыхРеквизитов Цикл
		...
		ЗагружаемыйРеквизит.Идентификатор = Новый УникальныйИдентификатор;
...
//
Процедура УстновитьПредставлениеДопУсловийТЗ(ТЗ, ПометкаИспользованияНастройки = Истина)
	...
	Для каждого ЗагружаемыйРеквизит Из ТЗ Цикл
		...
		Иначе
			...
			+?(ПустаяСтрока(ЗагружаемыйРеквизит.СвязьПоТипу), "", " по типу "+ЗагружаемыйРеквизит.СвязьПоТипу);
		КонецЕсли;
...
//
Процедура ЗаполнитьВТаблицуЗначенийДвиженияРегистраДокумента(КлючСтраницы, ОбъектЗагрузки = "") Экспорт
	...
	Если Метаданные.РегистрыБухгалтерии.Содержит(МетаданныеИсточника) Тогда
		...
		|	* 
		|ИЗ
		|	РегистрБухгалтерии."+МетаданныеИсточника.Имя+".ДвиженияССубконто(, , Регистратор = &Регистратор, , ) КАК ЖурналПроводок";
	...
	ИначеЕсли Метаданные.РегистрыСведений.Содержит(МетаданныеИсточника) Тогда
		...
		ТабПоле = РегистрыСведений[МетаданныеИсточника.Имя].СоздатьНаборЗаписей()
		
	ИначеЕсли Метаданные.РегистрыНакопления.Содержит(МетаданныеИсточника) Тогда
		
		ТабПоле = РегистрыНакопления[МетаданныеИсточника.Имя].СоздатьНаборЗаписей()
		
	ИначеЕсли Метаданные.РегистрыРасчета.Содержит(МетаданныеИсточника) Тогда
		
		ТабПоле = РегистрыРасчета[МетаданныеИсточника.Имя].СоздатьНаборЗаписей()
		
	КонецЕсли;
	
	Если НЕ ЭтоРегистрБухгалтерии Тогда
		ТабПоле.Отбор.Регистратор.Значение = ?(РежимЗагрузки = 3, СсылкаДокумента, СсылкаИсточника);
		ТабПоле.Прочитать();
		ДвиженияРегистра = ТабПоле.Выгрузить();
	КонецЕсли;
	
	Если КлючСтраницы = "ТаблицаЗагружаемыхРеквизитов" ИЛИ РежимЗагрузки = 5 Тогда
		АдресХранилищаДвиженияРегистра = ПоместитьВоВременноеХранилище(ДвиженияРегистра, Новый УникальныйИдентификатор)
	КонецЕсли;
	
	//ЗаполнитьНастройкамиСозданнуюСтраницу(КлючСтраницы, Истина);
...
Процедура ОбновитьДанныеТабличногоДокументаСервер()
	...
	Если РежимЗагрузки = 0 или РежимЗагрузки = 2 или РежимЗагрузки = 3 или РежимЗагрузки = 4 или РежимЗагрузки = 5 или НетВыбранныхМетаданных() Тогда
		Возврат
	КонецЕсли;
...
Функция мПрочитатьТабличныйДокументИзExcel(ИмяФайла, НомерЛистаExcel = 1) Экспорт
	...
	Если НЕ ПредварительныйПросмотрТабличногоДокументаПослеОткрытия И ПерваяСтрокаДанныхТабличногоДокумента > ВсегоСтрок Тогда Возврат Ложь КонецЕсли;
...
Процедура ЗаполнитьТабличныйДокументИзМассива(МассивДанныхКолонок, ВсегоСтрок, ВсегоКолонок)
	...
	Если ПредварительныйПросмотрТабличногоДокументаПослеОткрытия Тогда
		Область = ТабДокумент.ПолучитьОбласть("R2C1:R" + Формат(ТабДокумент.ВысотаТаблицы, "ЧГ=0") + "C" + Формат(ТабДокумент.ШиринаСтраницы, "ЧГ=0"));	
...
Функция мПрочитатьТабличныйДокументИзDBF(ИмяФайла) Экспорт
	...
	Пока Не XBase.ВКонце() Цикл
		...
		Для каждого Поле Из XBase.поля Цикл
			ТекущаяКолонка = ТекущаяКолонка + 1;
			ТабличныйДокумент.Область("R" + Формат(ТекущаяСтрока, "ЧГ=") +"C" + Формат(ТекущаяКолонка, "ЧГ=")).Текст = СокрЛП(XBase.ПолучитьЗначениеПоля(ТекущаяКолонка - 1));
		КонецЦикла;
...
Процедура УстановитьВидимостьКомандНаПанелях()
	...
	ДоступностьКнопкиСохранитьЗначения = НЕ НетВыбранныхМетаданных();
	...
	Для каждого КлючИЗначение Из ТаблицыНастроек Цикл
		...
		Элементы[КлючИЗначение.Значение + "КлючевойРеквизитПоиска"].Видимость = Элементы[КлючИЗначение.Значение + "ПолеПоиска"].Видимость;
...
Процедура ПолучитьТаблицуДанных(Команда)
	...
	Для каждого Строка Из ТаблицыНастроек Цикл Строка.Обработана = Ложь КонецЦикла;
...
Процедура ДобавитьНастройку(Команда)
	...
	Если Элементы.ТипОбъекта.СписокВыбора.Количество() Тогда
		...
		Форма.Элементы.ВидДобавляемойНастройки.СписокВыбора.Добавить(6, "Поиск через справочник");
		Для каждого ЭлементСпискаВыбора Из Элементы.ТипОбъекта.СписокВыбора Цикл
			...
			Форма.Элементы.Справочник1.СписокВыбора.Добавить(ЭлементСпискаВыбора.Значение, ЭлементСпискаВыбора.Представление);
		КонецЦикла;
	КонецЕсли;

	// Добавляем табличные части
	Если РежимЗагрузки = 5 И СписокДобавляемыхСтраниц.Количество() Тогда
	...
	Если Элементы.ТипОбъекта.СписокВыбора.Количество() Тогда
		Форма.Элементы.ВидДобавляемойНастройки.СписокВыбора.Добавить(3, "Поиск через регистр");
		//Форма.Элементы.ВидДобавляемойНастройки.СписокВыбора.Добавить(7, "Регистр сведений");
		Для каждого ЭлементСпискаВыбора Из Элементы.ИмяВидаРегистра.СписокВыбора Цикл
			Форма.Элементы.РегистрСведений.СписокВыбора.Добавить(ЭлементСпискаВыбора.Значение, ЭлементСпискаВыбора.Представление);
			//Форма.Элементы.РегистрСведений1.СписокВыбора.Добавить(ЭлементСпискаВыбора.Значение, ЭлементСпискаВыбора.Представление);
		КонецЦикла;
	КонецЕсли;
...
Процедура УдалитьТаблицуНастройки(Команда)
	...
	Если МассивСтрок.Количество() Тогда
		Если (МассивСтрок[0].РежимЗагрузки = 3 ИЛИ МассивСтрок[0].РежимЗагрузки = 6) И МассивСтрок[0].ЗагружаетсяДоОбъекта Тогда
			УдалитьРеквизитФормы(МассивСтрок[0].Значение + "ОбъектЗагрузки");	
		КонецЕсли;
		УдалитьЭлементФормы(МассивСтрок[0].Ключ);	
		УдалитьРеквизитФормы(МассивСтрок[0].Значение);
...
&НаКлиенте
Процедура СсылкаИсточникаСписокВыбораПриИзменении(Элемент)
			
	ТипСсылки = Новый ОписаниеТипов(СсылкаИсточникаСписокВыбора);
	СсылкаИсточника = ТипСсылки.ПривестиЗначение("");
...
Процедура СсылкаДокументаПриИзмененииНаСервере()
	...
	Если РежимЗагрузки = 3 Тогда
		Если ПрошлыйМетаданныеСсылкиИсточника = Неопределено или СсылкаДокумента = Неопределено или Не ПрошлыйМетаданныеСсылкиИсточника = СсылкаДокумента.Метаданные() Тогда
			...
			УстановитьСписокРегистров();
			Если ЗначениеЗаполнено(РегистрДокумента) Тогда
				УстановитьИсточник();
				ЗаполнитьВТаблицуЗначенийДвиженияРегистраДокумента("ТаблицаЗагружаемыхРеквизитов");
			КонецЕсли;
		КонецЕсли;
...
Функция ПолучитьИмяФормыСсылкиИсточника(СсылкаИсточника)
	...
	Если Документы.ТипВсеСсылки().СодержитТип(ТипЗнч(СсылкаИсточника)) Тогда
		Возврат "Документ." + СсылкаИсточника.Метаданные().Имя + ".ФормаСписка"
	ИначеЕсли Справочники.ТипВсеСсылки().СодержитТип(ТипЗнч(СсылкаИсточника)) Тогда
...
Процедура ТаблицаЗагружаемыхРеквизитовЗначениеПоУмолчаниюНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Для каждого Строка Из ТаблицыНастроек Цикл
		Если НЕ ЗначениеЗаполнено(Строка.ТипСтрокой) 
		 ИЛИ Строка = СтруктураДанныхСтроки.СтрокаНастроек 
		 ИЛИ (Строка.ДопНастройки И (Строка.РежимЗагрузки = 5 ИЛИ Строка.РежимЗагрузки = 1)) Тогда 
			Продолжить
		КонецЕсли;
		Если ТаблицыНастроек.Индекс(Строка) = 0 Тогда
			...
			КонецЕсли;		
		ИначеЕсли (Строка.РежимЗагрузки = 3 ИЛИ Строка.РежимЗагрузки = 6) И Строка.ЗагружаетсяДоОбъекта Тогда
			Если ТекДанные.ОписаниеТипов.СодержитТип(Тип(Строка.ТипСтрокой)) Тогда
				СписокВыбора.Добавить(Строка.Значение, Строка.Заголовок)
			КонецЕсли	
		Иначе
			Если Строка.РежимЗагрузки = 0 И ТекДанные.ОписаниеТипов.СодержитТип(Тип("СправочникСсылка."+Строка.ИмяОбъектаЗагрузки)) Тогда
				СписокВыбора.Добавить(Строка.Значение, Строка.Заголовок)
			КонецЕсли
		КонецЕсли;	
	КонецЦикла;
...
&НаКлиенте
Процедура ТаблицаЗагружаемыхРеквизитовПоискЧерезНастройку(Элемент)
	
	СтруктураДанныхСтроки = мПолучитьТекущиеДанныеСтраницыНастройки();
	
	ТаблицаЗаполненныхНастроек = ЭтаФорма[СтруктураДанныхСтроки.СтрокаНастроек.Значение];
	Строки = ТаблицаЗаполненныхНастроек.НайтиСтроки(Новый Структура("ИмяРеквизита", ЭтаФорма[СтруктураДанныхСтроки.СтрокаНастроек.Значение + "ОбъектЗагрузки"]));		
	Если Строки.Количество() Тогда
		СтруктураДанныхСтроки.СтрокаНастроек.ТипСтрокой = ПолучитьТипСтрокой(Строки[0].ОписаниеТипов.Типы()[0]);			
	Иначе
		ЭтаФорма[СтруктураДанныхСтроки.СтрокаНастроек.Значение + "ОбъектЗагрузки"] = "";		
	КонецЕсли;
...
Процедура ТаблицаЗагружаемыхРеквизитовНомерКолонкиПриИзменении(Элемент)
	...
	ИзменитьНомерКолонкиВДеревеПростыхНастроек(ТекДанные.Идентификатор, ТекДанные.НомерКолонки);
...
&НаКлиенте
Процедура ОткрытьОписаниеИзменений(Команда)
...
Процедура ОткрытьСтраничкуАвтора(Команда)
	...
	Приложение = "http://it-titan.ru/";
	
	Если ВерсияПлатформы83 Тогда
		НачатьЗапускПриложения(Новый ОписаниеОповещения("ЗапускПриложенияЗавершение", ЭтаФорма), Приложение);	
	Иначе
...
Процедура ОткрытьСтраничкуРазработки(Команда)
	...
	Приложение = "http://infostart.ru/public/284674/";
...
Процедура НачатьПроверкуВерсий(ЭтоПроверкаПриОткрытии = Ложь)
	...
	ТребуетсяОбновление = ВыполнитьСравнениеВерсий(ВерсияНаСейте, ИнформацияОбОбработке.ТекущаяВерсия);
	
	Если ТребуетсяОбновление Тогда
		
		ТекстВопроса = "Требуется обновление!.
						|Текущая версия: "+ ИнформацияОбОбработке.ТекущаяВерсия + " 
...
Функция ВыполнитьСравнениеВерсий(ПроверяемаяВерсия, ТекущаяВерсия)
	...
	Для х = 0 ПО КоличествоРазрядовТекущее Цикл
		...
		Если Число(МассивТекущий[х]) > Число(МассивНаСайте[х]) Тогда		
...
&НаКлиенте
Процедура ЗапускПриложенияЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
//
Функция ЗарегистрироватьОбработкуВИнформационнойБазеНаСервере(ТекущаяВерсия, РежимИспользования = "", СсылкаНаЭлемент = Неопределено, АдресДвоичныхДанных = Неопределено, ВторойВариантРегистрации = Ложь, ИмяФайлаОбработки)
	...
	РегистрационныеДанные = ПолучитьСведенияОВнешнейОбработке(ТекущаяВерсия, ВторойВариантРегистрации);
...
&НаСервереБезКонтекста
Функция ПолучитьСведенияОВнешнейОбработке(ТекущаяВерсия, ВторойВариантРегистрации = Ложь, ПараметрыРегистрации = Неопределено) Экспорт
	
	Выполнить("ПараметрыРегистрации = ДополнительныеОтчетыИОбработки.СведенияОВнешнейОбработке(""2.1.3.1"")");	
	
	ПараметрыРегистрации.Версия = ТекущаяВерсия;
...
Процедура ЗарегистрироватьвИБПослеПомещенияВХранилище(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт 
	...
	Результат = ЗарегистрироватьОбработкуВИнформационнойБазеНаСервере(ИнформацияОбОбработке.ТекущаяВерсия, "Поиск", ДополнительнаяОбработкаСсылка, Адрес, Ложь, ВыбранноеИмяФайла);
	...
	ДополнительнаяОбработкаСсылка = Неопределено;
	Результат = ЗарегистрироватьОбработкуВИнформационнойБазеНаСервере(ИнформацияОбОбработке.ТекущаяВерсия, "Поиск", ДополнительнаяОбработкаСсылка, Адрес, Истина, ВыбранноеИмяФайла);
	Если ТипЗнч(Результат) = Тип("Строка") Тогда
		ТекстВопроса = НСтр("ru = 'В информационной базе зарегистрирована обработка под именем - ""%1"".
					|Обновить существующую?'");
		ТекстВопроса = СтрЗаменить(ТекстВопроса, "%1", Результат);
...
Процедура ВопросРегистрацииОбработкиВ_ИБ(РезультатВопроса, ДополнительныеПараметры) Экспорт	
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда 
		ОчиститьСообщения();
		Результат = ЗарегистрироватьОбработкуВИнформационнойБазеНаСервере(ИнформацияОбОбработке.ТекущаяВерсия, "Обновить", ДополнительнаяОбработкаСсылка, АдресВоВременномХранилище, ВторойВариантРегистрированияОбработки, ИмяФайлаОбработки);
	КонецЕсли;
...
Функция ДобавитьСтраницуНастроек(ЗагружаемыйОбъект, Представление, ЭтоДопСтраница = Ложь, ЭтоРегистр = Ложь, ЗагружаетсяДоОбъекта = Ложь, ТекРежимЗагрузки = Неопределено)
	...
	ВозвращаемыеДанные = Новый Структура("КлючСтраницы, ПредставлениеСтраницы");
	...
	Если ЭтоДопСтраница ИЛИ ЗагружаетсяДоОбъекта Тогда
		МетаданныеОбъекта = ПолучитьМетаданныеОбъектаЗагрузки(ЗагружаемыйОбъект, ЭтоРегистр, ТекРежимЗагрузки);
		НаименованиеСтраницы = "ДополнительнаяНастройка_" + (КоличествоСозданныхПодчиненныхНастроек);
	...
	Если СтруктураМетаданныхИНастроек = Неопределено Тогда
		СтруктураМетаданныхИНастроек = Новый Структура;
	КонецЕсли;
	СтруктураМетаданныхИНастроек.Вставить(НаименованиеСтраницы, МетаданныеОбъекта);
	
	Если ЗначениеЗаполнено(МетаданныеОбъекта) Тогда Выполнить(МетаданныеОбъекта) Иначе МетаданныеОбъекта = Неопределено КонецЕсли;
	...
	ВозвращаемыеДанные.КлючСтраницы = НаименованиеСтраницы;
	ВозвращаемыеДанные.ПредставлениеСтраницы = ПредставлениеСтраницы;
	...
	// Если настройка для поиска через справочник или регистр добавляем объект для выбора реквизата который надо получить
	Если (ТекРежимЗагрузки = 3 ИЛИ ТекРежимЗагрузки = 6) И ЗагружаетсяДоОбъекта Тогда
		нРеквизит = Новый РеквизитФормы(НаименованиеСтраницы + "ОбъектЗагрузки", Новый ОписаниеТипов("Строка"),,"Получаемый объект");
		МассивДобавляемыхРеквизитов.Добавить(нРеквизит);	
	КонецЕсли;
	ИзменитьРеквизиты(МассивДобавляемыхРеквизитов);
	...
	//
	Если (ТекРежимЗагрузки = 3 ИЛИ ТекРежимЗагрузки = 6) И ЗагружаетсяДоОбъекта Тогда
		НовыйЭлемент = Элементы.Добавить(НаименованиеСтраницы + "ОбъектЗагрузки", Тип("ПолеФормы"), СтраницаГруппыРеквизитов);
		НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
		НовыйЭлемент.ПутьКДанным = НаименованиеСтраницы + "ОбъектЗагрузки";
		НовыйЭлемент.КнопкаВыпадающегоСписка = Истина;
		НовыйЭлемент.УстановитьДействие("ПриИзменении", "ТаблицаЗагружаемыхРеквизитовПоискЧерезНастройку");
	КонецЕсли;
	...
	НовыйЭлемент = Элементы.Добавить(НаименованиеСтраницы + "КлючевойРеквизитПоиска", Тип("ПолеФормы"), ЭлементТаблицаФормы);
	НовыйЭлемент.Вид = ВидПоляФормы.ПолеФлажка;
	НовыйЭлемент.ПутьКДанным = НаименованиеСтраницы + "." + "КлючевойРеквизитПоиска";
	НовыйЭлемент.Заголовок = "Ключевой";
	...
	НовыйЭлемент.УстановитьДействие("ПриИзменении", "ТаблицаЗагружаемыхРеквизитовНомерКолонкиПриИзменении");
...
&НаСервере
Процедура ПолучитьНастройкиДляСтраницыЗагрузки(СтруктураСтраницы, ВидДобавляемойНастройки, ПометкаИспользованияНастройки = Истина)
	
	ТаблицаНастройки = РеквизитФормыВЗначение(СтруктураСтраницы.КлючСтраницы);
	
	Если ВидДобавляемойНастройки = 0 ИЛИ ВидДобавляемойНастройки = 6 Тогда
		ЗаполнитьНастройкиКолонокСправочника(ТаблицаНастройки, СтруктураСтраницы.КлючСтраницы);	
	ИначеЕсли ВидДобавляемойНастройки = 1 Тогда
		ЗаполнитьНастройкиКолонокТабличнойЧасти(ТаблицаНастройки, СтруктураСтраницы.КлючСтраницы);
	ИначеЕсли ВидДобавляемойНастройки = 2 ИЛИ ВидДобавляемойНастройки = 5 Тогда
		ЗаполнитьВТаблицуЗначенийДвиженияРегистраДокумента(СтруктураСтраницы.КлючСтраницы);
		ЗаполнитьНастройкиКолонокРегистраДокумента(ТаблицаНастройки, СтруктураСтраницы.КлючСтраницы);	
	ИначеЕсли ВидДобавляемойНастройки = 3 ИЛИ ВидДобавляемойНастройки = 4 Тогда
		ЗаполнитьНастройкиКолонокРегистраСведений(ТаблицаНастройки, СтруктураСтраницы.КлючСтраницы);	
	КонецЕсли;
	
	Если ВидДобавляемойНастройки = 3 ИЛИ ВидДобавляемойНастройки = 6 Тогда
		Для каждого Строка Из ТаблицаНастройки Цикл
			Элементы[СтруктураСтраницы.КлючСтраницы + "ОбъектЗагрузки"].СписокВыбора.Добавить(Строка.ИмяРеквизита);
		КонецЦикла;
	КонецЕсли;
	
	УстновитьПредставлениеДопУсловийТЗ(ТаблицаНастройки, ПометкаИспользованияНастройки);	
	ЗначениеВРеквизитФормы(ТаблицаНастройки, СтруктураСтраницы.КлючСтраницы);
	
	// Добавляем простую настройку
	ДобавитьНастройкиВДеревоПростыхНастроек(СтруктураСтраницы, ТаблицаНастройки);
	
КонецПроцедуры

&НаСервере
// Устарела. Процедура заполняет настройками дополнительно созданную страницу настройки
//
...
//	ЭтоРегистр - Определяет, что добавляемая страница, является настройкой для загрузки в один из регистров (булево) 
//
Процедура ЗаполнитьНастройкамиСозданнуюСтраницу(НаименованиеСтраницы = "ТаблицаЗагружаемыхРеквизитов", ЭтоРегистр = Ложь, ЗагружаетсяДоОбъекта = Ложь, ПометкаИспользованияНастройки = Истина)
	...
	Если (ТекРежимЗагрузки = 1 ИЛИ ТекРежимЗагрузки = 5) И ЭтоРегистр Тогда
		ТекРежимЗагрузки = 3
	ИначеЕсли ТекРежимЗагрузки = 1 ИЛИ ТекРежимЗагрузки = 5 Тогда
		ТекРежимЗагрузки = 1
	ИначеЕсли ТекРежимЗагрузки = 3 И ЭтоРегистр Тогда
		ТекРежимЗагрузки = 2;
	ИначеЕсли ТекРежимЗагрузки = 2 Тогда
		ТекРежимЗагрузки = 4;
	ИначеЕсли ТекРежимЗагрузки = 3 Тогда
	...
	Если НаименованиеСтраницы = "ТаблицаЗагружаемыхРеквизитов" Тогда
		МетаданныеИсточника = ПолучитьМетаданныеОбъектаПоКлючуНастройки(НаименованиеСтраницы);
		СтруктураСтрнаницы = Новый Структура("КлючСтраницы, ПредставлениеСтраницы", НаименованиеСтраницы, "1: " + МетаданныеИсточника.Синоним);
		НомерКолонки = 0;
	КонецЕсли;
	
	ПолучитьНастройкиДляСтраницыЗагрузки(СтруктураСтрнаницы, ТекРежимЗагрузки, ПометкаИспользованияНастройки);
...
Процедура ОчиститьСозданныеСтраницы(РежимВосстановления = Ложь)
	...
	Для каждого Настройка Из ТаблицыНастроек Цикл
		УдалитьЭлементФормы(Настройка.Ключ);	
		УдалитьРеквизитФормы(Настройка.Значение);
		Если (Настройка.РежимЗагрузки = 3 ИЛИ Настройка.РежимЗагрузки = 6) И Настройка.ЗагружаетсяДоОбъекта Тогда
		 	УдалитьРеквизитФормы(Настройка.Значение + "ОбъектЗагрузки");
		КонецЕсли;
	КонецЦикла;
...
&НаСервере
Процедура УдалитьЭлементФормы(КлючЭлемента)
...
&НаСервере
Процедура УдалитьРеквизитФормы(Реквизит)
...
Процедура ОчиститьТаблицыНастроек()
	...
	// Добавляем условное оформление для таблице настроек
	...
	// Условное оформление к строкам верхнего уровня таблицы простых настроек
	Оформление = ЭтаФорма.УсловноеОформление.Элементы.Добавить();	
	Отбор = Оформление.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	Отбор.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	Отбор.Использование = Истина;
	Отбор.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ДеревоПростыхНастроек.КлючНастройки");
	Отбор.ПравоеЗначение = "";	
	ЗначениеУО = Оформление.Оформление.Элементы.Найти("ТолькоПросмотр");
	ЗначениеУО.Значение = Истина;
	ЗначениеУО.Использование = Истина;	
	ПолеУО = Оформление.Поля.Элементы.Добавить();
	ПолеУО.Поле = Новый ПолеКомпоновкиДанных("ДеревоПростыхНастроекНомерКолонки");
	ПолеУО.Использование = Истина;
	
	Дерево = РеквизитФормыВЗначение("ДеревоПростыхНастроек");
	Дерево.Строки.Очистить();
	ЗначениеВРеквизитФормы(Дерево, "ДеревоПростыхНастроек");
	
	ТаблицаСоответствияПростыхНастроек.Очистить();
	
	СтруктураСтраницы = ДобавитьСтраницуНастроек("ТаблицаЗагружаемыхРеквизитов", "Настройка");
...
Процедура ДобавитьУсловноеОформлениеТаблицыНастроек(НаименованиеЭлемента)
	...
	Отбор.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	Отбор.Использование = Истина;
	Отбор.ЛевоеЗначение = Новый ПолеКомпоновкиДанных(НаименованиеЭлемента + ".ПолеПоиска");
	Отбор.ПравоеЗначение = Ложь;	
	ЗначениеУО = Оформление.Оформление.Элементы.Найти("ТолькоПросмотр");
	ЗначениеУО.Значение = Истина;
	ЗначениеУО.Использование = Истина;	
	ПолеУО = Оформление.Поля.Элементы.Добавить();
	ПолеУО.Поле = Новый ПолеКомпоновкиДанных(НаименованиеЭлемента + "КлючевойРеквизитПоиска");
	ПолеУО.Использование = Истина;
...
&НаСервере
Функция ПолучитьМетаданныеОбъектаПоКлючуНастройки(КлючНастройки)
	
	МетаданныеОбъекта = СтруктураМетаданныхИНастроек[КлючНастройки];
	Если ЗначениеЗаполнено(МетаданныеОбъекта) Тогда Выполнить(МетаданныеОбъекта) Иначе МетаданныеОбъекта = Неопределено КонецЕсли;
...
&НаСервере
Процедура ДобавитьНастройкиВДеревоПростыхНастроек(СтруктураСтраницы, ТаблицаНастройки)
	
	Дерево = ДеревоПростыхНастроек.ПолучитьЭлементы();
	
	СтрокаТаблицыНастроек = Дерево.Добавить();
	СтрокаТаблицыНастроек.ПредставлениеРеквизита = СтруктураСтраницы.ПредставлениеСтраницы;
	
	КолекцияНастроек = СтрокаТаблицыНастроек.ПолучитьЭлементы();
	
	Для каждого СтрокаРеквизит Из ТаблицаНастройки Цикл
		
		нСтрокаРеквизит = КолекцияНастроек.Добавить();
		ЗаполнитьЗначенияСвойств(нСтрокаРеквизит, СтрокаРеквизит);
		нСтрокаРеквизит.КлючНастройки = СтруктураСтраницы.КлючСтраницы;
		
		нСтрокаСоответствия = ТаблицаСоответствияПростыхНастроек.Добавить();
		нСтрокаСоответствия.Идентификатор = СтрокаРеквизит.Идентификатор;
		нСтрокаСоответствия.ИдентификаторСтрокиДерева = нСтрокаРеквизит.ПолучитьИдентификатор();
...
&НаКлиенте
Процедура ДеревоПростыхНастроекНомерКолонкиПриИзменении(Элемент)
	
	Если НЕ РучнаяНумерацияКолонокТабличногоДокумента Тогда
		РучнаяНумерацияКолонокТабличногоДокумента = Истина;
		КлючИЗначение = ТаблицыНастроек[0];
		Элементы[КлючИЗначение.Значение + "РучнаяНумерацияКолонокТабличногоДокумента"].Пометка = Истина;
		УправлениеВидимостью();
	КонецЕсли;

	ТекущиеДанные = Элементы.ДеревоПростыхНастроек.ТекущиеДанные;
	Если ТекущиеДанные <> Неопределено Тогда
		
		ТаблицаНастроек = ЭтаФорма[ТекущиеДанные.КлючНастройки];
		СтрокиНастроек = ТаблицаНастроек.НайтиСтроки(Новый Структура("Идентификатор", ТекущиеДанные.Идентификатор));
		Если СтрокиНастроек.Количество() Тогда
			СтрокиНастроек[0].НомерКолонки = ТекущиеДанные.НомерКолонки;	
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Процедура ИзменитьНомерКолонкиВДеревеПростыхНастроек(Идентификатор, НомерКолонки)
	
	мСтроки = ТаблицаСоответствияПростыхНастроек.НайтиСтроки(Новый Структура("Идентификатор", Идентификатор));
	Если мСтроки.Количество() Тогда
		
		Строка = ДеревоПростыхНастроек.НайтиПоИдентификатору(мСтроки[0].ИдентификаторСтрокиДерева);
		Строка.НомерКолонки = НомерКолонки;
...
&НаСервере
Процедура УстановитьПростыеНастройки(ОбработкаОбъект)
	
	Элементы.ПростаяНастройка.Видимость = ИнформацияОбОбработке.ИспользуютсяПростыеНастройки;
	
	Если ИнформацияОбОбработке.ИспользуютсяПростыеНастройки Тогда
		
		ВосстановитьНастройкуИзМакета("ПростыеНастройки_МакетНастроек")
